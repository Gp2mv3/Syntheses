...
\begin{solution}
    On repère d'abord trois cas particuliers : la liste est composée d'un seul nœud (qu'on supprime), le nœud que l'on supprime est au début et le nœud est à la fin ; enfin, on traite le cas où le nœud n'est ni au début ni à la fin. On doit traiter chaque cas séparément afin d'éviter des \code{NullPointerException}.
    \lstinputlisting{src/q8.java}

    Une autre manière de faire, si la liste est bien construite, est présenté dans le code suivant. Seule la position du nœud par rapport aux premiers et derniers nœuds est utilisée (égalité ou non). Notons que le cas où \code{n} est le seul nœud de la liste correspond à l'exécution des deux \code{if} (\code{n} est au début \emph{et} à la fin), et que le cas où \code{n} n'est ni au début ni à la fin correspond à l'exécution des deux \code{else}.
