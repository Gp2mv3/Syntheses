...
\begin{solution}
    ...
    % Proposition de réponse alternative, mais peut-être inutile pour l'examen.
\end{solution}
...
\begin{solution}
    On repère trois cas particuliers : la liste est composée d'un seul élément (qu'on supprime), l'élément que l'on supprime est au début, l'élement est à la fin ; enfin, on traite le cas où l'élément n'est ni au début ni à la fin. On doit traiter chaque cas séparément afin d'éviter des \code{NullPointerException}.
    \lstinputlisting{src/q8.java}

    Une autre manière de faire, si la liste est bien construite, est présenté dans le code suivant. Seul la position de l'élément par rapport aux premiers et derniers éléments est utilisée (égalité ou non). Notons que le cas où \code{n} est le seul élément de la liste correspond à l'exécution des deux \code{if} (\code{n} est au début \emph{et} à la fin), et que le cas où \code{n} n'est ni au début ni à la fin correspond à l'exécution des deux \code{else}.
