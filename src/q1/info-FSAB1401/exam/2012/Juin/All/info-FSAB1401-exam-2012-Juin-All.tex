...
\begin{solution}
    On vérifie bien que les arguments du constructeur sont cohérents avec des \code{assert}.
...
\begin{solution}
    Pour satisfaire la précondition, on vérifie que la playlist contient \code{songin} en parcourant la liste chainée à l'aide d'un \code{runner}. Puis, on cherche le premier nœud tel que son \code{.next()} contient \code{songin} (en traitant le \code{head} comme un cas particulier).
