...
Avec $\perm_0$ la permittivité du vide
...
En présence d'un champ électrique uniforme, il y a rotation donc il y a un couple.

L'intensité du moment de force vaut
\[ \tau = pE\sin{\theta} \]
avec $p = qd$, $q$ étant la charge de chacun des pôles, $d$ la distance les séparant et $\theta$ l'angle que forme l'axe du dipôle avec le champ.
\[ \vec{\tau} = \vec{p} \times \vec{E} \]

L'énergie potentielle d'un dipôle électrique vaut
\[ U = -\vec{p} \cdot \vec{E} \]
