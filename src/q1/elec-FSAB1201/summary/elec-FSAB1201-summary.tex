...
Le courant passant à travers le circuit est s'exprime donc
...
En appliquant la loi de Kirchokk sur l'unique boucle du circuit, on trouve la relation
...
% TODO diagramme de phaseur en tikz
...
\begin{description}
    ...
    \item[redresseur] à l'aide de diiodes, il fait en sorte que le courant passe toujours dans le même sens dans la branche d'arrivée :
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{img/bloc_alim_graph.png}
    % SOURCE https://openclassrooms.com/courses/apprenez-a-monter-votre-ordinateur/l-alimentation-2
\end{figure}

\section{Amplificateur opérationnel}

% TODO mettre les circuits en circuitikz
...
\end{figure}
Le facteur d'amplification $A$ est souvent très grand ; on considère donc que la tension entre les bornes d'entrée est proche de zéro : $v_2 - v_2 = \frac{v_\mathrm{O}}{A} \approx 0$. Notons également que le courant entrant dans les branches d'entrée est nul (comme indiqué).

\subsection{Montage inverseur}
\begin{figure}[H]
    \centering
    \includegraphics[width=.5\textwidth]{img/montage_inverseur.png}
\end{figure}
En appliquant la méthode des noeuds au point 1, on trouve la relation
\[ \frac{v_\mathrm{I} - 0}{R_1} = \frac{0 - v_\mathrm{O}}{R_2} \Rightarrow \frac{v_\mathrm{O}}{v_\mathrm{I}} = -\frac{R_2}{R_1} \]

\subsection{Montage non-inverseur}
\begin{figure}[H]
    \centering
    \includegraphics[width=.4\textwidth]{img/montage_non_inverseur.png}
\end{figure}
La tension à la borne négative vaut pratiquement $v_\mathrm{I}$ ; on pose le sens du courant de droite à gauche et on applique encore la méthode des noeud à la borne négative
\[ \frac{v_\mathrm{O} - v_\mathrm{I}}{R_2} = \frac{v_\mathrm{I} - 0}{R_1} \Rightarrow \frac{v_\mathrm{O}}{v_\mathrm{I}} = 1+\frac{R_2}{R_1} \]

\subsection{Montage intégrateur}
\begin{figure}[H]
    \centering
    \includegraphics[width=.4\textwidth]{img/montage_integrateur.png}
\end{figure}
\[ \frac{v_\mathrm{I}}{R} = -C\frac{\dif v_\mathrm{O}}{\dif t} \Rightarrow v_\mathrm{O}(t) = -\frac{1}{RC} \int v_\mathrm{I}(t) \dif t \]

\subsection{Montage différentiateur}
\begin{figure}[H]
    \centering
        \includegraphics[width=.4\textwidth]{img/montage_differentiateur.png}
    \end{figure}
    \[ C\frac{\dif v_\mathrm{I}}{\dif t} = -\frac{v_\mathrm{O}}{R} \Rightarrow v_\mathrm{O}(t) = -RC \frac{\dif v_\mathrm{I}(t)}{\dif t} \]

\subsection{Montage sommateur}
\begin{figure}[H]
    \centering
    \includegraphics[width=.8\textwidth]{img/montage_sommateur.png}
\end{figure}
La tension de sortie du premier ampli vaut (comme indiqué)
