...
\hypertitle[']{Électricité}{1}{FSAB}{1201}{2015}{Janvier}
{Mattéo Couplet}
{Jean-Didier Legat}
...
\begin{enumerate}
    \item 
    On applique la loi de Gauss sur le cyclindre de rayon variable $r$ :
    \[ \oint \vec{E} \cdot \dif \vec{l} = \frac{Q_\mathrm{encl}}{\perm_0} \]
    En tout point d'une surface cylindrique, le champ électrique est uniforme et dirigé radialement vers l'extérieur (si $\lambda > 0$). On peut donc simplfier
...
\begin{circuitikz}[american]
    \draw (0, 0) 
    to[V=\SI{10}{\volt}] (0, 3)
    to[R=\SI{5}{\kilo\ohm}] (3, 3) node[label={above:$V_x$}] {}
    to[V=$2V_x$] (6, 3)
    to[R=\SI{10}{\kilo\ohm}] (6, 0)
    to[short] (0, 0);
...
\begin{enumerate}
    \item 
On applique la méthode des noeuds sur le noeud $V_x$ :
...
\begin{center}
\begin{circuitikz}[american]
    \draw (0, 0) to[short] (13, 0) node[label={below:B}] {}
    ...
    to[R=\SI{4}{\kilo\ohm}] (13, 3) node[label={above:A}] {};
...
\begin{solution}
On sait qu'aucun courant ne traverse la résistance de \SI{4}{\kilo\ohm}, donc la tension de Thévenin équivaut à la tension du points juste à gauche. On applique donc en ce point la méthode des noeuds
...
\begin{center}
    \begin{circuitikz}[american]
        ...
        \draw (5, 0) to[R=\SI{2}{\kilo\ohm}] (5, 2) to[L=\SI{12}{\milli\henry}] (5, 4) node[label={above:$V_x$}] {};
...
\begin{solution}
    \begin{enumerate}
        \item 
            ...
            \[ I_L(t) = A+B e^{-t/\tau} \]
            Commençons par calculer $I_L(0^-) = A+B$ ; l'interrupteur est ouvert et l'inductance se comporte comme un court-circuit. On applique tout d'abord la méthode des noeuds en $V_x$ :
            ...
            Ensuite, on cherche $I_L(\infty) = A$ ; l'interrupteur est fermé et l'inductance se comporte comme un court-circuit. Encore une fois, méthode des noeuds en $V_x$ :
        ...
        \item Un circuit adéquat est le montage sommateur suivant
            \begin{center}
                ...
                \begin{circuitikz}[american]
                    ...
                    to[short,-*] ++(1, 0) node[label={above:$V_\mathrm{out}$}] {}
                    ;
                \end{circuitikz}
            \end{center}
