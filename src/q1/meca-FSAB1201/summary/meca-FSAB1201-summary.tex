...
{Nicolas Cognaux\and Benoît Legat}
{Roland Keunings}
%    _
%   /A\
%  |   |
...
Un référentiel intertiel ou galiléen est un référentiel dans lequel la première loi de Newton est d'application ; ainsi, tout corps libre est à l'équilibre. Un référentiel qui est accéléré par rapport à un référentiel inertiel n'est pas inertiel.
...
Le travail effectuée sur une particule équivaut au changement en énergie cinétique de cette particule 
...
= \sqrt{\frac{2GM}{r}} \]
\[ v_{\text{libération}} = \sqrt{2}\ v_\mathrm{orbitale} \]
...
\begin{align*}
  m(A\sin{(\omega{t} + \phi)})'' & = - kA\sin{(\omega{t} + \phi)}\\
  -mA\omega^2\sin{(\omega{t} + \phi)} & = - kA\sin{(\omega{t} + \phi)}\\
  m\omega^2 & = k
\end{align*}
On trouve alors
\[ \omega = \sqrt{\frac{k}{m}} \]
...
Soit
\[ F = ma = m\frac{d^2x}{dt^2} = m\frac{Ld^2\theta}{dt^2} = -mg\sin{(\theta )} \]
...
On peut ensuite trouver $\omega$ en injectant la solution dans l'équation
\begin{align*}
  L(\theta_0\sin{(\omega{t} + \phi)})'' & = -g\theta{(t)}\\
  -L\theta_0\omega^2\sin{(\omega{t} + \phi)} & = -g\theta_0\sin{(\omega{t} + \phi)}\\
  L\omega^2 & = g
\end{align*}
On trouve alors
\[ \omega = \sqrt{\frac{g}{L}} \]
