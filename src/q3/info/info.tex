...
En sortant du language noyau,
on peut utiliser un raccourcis assez pratique pour éviter
d'écrire le mot local.
Après les \keyword{} \lstinline|proc|, \lstinline|fun| et
...
La ligne 5 s'écrirait en language noyau en
\begin{lstlisting}
local D E F in
  D = @C
  E = 1
...
séquentielle \lstinline|<v> = <s> <v>_1| car c'est faux car en language
...
du moins avec les languages classiques comme le \java{}.
...
Il faut qu'\emph{en language noyau},
...
Par chance, lors de la traduction en language noyau automatique,
...
\end{lstlisting}
n'est pas récursif terminal car en language noyau, ça donne
\begin{lstlisting}
proc {Sum N R}
  local Rec Z Eq in
    Z = 0
...
Comme \oz{} est malin, il écrit alors le language noyau comme suit
\begin{lstlisting}
fun {ListNums N R}
  local Z Eq in
    Z = 0
