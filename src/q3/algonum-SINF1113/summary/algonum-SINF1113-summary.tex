...
Dans la représentation fixed-point, un nombre $x$ est représenté par $2^k * x$. Par exemple :
\begin{itemize}
...
\begin{figure}[!ht]
	...
	\caption{Précision du standard IEEE754}
\end{figure}
...
\begin{figure}[!ht]
	...
	\caption{Performances du tiling}
\end{figure}
...
\end{itemize}
On ne fait donc que $7$ multiplications (une multiplication est plus \og chère \fg{} qu'une addition : Voir cours d'élec de première) donc la complexité est de $O(2^{2.81})$ pour pour l'algorithme de Strassen. \newline
...
Une \textbf{matrice creuse} (\emph{sparse matrix} en anglais) est une matrice très grande où la plupart des entrées sont $0$, on peut les représenter comme 
\begin{itemize}
    ...
    \begin{itemize}[label={$\to$}]
        \item Ligne 1 = $\{0,0,0,4,5,0,0,0,0,0,1\}$
...
L'élimination de Gauss-Jordan permet d'obtenir une matrice échelonnée (réduite). Pour résoudre un système $AX = B$, on va
\begin{enumerate}
...
\begin{enumerate}
    ...
    \item Répéter les étapes précédentes avec la ligne suivante.
\end{enumerate}
...
\begin{figure}[!ht]
	...
	\caption{Problème avec la méthode de Vandermonde}
\end{figure}
...
\begin{figure}[!ht]
	...
	\caption{Interpolation en escalier}
\end{figure}
Linéairement :
\begin{figure}[!ht]
	\centering
...
\begin{figure}[!ht]
	...
	\caption{Cercle avec courbes paramétriques}
\end{figure}
...
\begin{figure}[!ht]
	...
	\caption{Interpolation de Riemann}
\end{figure}
...
\begin{figure}[!ht]
	...
	\caption{Closed points VS open points}
\end{figure}
...
\begin{figure}[!ht]
	...
	\caption{Méthode de composition}
\end{figure}
...
\begin{figure}[!ht]
	...
	\caption{Intégration adaptative}
\end{figure}
...
\begin{figure}[!ht]
	...
	\caption{One-sided forward differencing}
\end{figure}
...
\begin{figure}[!ht]
	...
	\caption{Two sided forward differencing}
\end{figure}
...
Une fois le système transformé, on résout le système très facilement en résolvant toutes les équations à chaque itération. D'abord, on commence par la dérivée la plus élevée et on finit par F(x).


\section{Formulaire}
\newpage
\begin{figure}
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{img/formulaire.jpeg}
\end{figure}
