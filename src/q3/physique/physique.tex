...
L'équation d'onde à une dimension est donnée par 
$$v^2\ffpart{f(x,t)}{x} = \ffpart{f(x,t)}{t}$$
où $v$ est la vitesse de propagation.
La solution d'une telle équation est de la forme 
$f(x,t) = f(x \pm vt)$.

\subsection{Onde sinusoidale}
Une onde sinusuoidale a l'équation suivante
...
\subsubsection{Propagation d'une onde électromagnétique dans le vide}
Contrairement aux ondes mécaniques, les ondes électromagnétiques
n'ont pas besoin d'un milieu pour se propager. Pour prouver cela,
utilisons les équations de Maxwell dans le vide, et plus particulièrement

\begin{align*}
  \rotn{\E} & = -\mu_0\fpart{\vec{H}}{t} \\
  \rotn{\vec{H}} & = \perm_0\fpart{\E}{t}. 
\end{align*}

Comme on sait que $\vec{E} \perp \vec{H}$, appliquons ces deux équations
à une onde électromagnétique plane où $\vec{E}$ ne possède qu'une composante
$E_y$ et $\vec{H}$ une composante $H_z$. En calculant 
les rotationnels et en égalant chaque côté des deux équations composante par
composante, on trouve\footnote{On trouve également que $E_y$ et $H_z$ ne dépendent
que de $x$.}

\begin{align*}
  \fpart{E_y}{x} & = -\mu_0\fpart{H_z}{t} \\
  \fpart{H_z}{x} & = -\perm_0\fpart{\E_y}{t}. 
\end{align*}

Ces deux équations forment les équations de propagation de $\vec{E}$ et $\vec{H}$.
Elles prouvent qu'un champ magnétique variable en fonction du temps
implique la présence d'un champ électrique variable en fonction de l'espace et
vice-versa. En prenant la dérivée partielle par rapport à $t$ (resp. $x$)
de la première équation et par rapport à $x$ de la deuxième équation (resp. $t$),
on obtient les deux équations d'ondes 

\begin{align*}
   \ffpart{H_z}{t} & = \frac{1}{\perm_0\mu_0}\ffpart{H_z}{x}\\
   \ffpart{E_y}{t} & = \frac{1}{\perm_0\mu_0}\ffpart{E_y}{x}. 
\end{align*}

En comparant ces équations d'ondes à la forme générale de l'équation
d'onde, on retombe bien sur $c = \frac{1}{\sqrt{\perm_0\mu_0}}$.

\subsubsection{Impédance caractéristique}
On définit l'impédance caractéristique d'une onde dans un milieu
de propagation  

$$Z = \frac{E}{H} = \sqrt{\frac{\mu}{\perm}}.$$

Dans le vide, cette impédance vaut $Z_0 =\unit{377}{\ohm}$.
...
\begin{enumerate}
	...
	de réfraction $n_0$, dans ce milieu l'onde a une longeur 
	d'onde $\lambda_0$ ;
	\item Le milieu 1 (dans le film) d'indice de réfraction 
	$n_1$, dans ce milieu l'onde a une longeur d'onde $\lambda_1$ ;
	\item Le milieu 2 (en dessous du film) d'indice de réfraction 
	$n_2$, dans ce milieu l'onde a une longeur d'onde $\lambda_2$ ;
\end{enumerate}
...
incidents (le rouge et le bleu sur la figure \ref{fig:film-mince})
est donné par 
