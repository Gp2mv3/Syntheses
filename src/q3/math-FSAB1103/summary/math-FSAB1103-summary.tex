...
{Gaetan Cassiers\and Beno\^it Legat\and Antoine Paris}
...
\begin{itemize}
	\item $\lambda = -K^{2}$ nous donnera une solution du type $\Theta = A \cos(K\theta) + B \sin(K \theta)$. Pour trouver R nous voyons que nous avons une équation d'Euler:
	
	$$\sum^{N}_{i = 0} A_{i} x^{i} \frac{d^{i}y}{dx^{i}}= 0$$
	
	nous trouvons dans notre cas
	$$R_{n} = C_{n} r^{k_{n}} + D_{n} r^{-k_{n}} $$ 
	\begin{myrem}
		si la solution est définie en r = 0 alors nous avons $D_{n}$ = 0	
	\end{myrem}
	\medbreak
	\item $\lambda = 0$ nous donnera une solution du type $\Theta = A_{0} \theta + B_{0}$. Nous trouvons pour R que
	
	$$r R'' + R' = 0$$
	
	qui a comme solution générale:
	
	$$ R_{n} = C_{n} \log(r) + D_{n} $$
...
\end{itemize}
	...
	\begin{myprop}
		...
		\subsection{L'équation d'onde par séparation de variable}
			...
			Imaginons maintenant que $\Omega$ soit un rectange de dimension $L \times H$
			...
			avec les diverses conditions initiales il est alors possible de trouver les coefficients comme précédement.
			...
			\begin{myrem}
				Dans le cas ou l'originie ne ferait pas partie du domaine on ne peut néglier les fonctions Y
			\end{myrem}
...
% TODO : équation d'onde par séparation de variable, équation de diffusion
% par séparation de variable.
...
\begin{mydef}[Argrument principal $\Arg(z)$]
...
\begin{mydef}[Exponentielle complexe]
