...
\begin{solution}
		\begin{enumerate}
			...
			On trouve le graphe de la solution à la figure \ref{fig:diff}
					\begin{solfig}{diff}{u(x,t) pour différent temps.}
...
\begin{enumerate}
	...
	\item On peut écrire
	$$ w = \frac{1}{i}(\log(\sqrt{(iy)^2-1}+i) - \log(z)) $$
	...
	\item \[\eta(x) = \sqrt{x^2-1}+i=xe^{i\arctan\left(\frac{1}{\sqrt{x^2-1}}\right)}\]
		\[w(x) = \frac{1}{i} \log\left(\frac{xe^{i\arctan\left(\frac{1}{\sqrt{x^2-1}}\right)}}{x}\right) = \tan\left(\frac{1}{\sqrt{x^2-1}}\right)\]
