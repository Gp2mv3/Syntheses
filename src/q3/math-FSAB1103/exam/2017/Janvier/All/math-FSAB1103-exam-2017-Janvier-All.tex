...
1. Donnez les unités de $\alpha$ et $\beta$. \\ \\
2. Obtenez le réseau de caractéristiques (équation et esquisse). L'esquisse doit être réalisée pour $s=0,L/2$ et $L$, avec des axes adimensionnels (\textbf{Aide}: $e^{1/8}\simeq 1,13$ et $e^{1/2}\simeq 1,65$). Pourquoi ne peut-on pas imposer de condition limite en $x=0$?\\ \\
3. Donner la solution $u(x,t)$ pour les 2 morceaux. A partir de quel temps adimensionnel la solution commence-t-elle à croître?\\ 
...
En traçant la solution pour x fixé, on voit bien qu'elle commence à croître pour $\frac{\alpha t}{\beta}=1/2$ et $0\leqslant s \leqslant L$:\\
...
1) $u_p(a,\theta)=0$\\
2) $u_p(r,\beta)=u_0\ln(\frac{r}{a})$\\
3) $\frac{\partial u_p}{\partial r}(b,\theta)=f(\theta)$ (une fonction inconnue)\\
4) $u_p(r,0)=0$ \\ \\
...
1) $u_h(a,\theta)=0$\\
2) $u_h(r,\beta)=0$\\
3) $\frac{\partial u_h}{\partial r}(b,\theta)=\frac{u_0}{b}g(\theta)-f(\theta)=\frac{u_0}{b}g(\theta)-\frac{u_0\theta}{\beta b}$\\
4) $u_h(r,0)=0$ \\ \\
...
$$u_{h,n}(b,\theta)= E_n \sin (k_n\theta)\Bigg(\Big(\frac{r}{a}\Big)^k-\Big(\frac{r}{a}\Big)^{-k}\Bigg)$$
avec $E=B_n^*C$.
Enfin, par la principe de superposition des solutions,
$$u_h(b,\theta)= \sum\limits_{n=1}^\infty E_n \sin (k_n\theta)\Bigg(\Big(\frac{r}{a}\Big)^k-\Big(\frac{r}{a}\Big)^{-k}\Bigg)$$
...
Il est utile de vérifier les unités, en sachant que les arguments des fonctions (sinus, intégrales, exposants) sont adimensionnels.\\
...
La solution homogène fonctionne tout aussi bien, l'argument du sinus n'a pas d'unité, tout comme $r/a$, et $E_n$ a les unités de $u$.
...
$$0<\lim_{R \to \infty}|f(z)z|=\lim_{R \to \infty}|\frac{\sqrt{Re^{i\theta}}}{(1+Re^{i\theta})(2+Re^{i\theta})}Re^{i\theta}|$$
$$\leqslant \lim_{R \to \infty}|\frac{R^{1/2}e^{i\theta/2}}{(R-1)(R-2)}R|=\lim_{R \to \infty}|\frac{R^{3/2}}{(R-1)(R-2)}|=0 $$
en posant $z=Re^{i\theta}$. L'inégalité de Cauchy est importante pour ce calcul:
$$|z_1+z_2|\leqslant||z_1|-|z_2||$$
Elle permet cela:
$$|1+Re^{i\theta}|\leqslant|1-R|=|R-1|$$
