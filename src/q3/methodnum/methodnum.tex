...
\begin{myrem}\label{rem:borne}
	...
	$C_{n+1}$ qui borne la dérivée d'ordre $n+1$ de $u(x)$ (dans certains cas c'est très simple, comme
	...
	$$|e^h(x)| \leq \frac{C_{n+1}}{(n+1)!} \cdot n! \frac{h^{n+1}}{4} = \frac{C_{n+1}}{4(n+1)}h^{n+1}.$$
...
ce résultat de convergence pour des fonctions $u(x)$ dont les dérivées d'ordre $n+1$ sont
bornées sur le domaine d'interpolation par une constante $C_{n+1}$ (e.g $\sin(x)$, $\cos(x)$, $e^x$).
En effet, en se rappelant que $h = \frac{b-a}{n}$, on a bien 

$$\lim_{n \to \infty} e^h(x) = C_{n+1} \lim_{n \to \infty} \frac{1}{4(n+1)}(\frac{b-a}{n})^{n+1} = 0.$$
