...
que la méthode numérique pour la résoudre soit stable.
...
Si une méthode à pas séparé a une erreur locale de discrétisation d'ordre
$\bigoh(h^{n+1})$, son erreur globale sera $\bigoh(h^n)$.

\paragraph{Problèmes raides}
Pour le problèmes raides, les méthodes implicites sont meilleurs que les
méthodes explicites.
...
\begin{center}
  \begin{tabular}{|l|ccc|}
    ...
    Ordre & 1 & 1 & 2\\
    Stabilité & $|1 + h\lambda| < 1$ & $1 < |1 - h\lambda|$ & $|h\lambda| < 0$\\
    \hline
  \end{tabular}
\end{center}
