...
\hypertitle{M\'ethodes num\'eriques}{3}{FSAB}{1104}{Benoît Legat}{Benoît Legat}
...
problème de Cauchy, on analyse la stabilité du problème modèle
\[ u' = \lambda u \]
où $\lambda = \left.\fpart{f}{u}\right|_{(x, u)}$
au lieu du problème de départ
\[ u' = f(x, u). \]

En effet, supposons qu'on ait une erreur $e_i$ au pas $i$, on a
\begin{align*}
  e_{i+1} & = U_{i+1} - u(X_{i+1})\\
  & = f(X_i, u(X_i) + e_i) - f(X_i, u(X_i))\\
  & = \left.\fpart{f}{u}\right|_{(X_i,U_i)} e_i
\end{align*}

Par exemple, pour Crank-Nicolson, on a
\begin{align*}
  U_{i+1} & = U_i + \frac{h}{2} (f(X_i, U_i) + f(X_{i+1}, U_{i+1}))\\
  & = U_i + \frac{h\lambda}{2} (U_i + U_{i+1})\\
  & = U_i \frac{2 + h\lambda}{2 - h\lambda}
\end{align*}
d'où
\begin{align*}
  e_{i+1} & = U_{i+1} - u(X_{i+1})\\
  & = \frac{2 + h\lambda}{2 - h\lambda} (u(X_i) + e_i - u(X_i))\\
  & = \frac{2 + h\lambda}{2 - h\lambda} e_i
\end{align*}
On impose donc
