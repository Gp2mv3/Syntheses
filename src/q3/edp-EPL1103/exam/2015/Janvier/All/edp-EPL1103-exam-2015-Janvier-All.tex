...
\hypertitle{Math\'ematique}{3}{FSAB}{1103}{2015}{Janvier}{All}
{Martin Braquet \and Simon Demaré}
...
\subsection*{Aide}
\begin{align*}
\int \cos{(a \eta)} \dif \eta &= \dfrac{1}{a} \sin{(a\eta)}\\
\int \dfrac{ \dif \eta}{\cos{(a \eta)}} &= \dfrac{1}{a} \cdot \ln\Big|\tan\Big(\dfrac{a \eta}{2} + \dfrac{\pi}{4}\Big)\Big|.
\end{align*}
...
\begin{solution}
  ...
  \[ \oint_C f(z)\: \dif z = 2\pi\sum_i \mathrm{Res}(p_i) \]

  On prouve à l'aide des lemmes de Jordan que l'intégrale sur l'encoche et sur le contour du demi-cercle tend vers $0$.
  
  Il ne reste plus que deux intégrales sur l'axe réel:
  
  \[ \int_0^\infty f(z)\: \dif z + \int_{-\infty}^0 f(z)\: \dif z = 2\pi \:\mathrm{Res}(\imag) \]
  ...
  On a donc
  \[ \phantom{.} \left(1 + (-1)^p\right)\int^{\infty}_0 \frac{x^p}{1+x^2} \dif x = -\frac{\imag^{p+1}}{2} 2\pi \imag = \pi \imag^p, \]
  d'où
    $$\phantom{,} I =\frac{\pi \imag^{p}}{1 + (-1)^p}=\frac{\pi \e^{\imag \pi p/2}}{1 + \e^{\imag \pi p}}=\frac{\pi}{\e^{-\imag \pi p/2} + \e^{\imag \pi p/2}}=\frac{\pi}{2\cos(p\pi/2)}.$$
    
    Il est toujours utile de vérifier que la solution finale soit un nombre réel, puisque qu'il s'agit bien entendu de l'intégrale d'une fonction réelle. 
\end{solution}
