...
\begin{itemize}
  ...
  \item
    ...
    $T(\kelvin) = \theta(\celsius) + \numprint[\kelvin]{273.15}$;
  \item
    Le volume molaire (ou massique) $V_m = \frac{V}{n}$ ($\nu = \frac{V}{m}$)
    exprimé en $\meter\cubed\per\mole$ ($\meter\cubed\per\kilogram$).
\end{itemize}
...
\numprint[\kelvin]{0} et tous les gaz sont liquéfiés ou
  ...
  \begin{center}
    \begin{tikzpicture}[x=3cm,y=1cm]
      ...
      {\begin{minipage}{3cm}\scriptsize
        Near the dissociation temperature at \numprint[\kelvin]{3200}
...
$C_{vm} = \frac52 R$ au lieu de $C_{vm} = \frac72 R$.
...
et vaut \numprint[\joule\per\kilogram\kelvin]{287.1} dans le cas de l'air.
Finalement,
\[ pV = mR^*T \]
...
\subsubsection{travail des forces externes à distance}
