...
$N_A = \frac{N}{n}$ le nombre d'Avogadro\footnote{$N_A$ est donc le nombre de molécule par mole.} valant $\si{6.02214e23}{}$,
...
Des erreurs d'unités sont fréquemment commises en utilisant cette célèbre équation. C'est pourquoi il est utile de rappeler que si on utilise la valeur de $R$ fournie ci-dessus, cela sous-entend que l'on travaille dans le système d'unités SI. $p$ est donc en $\pascal$, $V$ en $\meter\cubed$, $n$ en $\mole$ bien sûr, $T$ en $\kelvin$ et l'unité de $R$ vous est déjà fournie ci-dessus.
