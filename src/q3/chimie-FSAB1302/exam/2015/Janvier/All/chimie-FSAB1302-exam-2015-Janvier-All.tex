...
\begin{enumerate}
  ...
  \item Tout autre paramètre étant constant, quel rapport de détente serait le rendement
...
\begin{solution}
	...
	\begin{center}
		...
		\end{tabular}
	\end{center}
	$$m = \frac{PV}{R^*T} = 0.0012 $$ 
	
	Avec la transformation 1-2 étant isentropique
	$$T_2 = \exp\left(-\frac{R^*\ln(\frac{V_f}{V_i})}{c_v}\right)T_1 =754 $$
	
	Avec la transformation 2-3 étant isochore
	$$ T_3  = T_2 + dT = T_2 + \frac{Q}{c_v} = 3541$$
	
	Avec la transformation 3-4 étant isentropique
	$$T_4 = \exp\left(-\frac{R^*\ln(\frac{V_f}{V_i})}{c_v}\right)T_3 =1268 $$
	
	Nous avons deux variables pour chaque lignes nous pouvons maintenant remplir le tableau
	...
	\begin{center}
		\begin{tabular}{|c|c|c|c|c|}
			...
			2&25.1&753&0.0001&0\\
			\hline
			3&118&3541&0.0001&$S_3$\\
			\hline
			4&3.25&1268&0.0013&$S_3$\\
			\hline
			5&1&390&0.0013&$S_5$\\
			\hline
		\end{tabular}
	...
	$$ S_3 = c_p \ln(\frac{T_f}{T_i}) = 1.29 $$
	$$ S_5 = S_3 + c_p \ln(\frac{T_f}{T_i}) = 0.31 $$
	
	$$Q_{2-3} = m cv \Delta T = 2321 $$
	$$Q_{4-5} = m cv \Delta T = -732 $$
	$$Q_{5-1} = m cp \Delta T = -105 $$
	
	$$W_{tot} = - \sum Q = \SI{1484}{\joule}$$
	$$\eta = \frac{W_{tot}}{Q_c} = 0.64$$
	
	La chaleur $Q_C$ ne variant pas avec le rapport $\alpha = \frac{V_4}{V_3}$ il suffit de minimiser le travail total
	$$W_{tot} = \frac{P_2V_2 - P_1V_1}{\gamma - 1} + \frac{P_3V_3 - P_4V_4}{\gamma - 1} +  10^5 * (V_1 - V_4)$$
	$$W_{tot} = \frac{P_2V_2 - P_1V_1}{\gamma - 1} + \frac{P_3V_3 - P_3 \alpha ^{-\gamma}V_3 * \alpha}{\gamma - 1} +  10^5 * (V_1 - \alpha V_3)$$
	$$W_{tot}' = - P_3 \alpha ^{-\gamma}V_3  - 10^5 * \alpha V_3 = 0$$
	$$\alpha = (P_3)^{1/1.4} = 30 $$
	
	$$T_4 = \exp\left(-\frac{R^*\ln(\frac{V_f}{V_i})}{c_v}\right)T_3 =906 $$
	$$ W_{tot} = 1621$$
	$$\eta = 0.70$$
