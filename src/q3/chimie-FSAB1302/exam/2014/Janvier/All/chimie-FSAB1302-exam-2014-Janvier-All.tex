...
\newcommand{\estd}[1]{E\std_{\textnormal{#1}}}
...
Discutez l’avantage éventuel que vous voyez dans l’utilisation d’un $\ph$ faible.
    \item Déduisez de ce qui précède le $\Delta_r G\std$ de la réaction globale et vérifiez si votre résultat est en accord
...
Discutez ce qui se passerait si, en plus des cations $\ce{H+}$ (ou $\ce{H3O+}$
...
\begin{enumerate}
    ...
    \item 
    $$\phantom{.} E_{\textnormal{cell}}=E_{\textnormal{cathode}}-E_{\textnormal{anode}}=\eca-\ean= \SI{-1.23}{\volt}.$$
    ...
    $$\ce{Cu++ +2e- -> Cu} \qquad E\std = \SI{0.34}{\volt}$$
    La réaction globale est alors:
    $$\ce{H2O + Cu++ -> (1/2) O2 + 2H+ + Cu} \qquad E\std = \num{0.34} - \num{1.23} = \SI{-0.89}{\volt}$$
    La force électromotrice de celle-ci est
    $$E_{\textnormal{cell}}= \num{-0.89} + \frac{RT}{2F}\ln\ce{[Cu^{++}]}$$
    Cette réaction va donc se faire préférentiellement jusqu'à ce son potentiel vaille $\SI{-1.23}{\volt}$. 
