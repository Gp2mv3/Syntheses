...
\begin{itemize}
	...
	\item Chaque fonction ou procédure récursive doit réaliser une seule boucle. Si on fait plusieurs boucles dans une même fonction, le nombre d'arguments augmente beaucoup au moins deux  arguments en plus par boucle) et le corps de la fonction devient beaucoup plus compliqué (i ly a une masse de conditionnels (\lstinline|if| et \lstinline|case|) imbriqués)
\end{itemize}
...
\begin{itemize}
	...
	\item Définissez la fonction \lstinline|Unflatten|. L'appel \lstinline|{Unflatten L}| avec une liste $L = [a_0\ a_1\ \ldots\ a_{n-1}]$ dont les éléments sont des entiers ou des atomes, renvoie une nouvelle lite \lstinline|M| qui peut contenir des listes. La transformation de \lstinline|L| en \lstinline|M| se fait dans l'ordre des éléments de \lstinline|L| et suit les trois règles suivantes:
	\begin{enumerate}
		\item La sous séquence $k\ a_1\ a_2 \ldots _k$ est en transformée en $[a_1\ a_2 \ldots a_k]$. Ici, k est une entier non-négatif qui donne le nombre d'éléments consécutifs à traiter.
...
\end{itemize}
	N'hésitez pas à définir des fonctions auxiliaires pour simplifier au maximum vos définitions. Pour chaque fonction auxiliaire il faut donner la spécification. Si vos fonctions auxiliaires ne font pas partie du système Mozart vous devez donner leur définitions dans votre réponse.
...
75
	...
	\lstinputlisting{Q2Sol.oz}
		...
		\begin{enumerate}
			...
			\item
			$(\{[(F1 = proc\{\$\text{ }X\text{ }R\}R = A + X end,\{A \rightarrow a\}),\dots ]\},\{a = 1,f1\},\{\})$
...
\begin{solution}
	\begin{itemize}
		\item Complexité temporelle avec la notation grand Ol On dit que la complexité temporelle est $O(g(n))$ si il existe C appartenant à R tel que: $ f(n) \leq Cg(n)$. C'est à dire que g(n) est une approximation pessimiste de f(n)p à une facteur près. C'est la complexité du pire cas.
...
{Browse {CreateL 10}}
\end{lstlisting}
est de complexite $O(n)$
\item Arbre binaire ordonné:
Un arbre est composé d'un nœud (record) ou d'une feuille (leaf). Dans un arbre binaire, chaque nœud possède 2 branches. L'arbre est dit ordonne si, pour chaque nœud, la clef du sous arbre de gauche(respectivement droite) est inférieur (respectivement supérieur) à la clef du noeud.
...
\item Portée d'une occurrence d'un identificateur: C'est la région de programme dans laquelle l'identificateur est lie a une certaine variable. Elle peut être contrôlée par l'instruction \lstinline|local <x> in <s> end|. Dans ce cas, la porte est dite lexicale ou statique.
