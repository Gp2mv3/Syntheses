...
\begin{enumerate}[label=(\alph*)]
	...
	\[ CE_{mc2} = \emptyset \]
	\[ CE_{p1} = \{ \} \]
	\item Exécutons le programme pas par pas.
	\begin{enumerate}
		...
		\item
		\[ \bigg( \big[ (<l2-l38>, E_2=E_1 \cup \{ \texttt{MC} \rightarrow mc, \texttt{C} \rightarrow c \} ) \big], \sigma_2 = \sigma_1 \cup \big\{ mc, c \big\}, \mu_1 \bigg) \]
		Exécution du local.
		\item
		\[ \bigg( \big[ (<l2-l5>, E_2), (<l6-l38>, E_2) \big], \sigma_2, \mu_1 \bigg) \]
		Composition d'instruction. Ne l'oubliez pas à l'examen !
		\item
		\[ \bigg( \big[ (<l3-l4>, E_3=E_2 \cup \{ \texttt{Zero} \rightarrow z \}), (<l6-l38>, E_2) \big], \sigma_3 = \sigma_2 \cup \big\{ z \big\}, \mu_1 \bigg) \]
		Exécution du local.
		\item Avant la création d'une cellule.
		\[ \bigg( \big[ (\texttt{\{NewCell Zero C\}}, E_3), (<l6-l38>, E_2) \big], \sigma_4 = \sigma_2 \cup \big\{ z=0 \big\}, \mu_1 \bigg) \]
		Composition d'instructions suivie par une création de valeur pour affecter $z$.
		\item Après la création d'une cellule.
		\[ \bigg( \big[(<l6-l38>, E_2) \big], \sigma_5 = \sigma_4 \cup \big\{ c = \gamma \big\}, \mu_2 = \mu_1 \cup \{ c\colon z \} \bigg) \]
		La cellule a été créée et est affectée au nom $\gamma$, et a comme contenu $z=0$.
		\item Avant la déclaration d'une procédure
		\[ \bigg( \big[ (<l6-l34>, E_2), (<l35-l38>, E_2) \big], \sigma_5, \mu_2 \bigg) \]
		Composition d'instruction.
		\item Après la déclaration d'une procédure
		\[ \bigg( \big[ (<l35-l38>, E_2) \big], \sigma_6 = \sigma_5 \cup \big\{ mc = (\procc{E R1} <l7-l33> \End, CE_{mc}) \big\}, \mu_2 \bigg) \]
		...
		\[ \bigg( \big[ (<l36>, E_4 = E_2 \cup \{ \texttt{R2} \rightarrow r2 \}), (<l37>, E_4) \big], \sigma_7 = \sigma_6 \cup \big\{ r2 \big\}, \mu_2 \bigg) \]
	...
	%\[ \bigg( \big[ (<l8-l32>, E_6 = E_5 \cup \{ \texttt{D} \rightarrow d, \texttt{MC2} \rightarrow mc2 \}), (<l37>, E_4) \big], \sigma_7 \cup \big\{ mc2, d \big\}, \mu_2 \bigg) \]
	%\[ \bigg( \big[ (<l8-l11>, E_6), (<l12-l32>, E_6 = E_5 \cup \{ \texttt{D} \rightarrow d, \texttt{MC2} \rightarrow mc2 \}), (<l37>, E_4) \big], \sigma_7 \cup \big\{ mc2, d \big\}, \mu_2 \bigg) \]
	%\[ \bigg( \big[ (<l12-l14, E_6), (<l15-l32>, E_6 = E_5 \cup \{ \texttt{D} \rightarrow d, \texttt{MC2} \rightarrow mc2 \}), (<l37>, E_4) \big], \sigma_7 \cup \big\{ mc2, d=\delta, one=1 \big\}, \mu_3 = \mu_2 \cup \big\{ d\colon one \big\} \bigg) \]
	%\[ \bigg( \big[ (<l15-l20>, E_6), (<l21-l32>, E_6 = E_5 \cup \{ \texttt{D} \rightarrow d, \texttt{MC2} \rightarrow mc2 \}), (<l37>, E_4) \big], \sigma_7 \cup \big\{ mc2=(\procc{P C} <l13> \End, \emptyset), d=\delta, one=1 \big\}, \mu_3 = \mu_2 \cup \big\{ d\colon one \big\} \bigg) \]
	%\[ \bigg( \big[ (<l16-l19>, E_8 = E_6 \cup \{ \texttt{V1} \rightarrow v1, \texttt{V2} \rightarrow v2, \texttt{V3} \rightarrow v3 \})(<l21-l32>, E_6 = E_5 \cup \{ \texttt{D} \rightarrow d, \texttt{MC2} \rightarrow mc2 \}), (<l37>, E_4) \big], \sigma_7 \cup \big\{ mc2=(\procc{P C} <l13> \End, \emptyset), d=\delta, one=1, v1, v2, v3 \big\}, \mu_3 = \mu_2 \cup \big\{ d\colon one \big\} \bigg) \]
	\[ \left( \begin{bmatrix}
	(<l17-l19>, E_8 = E_6 \cup \{ \texttt{V1} \rightarrow v1, \texttt{V2} \rightarrow v2, \texttt{V3} \rightarrow v3 \}), \\
	(<l21-l32>, E_6 = E_5 \cup \{ \texttt{D} \rightarrow d, \texttt{MC2} \rightarrow mc2 \}), \\
	(<l37>, E_4)
	\end{bmatrix}, \right. \]
	\[ \left. \sigma_8 = \sigma_7 \cup \big\{ mc2=(\procc{P C} <l13> \End, \emptyset), d=\delta, one=1, v1=1, v2, v3 \big\}, \mu_3 = \mu_2 \cup \big\{ d\colon one \big\} \right) \]
	...
	\[ \bigg( \big[ (<l18-l19>, E_8), (<l21-l32>, E_6), (<l37>, E_4) \big], \sigma_9 = \sigma_8 \cup \big\{ v2=z \big\}, \mu_3 \bigg) \]
	Lecture d'une cellule. L'identificateur \texttt{E} réfère à la cellule $c$, dont le contenu est la variable $z$, qui vaut $0$. Par conséquent, la variable $v2$ (référencée par \texttt{V2}) est donc affecté à la valeur de $z$, c'est-à-dire $0$. Il est important de noter que $v2$ ne contient pas directement la valeur $0$.
	\item Avant l'affectation d'une cellule
	\[ \bigg( \big[ (<l19>, E_8), (<l21-l32>, E_6), (<l37>, E_4) \big], \sigma_10 = \sigma_9 \cup \big\{ v3=1 \big\}, \mu_3 \bigg) \]
	Composition, exécution de la somme et donc création de valeur pour $v3$.
	\item Après l'affectation d'une cellule
	\[ \bigg( \big[ (<l21-l32>, E_6), (<l37>, E_4) \big], \sigma_10, \mu_4 = \mu_3 \cup \big\{ c:v3 \big\} = \big\{ c:v3, d:one \big\} \bigg) \]
	Composition, exécution de la somme et donc création de valeur pour $v3$. Il est important de noter que $c$ ne contient pas directement la valeur $1$ de $v3$.
	...
	\item Composition. Déclaration de \texttt{P1}. Composition. Création de la valeur procédurale des lignes 23 à 27, affectée dans $p1$. Appel de la procédure $mc2$ : la ligne 13 est sur la pile sémantique, avec $C$ qui réfère à $c$, et $P$ qui réfère à $p1$. Appel de la procédure $p1$ : les lignes 23 à 27 sont sur la pile sémantique, avec $D$ qui réfère à $c$. Exécution des lignes 23 à 27, qui se terminent en mettant dans la cellule $d$ une variable $w3=11$. Exécution de la ligne 32, à savoir lecture d'une cellule, et liaison entre $\texttt{R1}\rightarrow r2$ et $one$. Enfin, exécution du Browse, qui affiche le contenu de $r2$, à savoir $one=1$.
	\end{enumerate}
\end{enumerate}
\end{solution}
