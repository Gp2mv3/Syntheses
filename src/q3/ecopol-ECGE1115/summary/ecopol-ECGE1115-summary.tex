...
\hypertitle{Synthèse d'Économie Politique - ECGE1115}{3}{ECGE}{1115}
{Guillaume Prieur}
...
\begin{itemize}
    ...
    \item Le coût d'un bien est ce à quoi on est prêt à reconcer pour l'obtenir. On parle de \it{coût d'opportunité}.
...
\bf{Variables endogènes} : Varibales appliquées par le modèle.\\
...
\bf{Productivité marinale} : définie par une augmentation de production résultant d'une augmentation d'input, elle est représentée par $Pm(x) = \frac{dy}{dx} = F'(x)$.\\
\\
...
\begin{itemize}
    \item \bf{Inputs fixes} : Ils déterminent la \it{capacité de production} ($k$) de l'entreprise. Le \it{coût fixe} $CF$ est la valeur des inputs fixes aux prix en vigeur. Il est indépendant de la quantité produite.
...
\begin{itemize}
    ...
    \begin{align*}
        ...
        & CVM(q) = \frac{CV(q)}{q} \text{ est le coût varible moyen}
    \end{align*}
        \item \bf{Coût marginal} : $Cm(q) = CT'(q) = CV'(q)$
\end{itemize}
On a $Cm$ qui est croissant, qui passet par le minimum de la courbe $CTM$ et coïncide avec la courbe $CVM$ à l'origine (cfr. Figure \ref{cmctm}). Soit $Cm(0) = CVM(0)$ et $Cm(q_0) = CTM(q_0)$.
\begin{figure}[ht!]
    \centering
...
\item \bf{Déterminants exogènes de l'offre} : la technologie, les ressources primaires et leurs prix, le captial physique, les prix des facteurs de production, l'incertitude.
\end{itemize}
...
\noindent
L'élasicité est définie comme le rapport de variations relatives de deux variables liées l'une à l'autre ou le rapport de variation entre $x$ et $y$ pour $y=f(x)$.
...
\end{itemize}
Dans une perspective à long terme, il n'y a pas de coût fixe et les deux seuils coincident puisque les courbes $CV$ et $CT$ se confondent.
...
Le pouvoir du monopoleur est donc d'autant plus important que l'élasiticité-prix de la demande est faible.
\subsubsection{Coût total moyen, coût marginal et profit}
\noindent
...
\subsubsection{Vesion coopérative de l'oligopole de Cournot}
\noindent
Les entreprises déterminent simultanéement les quantités à produire et maximisent le profit.
...
\noindent
L'efficacité des travailleurs à produire des biens est applée la productivité du travail. La productivité est le produit par unité de travail. On définit donc la fonction de production comme
\[y = A * f(L,K,H,N)\]
...
Pour un pays avancé, il faut repouser l'état stationnaire en faisant des progrès technologiques ou des progrès dans le capital humain.
...
\begin{itemize}
    ...
    \item \bf{Revenur futur anticipé} : Plus le revenu anticipé sera élevé, plus on aura tendance à consommer au détriment de l'épargne.
\end{itemize}
...
Si $S_g<0$, on parle de déficit budgétaire. La dette publique aumgente et l'offre de fonds prêtables diminue.\\
...
\noindent
Les flux de capitaux sont définis par le fait que les résidents d'un pays participent aux marchés financiers mondiaux. On a que les sorties nettes de capitaux SNC valent l'acquisition d'actifs étrangers par des belges moins l'aquisition d'actifs belges par des étrangers ou SNC $=($AAEB$-$AABE$)=$ XN. Il faut toujours que XN et SNC soient égaux. Si une entreprise belge vend un bien à l'étranger, alors XN$\uparrow$.
...
On définit également le chômage cyclique ou conjucturel comme l'écart du chômage par rapport au niveau naturel. Cet écart est créé par des fluctuations économiques de court terme.
\section{La monnaie}
