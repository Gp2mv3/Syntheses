...
\begin{enumerate}
  \item Quelles sont les unités
    de $\alpha(x)$ et $c(x)$ et qu'est-ce physiquement ?
  \item On considère un problème en milieu semi-infini ($0 \leq x < \infty$) et avec $\alpha = 0$.
    On a $c(x) = L(x) = c_0 \exp(-x/L)$, avec $c_0$ et $L$ constants.
    ...
    \begin{enumerate}
      ...
      \item Obtenez le réseau des caractéristiques: équation et dessin.
      \item Obtenez l'expression de la solution dans le régime où $u(x,t)$ n'est pas nulle.
    \end{enumerate}
\end{enumerate}
