...
% Remarque : l'équation de densité de traffic n'a pas été vue lors de 
% l'année 2014-2015. Cependant elle semble assez intéressante est permet
% d'avoir un exemple concret d'utilisation d'une EDP (et plus précisement
% d'une équation de transport sous forme conservative), on pourrait peut-
% être la placer en annexe?
...
$\fpart{\phi}{y}$. On suppose que l'on dispose de $\phi(s)$ et
$\fpart{\phi}{n}(s)$ le long de la courbe paramétrisée $\Gamma
...
Soient deux conditions initiales : $\phi(s) = f(s)$ et
$\fpart{\phi}{t}(s) = g(s)$ le long de $\Gamma \equiv (x(s), y(s)$.

La formule d'\textit{Alembert} nous dit alors :
...
% TO DO : zone de dépendance et zone d'influence.
...
On remarque qu'on a ici besoin d'une seule condition initiale sur $\phi(s)$
puisque $\fpart{\phi}{t}$ est donné par l'EDP elle même.
