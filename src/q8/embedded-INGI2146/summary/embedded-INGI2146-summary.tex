...
\begin{itemize}
    ...
    \item \textbf{Visitor Location Register (VLR)}: Local database of a
        MSC (often built into MSC). It contains information from HLR (or
        MS) when subscriber has roamed to VLR's service area (each BS
        served by exactly one VLR). It contains:
        \begin{itemize}
