...
\end{enumerate}
$\Rightarrow$ Advatages of separating IMSI and MSISDN: MSISDN can
...
GSM security consist of:
\begin{description}
	\item[Authentication:] MS is authenticated
	\item[Encryption:] Communication between MS and BTS is encrypted
\end{description}
...
\begin{itemize}
	...
	\item Session key can be changer, at reqular intervals
\end{itemize}
...
\begin{itemize}
	...
	\begin{itemize}
		\item Kc
		\item Current frame number N: different for every burst
	\end{itemize}
	\item Output:
	\begin{itemize}
		\item 114-bit sequence C=A5/x(Kc,N)
	\end{itemize}
	
	\item Encryption:
	\begin{itemize}
		\item Encrypted burst = Original burst data XOR C
	\end{itemize}
\end{itemize}
...
\begin{itemize}
	\item Strength:
	\begin{itemize}
		\item Keys and key generation inside SIM card, not by phone software
	\end{itemize}
	\item Weaknesses:
	\begin{itemize}
		\item A3/A5/A8 algorithm were secret and not publicly discussed
		\item 128 bit key length: Too short for modern computers
		\item Communication between BTS and BSC not encrypted
		\item MS is authenticated to network but not vice versa
		\item Network operator knows all keys
		\item No end-to-end authentication \& encryption
	\end{itemize}
\end{itemize}
