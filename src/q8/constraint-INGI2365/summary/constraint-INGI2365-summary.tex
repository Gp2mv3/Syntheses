...
propagate(){
    ...
    \begin{tabular}{m{7cm}cm{6cm}}
        \begin{itemize}
            \item $O(r^2 \times d^r)$ for \texttt{propagate()}
            \item $O(r \times d)$ calls of \texttt{propagate()} in \texttt{propagateQueueCBDomFilter()}
        \end{itemize}
        & $\to$ &
...
propagate_var(x){
    ...
    \begin{tabular}{m{7cm}cm{6cm}}
        \begin{itemize}
            \item $O(d)$ for \texttt{setup()}
            \item $O(d^2)$ for \texttt{propagate()}
            \item $O(d)$ calls of \texttt{propagate()} in \texttt{propagateQueueCBDomFilter()}
        \end{itemize}
        & $\to$ &
...
\section{Scheduling Satisfacion problem}
            ...
            \item \underline{Unary resources} : can be used by only one activity
                ...
                \begin{enumerate}
                    ...
                    \item Overload checking : checks that it is possible
                        to schedule all activiites ($O(n^2)$ or $O(n log n)$).
                    ...
                    \item Detectable precedences : compote the \texttt{ect} of the group
                        to update the \texttt{est} of the activity ($O(n^2)$ or $O(n log n)$).
            ...
            \item \underline{Cumulative resources} : can be used by activities up to a capacity
                ...
                \begin{enumerate}
                    ...
                    \item Energetic reasoning : represent activity comsumption as water
                        and check if all water fits in a bucket ($O(n^4)$ or $O(n^3 log n)$)
...
\begin{itemize}
    \item Check if v in the domain: O(1)
    \item Remove value c: O(1)
