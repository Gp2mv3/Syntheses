...
\paragraph{Machine virtuelle} L'architecture de la machine est implémentée en software et cette implémentation exécute un programme comme une machine physique. Un exemple d'une machine virtuelle, JVM (java virtual machine). L'avantage d'une machine virtuelle est que les programmes générés sont indépendents de l'architecture réelle de la machine, il suffit qu'il existe une machine virtuel pour cette architecture.
...
=======
Une première manière de manager la mémoire est de manière explicite, le programmeur doit s'en occuper (malloc/free). Cela présente plusieurs désavantages comme : 
\begin{itemize}
\item peut mener à bcp d'erreur (double free, seg fault,...) 
\item programmation modulaire bcp plus complexe (il faut se mettre d'accord sur qui free, qui malloc,...).
\item trouver de corriger les bugs est très compliqué car parfois un bug apparait longtemps après le lancement du programme.
\end{itemize}
>>>>>>> f83bbfbbf57ccba8650ef0329394c74b5e4d552e
...
Cette méthode est rapide, sûre et marche dans la plupart des cas mais si il y a des références circulaire (A ref B et B ref A), ca ne marche pas.

\subsection{Mark \& Sweep}

Mark \& Sweep contiene deux phases :

\begin{enumerate}
\item Mark : On démarre des roots, on parcour la mémoire comme un graph en marquant tout les objets atteignables à partir des roots.

\item Sweep : On itère sur tout les objets du heap, on supprime les objets non marqué (non atteignables) et on remet à zéro les objets marqués.
\end{enumerate}

On peut le faire en Depht-first Search (avec ou sans récursion) ou alors en pointer reverse. Le pointer reverse est utilisé pour ne pas utiliser de la mémoire lors du garbage collection, en effet on lance en général un garbage collector quand on est a court de mémoire alors si il doit lui-même utiliser de la mémoire pour s'exécuter il va avoir un problème. De plus l'espace mémoire requis pour le garbadge collector avec Mark & Sweep n'est pas
borné, il dépend du nombre d'éléments a concerver.
>>>>>>> f83bbfbbf57ccba8650ef0329394c74b5e4d552e
...
=======
On divise la heap en 2 parties : 
\begin{enumerate}
\item from-space (old space) 
\item to-space (new space)
\end{enumerate}

Initialement tout les chunck utilisé sont dans le from-space. Au lieu de marquer les chuncks utilisés, on les copie dans le to-space. Tout en maintenant à l'ancien emplacement un pointeur de référence vers le nouvel emplacement de l'objet.
...
Les avantages sont : 
\begin{itemize}
\item facile d'allouer de la mémoire (à la fin)
\item pas de fragmentation (compactage automatique)
\item le temps est proportionnel au nombre d'objets vivants. Par contre, il faut le double d'espace mémoire.
\end{itemize}

Par contre le même problème qu'avec mark and sweep sans la technique des pointeur inversé se pose. Il faut traverser tous les elements sans utilisé 
de l'espace mémoire pour le faire. Pour cela le to-space (new space) va être divisé en 3 parties :
...
La première étape va être de copier dans le to-space les objets pointés par les roots et de créer les "forwarding pointers"
La deuxième étape va scanner les objets non scanner et copier tous les objets pointer par ceux ci dans les to-space

Si on rencontre un objet déjà copié il suffit de suivre son forwarding pointer pour savoir ou il se trouve.

\subsubsection{Conclusion}

\begin{itemize}
\item copying collection is very efficient
\item l'allocation est très peu couteuse
\item le barbage collection des objets est également peu couteux spécialement si beaucoup d'objet sont à jeter
...
<<<<<<< HEAD
=======
On divise la heap en deux ou plusieur parties, les jeunes et les vieux objets. Quand on alloue de la place pour un objet il est mis dans la partie jeune.

On fait du garbage collection sur dans les objets concidéré comme "jeune" a un rythme soutenu. Si ces objets ont survécu à plusieurs passage du garbage collector, alors on les place dans la partie des vieux objets. Les vieux objets subissent également les garbage collection mais beaucoup moins souvent que les jeunes objets.
...
qu'il accompagne.
=======
Ils n'existent pas en Scala. Il suffit de faire comme si on accédait a une champ en Java. "nomDeLaClasse.Champ = 'Hélène roule en tracteur'".
...
\begin{lstlisting}
implicit def StringWrapper(S: String) = { def method1() = {} }
...
Le keyword implicit va dire au compilateur de lui-même procurer un paramètre à la méthode. Pour cela il faut qu'il y ai un paramètre implicit déclaré et visible par la fonction et qu'il ai le type requis.
