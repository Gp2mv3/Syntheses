...
We then try to see how to solve problems using it.
...
The set of problems for which solving a linear system of equations is very efficient is triangular matrices.
Then we look at the problems that can be reduced to triangular matrices : positive semidefinite matrices with Cholesky factorization.

We can see that the performance of the Newton method is poorly described in the standard theory for self-concordant functions.
First the norm we use depends on the coordinate system while the Newton method does not depend ont he coordinate system.
...
\begin{align*}
  ...
  W(x;r)     & = \{\, y \in \Rn \mid \|y-x\| \leq r \,\} = \cl(W^0(x;r)).
\end{align*}
...
We have seen in \eqref{eq:M_f_scaling} that we can change $M_f$ at will just by scaling the output of $f$.
...
\begin{multicols}{2}
  ...
  \begin{tikzpicture}[x=1.5cm,y=1.5cm,samples=64]
...
This condition cannot be strengthened, $\lambda_f(x) = 1$ is not enough (see \cite[Example~4.1.2]{nesterov2004introductory}).
