...
\begin{equation}
  ...
  \Big(l-\frac{r}{2}M\Big)I \leq G(x) \leq \Big(L-\frac{r}{2}M\Big)I.
\end{equation}
...
we can bound (modulo a constant) the sum $\sum_{i=0}^{k} \frac{1}{i+1}$ by $\int_0^k \frac{1}{x+1} \dif x = \ln(k+1)$ and
the sum $\sum_{i=0}^{k} \frac{1}{\sqrt{i+1}}$ by $\int_0^k \frac{1}{\sqrt{x+1}} \dif x = 2(\sqrt(k+1)-1)$.
Therefore we have
\[ f_k^* - f^* \leq M\frac{R^2+r^2\ln(k+1)}{4r\sqrt{k+1}}. \]
...
However, if we take $h_i = \frac{r}{i+1}$, we can see that $\sum_{i=0}^k h_i$ converges.
...
Again, we can bound (modulo a constant) the sum $\sum_{i=0}^{k} \frac{1}{i+1}$ by $\ln(k+1)$ and
the sum $\sum_{i=0}^{k} \frac{1}{(i+1)^2}$ by $\int_0^k \frac{1}{(x+1)^2} \dif x = 1 - \frac{1}{k+1}$.
Therefore we have
\[ f_k^* - f^* \leq M\frac{R^2+r^2-\frac{r^2}{k+1}}{2r\ln(k+1)}. \]


\subsubsection{Polyak step-size}
When $f^*$ is known, we can do better
First notice by definition of the subgradient, if $y$ is the projection of $x^*$ on the hyperplane, we have
...
It is the first most important fact that form the basis for almost all consequent results.
The second one is the following.
For all $x \in \dom f$ and $y \in W(x;r)$ with $r < M_f/2$,
we have second is the ``new \eqref{eq:lipschitzsecondeig}'':
\[ \frac{M_f}{2}(1-r)^2f''(x) \preceq f''(y) \preceq \frac{1}{1-r^2}f''(x) \]
and the ``new \eqref{eq:lipschitzsecongG}'':
\[ \frac{M_f}{2}(1-r+\frac{r^3}{3} f''(x) \preceq G(x,y) \preceq \frac{M_f}{2}\frac{1}{1-r}f''(x) \]
where
...
We can see that we have $\{\lambda_f(x_k)\}$ forms a relaxation sequence if $[w_*(\lambda_f(x_k))]^2 \leq \lambda_f(x_k)$,
