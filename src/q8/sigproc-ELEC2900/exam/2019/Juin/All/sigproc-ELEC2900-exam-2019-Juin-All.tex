...
\begin{enumerate}
    \item Remind what are the conditions for a system function $H(z)=\frac{\sum_{k=0}^Ma_kz^{-k}}{\sum_{k=0}^Nb_kz^{-k}}$ to be minimum phase.
...
\begin{enumerate}
    ...
    \item Suppose we have a signal $x[n] \in \mathbb{R}^N$ such that  $x[n] = (x[0],x[1],\dots,x[N-1])$, we build the coefficients $(c[0],c[1],\dots,c[N-1])$, where $c[0]$ is the approximation coefficient at level 0, $c[1]$ is the detail coefficient at level 0, and $(c[2j],\dots,c[2j+1])$ are the detail coefficient at level $j \in (0,J-1)$.
