...
The steady-state distribution $\pi$ represents the limiting probability distribution.
...
\begin{mytheo}
  ...
  If the vector $\pi > 0$ is such that
  \begin{equation}
    \label{eq:reversible}
    P_{ij}\pi_i = P_{ji} \pi_j, \quad \forall i,j
  \end{equation}
  and $\pi\1 = 1$
  then $\pi$ is the steady-state distribution
...
The value of $\pi$ found by this theorem is the average of both.
...
What can we do if a chain is not reversible to find the vector $\pi$ ?
...
\begin{itemize}
  \item If $\sum_{n=1}^\infty \sum_{i=0}^{n-1} \rho_i < \infty$ (e.g. $\exists \epsilon > 0$ and $I$ such that $\rho_i \leq 1 - \epsilon$ for all $i > I$)
    then all the states are positive recurrent: $\pi > 0$.
  \item Otherwise, no state is positive recurrent: $\pi = 0$.
\end{itemize}
...
If the embedded Markov Chain is irreducible and positive recurrent with steady-state distribution $\pi$,
...
\begin{mytheo}
  ...
  If the steady states probability $\p > 0$ exists and $\bar{U}_\p < \infty$ then
...
\begin{mytheo}
  ...
  such that $\p\1 = 1$ and $\bar{U}_\p < \infty$, then the process is reversible and the embedded Markov Chain is positive recurrent and reversible.
\end{mytheo}
