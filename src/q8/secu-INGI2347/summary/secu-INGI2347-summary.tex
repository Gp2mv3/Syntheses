...
\begin{itemize}
	\item Confidentialité : seuls les acteurs de la communication peuvent
	savoir ce qui est dit.
	\item Intégrité : assurer au destinataire que le message reçu n'a pas
	été modifié.
	\item Authentification : vérifier l'identité de celui qui a envoyé le
	message.
	\item Non-répudiation: pouvoir convaincre un tier que ce qui a été 
	dit est correct.
\end{itemize}
...
Il est important de retenir que la cryptographie est la base pour 
beaucoup de sécurité mais ce n'est pas:
\begin{itemize}
...
\end{itemize}
La cryptotologie est la science qui combine cryptogrphie et la
cryptanalyse.
\subsection{Cryptographie à clé symmétrique}
Un cipher défini pour
\begin{itemize}
	\item M, l'ensemble des messages
	\item K, l'ensemble des clés
	\item C, l'ensemble des codes
\end{itemize}
Il y a une paire d'algorithme E,D où :$$E:K\ x\ M\to C,\quad D:K\ x\ C\to M$$
tel que $\forall m \in M,k\in K:\quad D(k,E(k,m))=m$\\
E est souvent randomize alors que D est toujours deterministique.
Il y a deux types de cipher, par bloc et par stream.
\subsubsection{Stream Ciphers}
Les stream ciphers consiste à agir sur le plaintext symbole par symbole.
\paragraph{One Time Pad (Vernam)}
$M=C=\{0,1\}^n,\ K=\{0,1\}^n$ tel que k est une clé random aussi longue
que le message. 
$$E(k,m)=k\oplus m,\quad D(k,c)=k\oplus c$$
On a bien que $D(k,E(k,m))=D(k,k\oplus m)=k\oplus(k\oplus)=(k\oplus k)\oplus m=0\oplus m=m$
\paragraph{}
Le problème du OTP c'est que la clé doit être aussi longue que le message. 
Pour remédier à cela, on utilse une Pseudo random key (qui aura la même
taille que le message) qui sera généré à partir de la clé de départ 
qui elle pourra être plus petite que le message. $m\oplus PRG(k) \to C$\\
...
\subsubsection{Sécurité Sémantique}
\begin{figure}[h!]
	\centering
	\includegraphics[scale=0.5]{img/semantic_security_otp.PNG}
\end{figure}
On va dire que E est sémantiquement sécurisé si pour tout attacker
efficace A, $Adv_ss[A,E]$ est négligeable ( négligeable n'est pas
vraiment défini)
\paragraph{Modèle d'assaillant}
\begin{description}
	\item[Ciphertext-only attack:] L'attacker a accès au cipertext
	de un ou plusieurs message encrypté avec la même clé.
	\item[Known-plaintext attack:] L'attacker a accès à des 
	paires ciphertext-plaintext encrypté avec la même clé.
	\item[Chosen-ciphertext attack:] L'attacker a un oracle de décryption
	Il peut choisir des ciphertext (basé sur la même clé) et recevoir le 
	plaintext correspondant.
	\item[Chosen-plaintext attack:]  L'attacker a un oracle d'encryption,
	Il peut choisir un plaintext et recevoir le ciphertext correspondant
	basé sur la même clé.
	(Plus puissant que le CCA)
\end{description}
\subsubsection{Block Ciphers}
Ils agissent sur le plaintext bloc par bloc de symbole. Les plus 
utilisés sont:
\begin{itemize}
	\item 3DES: par block de 64 bits et une clé de 128 bits
	\item AES: par block de 128 bits et une clé de 128,192,256 bits
\end{itemize}
Les block ciphers fonctionnent par itération. C'est à dire que 
les blocks chiffrés sont utilisé comme input pour le chiffrement 
des blocks suivants.
...
\begin{figure}[h!]
	...
	\hline
	Rapide &Lent\\
	Peu Sécurisé&Sécurisé selon l'algo\\
	\hline
\end{tabular}
\end{figure}
...
\end{enumerate}
Alice et Bob partage donc une clé $K_AB$.\\
Ce protocol est sécurisé contre les eavesdroppers mais vulnérable au 
attaque man-in-the-middle. Tout ce que eavesdropper voit c'est 
$E(k_A,''A,B''||k_AB)\quad;\quad E(k_B,'''A,B''||k_AB)$.
\paragraph{Note:} $(E,D) \rightarrow$ eavesdropper learns nothing about $k_AB$
...
\begin{itemize}
	...
	\item Seul la clé privée doit être secrète.
\end{itemize}
Chaque acteur va générer une paire clés (privé/publique). Si l'on veut
envoyer un message crypté à un destinataire, il faut utiliser sa clé publique.
\paragraph{}
un cipher de crypto à clé publique est composé de 3 algos:
\begin{itemize}
...
\begin{figure}[h!]
	\centering
	\includegraphics[scale=0.5]{img/semantic_security_pc.PNG}
\end{figure}
...
\end{enumerate}
Cette façon est sécurisé contre les eavesdroppers mais pas contre le 
man-in-the-middle. En effet le MiTM peut se faire passer pour
Alice quand il communique avec Bob et inversément.
\subsubsection{Trade-offs}
La crypto à clé publique demande plus de calcul que la crypto 
à clé partagé.
...
La génération de la paire de clé se fait avec l'indicatrice d'
Euler(totient function) $\phi$ et le theoreme d'Euler.
\begin{itemize}
	\item $\phi(n)$ compte tout les entiers positifs $\le$n qui 
	sont premier avec n. (GDC avec n = 1)
	\item Le théorème d'Euler nous dit $a^{\phi(n)}\equiv 1 mod n$ pour
...
\end{enumerate}
La clé publique = (e,n) et la clé secrète = (d,n).
\paragraph{Encryption/Decryption}
Avant d'encrypter le message M, , il est changé en entier m tel que
 $0 \le m < n$.
Pour encrypter, on utilise (e,n)
\begin{itemize}
	...
	\end{enumerate}
\end{itemize}
On effectue les puissance avec les racines carré pour aller plus vite.\\
La décryption se fait avec (d,n), $m\equiv\ c^d\ mod\ n$.
$begin{array}{rcr}
  c^d\ mod\ n & \equiv & (m^e\ mod\ n)^d\ mod\ n\\
 			    &\equiv & m^{e\times d}\ mod\ n\\
 			    &\equiv & m^1\ mod n
  \end{array}$
La dernière étape fonction grâce au théorême de Euler.
\paragraph{}
...
Une trapdoor function est une fonction qui facile à calculer dans un sens 
mais difficie de calculer dans le sens opposé sans information 
supplémentaire.
\begin{itemize}
	\item Si f est une trapdoor function alors il existe un 
	secret y tel que avec f(x) et y il est facile de trouver x
\end{itemize}
\subsection{Diffie-Hellman Echange de clé}
Le probléme de la crypto à clé partagé est de savoir comment distribuer
le clé partagé.  Diffie-Hellman permet de régler ce problème sans
forcément utiliser un tier de confiance.\\
Supposons que l'Alice et Bob veulent se mettre d'accord sur 
un secret.
\begin{enumerate}
	\item Alice choisit un nombre premier $g$ (sur $\ge$512bits)
	\item Alice choisit $g<p$ tel $g$ est une racine primitive de p.
	\item \begin{description}
			\item[Racine primitive]: $g$ est une racine primitive de $p$
			si $\forall n \in \{1,...,p-1\} peut être écrit comme $g^k\ mod\ p$
			\item[] Ex: 2 est une racine primitive de 5, $2^k=1,2^1=2,2^2=4,2^3=3\ mod\ 5$
		  \end{description}•}
...
\end{enumerate} 
Alice peut calculer $g^{AB}\ mod\ p$ car elle connait A:$$g^{AB}\ mod\ p=(g^B\ mod\ p)^A\ mod\ p$$
...
Pour pouvoir prévénir les attaque man-in-the-middle, il faut que le 
canal de communication soit authentifier. Cela se fera grâce à 
SSL/TLS
...
\begin{description}
	...
	n-bit. Un attacke doit avoir dur à modifier un message sans
...
L'utilisation du paradoxe de l'anniversaire permet de trouver des
collisions. On hache beaucoup de message aléatoire jusqu'à ce 
qu'on trouve un collision. Pour des message de n bits, 
on a de bonne chance de trouver une collision après $2^{\frac{n}{2}}$ 
essaie.
\subsection{MAC}
Les MAC (Message Authentification code) sont utilisés pour s'assurer 
de l'intégrité d'un message.
...
\end{itemize}
S(k,m) renvoie t dans T et V(k,m,t) renvoie vrai ou faux. Ils doivent
respecté la propriété de consistence$$\forall k \in K, \forall m\in M\quad V(k,m,S(k,m))='yes'$$
Un MAC est considéré comme sécurisé si un attacker ne peut pas produire
 une paire message/tag
valide meme en ayant une paire valide.
\subsubsection{Pseudo Random Function}
Pour une fonction $F:K\  x\  Y\to Z$ définir un MAC $I_F =(S,V)$
\begin{itemize}
	\item S(k,m) := F(k,m)
...
\paragraph{Note:}(la page wikipédia des rainbow table est assez bien faites pour comprendre le
...
%\begin{itemize}
%\item Start1 = A1,1─h B1,1 ─r A1,2 ─ h  B1,2─ r... ─rA1,t-1─ h B1,t-1─r A1,t = End1
%\item Start2 = A2,1─h B2,1─r  A2,2─ h B2,2─r... ─rA2,t-1─ h B2,t-1─r A2,t = End2
%...
%\item Startm = Am,1─h Bm,1─ r Am,2─ h Bm,2─ r ... ─ r Am,t-1─ h Bm,t-1─ rAm,t = Endm
%\end{itemize}
...
\url{http://www.metu.edu.tr/~ccalik/timememory.pdf}
