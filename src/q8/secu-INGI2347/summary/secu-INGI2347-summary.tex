...
{Houtain Nicolas \and Kabasele Nicolas \and Julien Bouilliez\and Julien Bergiers\and Quentin Cappart\and Thomas Haine\and Julien Henricot}
{Gildas Avoine}
...
\begin{tabular}{|c|p{3cm}|p{8cm}|}
    \hline
    par bloc & act on the plaintext one symbole at a time & \begin{itemize}
        \item[+] High speed rate, cheap hardware implementations 
        \item[-] Security analysis is not well established
    \end{itemize}
    \\
    \hline
    par stream & act on the plainttext in blocks of symbols & \begin{itemize}
...
\begin{center}
    E est \textbf{sémantiquement sécurisé} si $\forall$ attacker
efficace A : $Adv_ss[A,E]$ est négligeable 
\end{center}
$\to$ Notons que négligeable n'est pas vraiment défini.
...
pose un problème de ''scalabilité''. Pour faire face à cela on utilise 
un firewall par lequel tout le traffic va passer. On fait un distinction
entre:
\begin{itemize}
...
contrôle d'accès. (user interne accès à tout, externe restreint) Si
jamais il se passe un situation qui n'est pas défini par la politique
...
\end{itemize}
On préfère la deuxième car les failles sont détectés plus vite et 
font moins mal.
Un pare-feu peut être réalisé en une ou plusieurs composantes.
\subsection{Types de pare-feu}
Les pare-feu peuvent être logiciel ou de matériel.
\begin{itemize}
...
Pour faire face au manque d'adresses IPv4, on utilise un NAT.
Il va permettre de séparé les adresses IP en adresses privés et 
en adresses publiques. Les adresses privés peuvent réutilisé tant 
quels sont dans des domaine différents.
\subsubsection{NAT dynamique}
Le NAT dynanmic maintiens une table dont les entrées ont la forme
...
Les entrées de tables sont définis de manière staqtique. En 
général l'on fait un entrée par protocole de la couche d'application. 
...
\begin{figure}[h!]
	...
	Réutilisation de l'espace d'adresse IP& Diminue le througput\\
	\hline
	\end{tabular}
\end{figure}
...
les différents types de connections (inbound/outbound). Pour régler
cela, on ajoute la possibilité de définir une règle sur le bit ACK d'un
segment TCP.
...
\begin{itemize}
	...
	qui sont incorrect.
	\item Il peut remplaces le numéro de séquenc (randomize le
...
Le traffic diriger pour un certain port est automatiquement diriger 
vers le proxy. Le proxy ne modifie pas la requête ou la réponse au dela
de ce qui est nécessaire pour l'authentification et l'identification.\\
L Il est possible de détecter un proxy transparent par l'adresse IP
...
Se trouve dans le domaine du serveur et fait du load balancing des 
requete entre plusieurs serveurs sans que le client le sâche.\\
Le client doit d'abord s'authentifier au près du proxy avant de 
pouvoir communiquer avec l'un des serveurs.

%ON Parle pas de ça dans les slides
%Authentication:
...
%On en a pas parlé dans les slide
%FTP Proxy:
