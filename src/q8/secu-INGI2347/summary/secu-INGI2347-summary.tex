...
(A6) edge node[above] {R} (A7);
    ...
    \node[draw, circle] (B1) [below=of A] {S2};
...
l'endroit où il devait trouver le hash recherché : $C' \neq C$. 
...
requis pour une rainbow table parfaite (sans merge) est de : $m_{max}(t) \simeq \frac{2N}{t+1}$
\item Avec $t$ et pour un problème de taille $N$, le pourcentage de réussite espéré d'une rainbow table parfaite est de :
$P_{max}(t) \simeq 1- (1- \frac{2}{t+1}) )^{t}$ \\
Qui tend vers $1-\e^{-2} \simeq \SI{86}{\percent}$ lorsque $t$ est grand.
\end{itemize}
