...
Type (size and how to interpret) + Value

\subsubsection{Records} 

Collection of data item called field. A format specified how to read
this record.
...
\begin{itemize}
    \item The data file myst be ordered on a key field
...
\begin{tabular}{m{7cm}m{8cm}}
\begin{itemize}
    \item The data file myst be ordered on a non-key field
...
B-Tree solve the problem to insert and delet new entries in
a multi level which must be ordered. (In this data-structure
these operation are quite efficient)

\begin{itemize}
    \item Insert element in a node an split in two node if the 
