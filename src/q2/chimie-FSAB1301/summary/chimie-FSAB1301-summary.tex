...
\renewcommand\sorb{\mathrm{s}}
\renewcommand\porb{\mathrm{p}}
\renewcommand\dorb{\mathrm{d}}
...
Par la longueur d'onde de de Broglie
\[ n \frac{h}{2\pi} = m v r \]
...
Il y a exactement $2l + 1$ orientations possibles pour une orbitale de nombre quantique $l$.
...
La liaison dans une molécule $\ce{HCl}$ est donc une liaison covalente.
...
plus les températures de fusion ($t\degree_{\mathrm{fus}}$) et d'ébullition ($t\degree_{\textrm{éb}}$) sont importantes.
...
l'énergie nécessaire pour exercer $\numprint[\newton]{1}$ sur $\numprint[\meter]{1}$)
a deux composantes :
...
Le symbole $\std$ (dans $H\std$ par exemple) indique que la pression est
...
\begin{enumerate}
	...
	\item Effectuer la réaction à $T_\mathrm{ref}$ : $\Delta H\std(T_\mathrm{ref})$ ;
...
nous suffit de considérer l'univers, qui est un système isolé, dans
lequel est situé notre système non-isolé.

Pour déterminer si un système non-isolé est à l'équilibre ou non,
on défini une nouvelle fonction, appelée l'enthalpie libre de Gibbs :
...
\subsection{Degré d'avancement d'une réaction}
...
où $a = \frac{p_p^{eq}}{p\std}$ et les pressions partielles sont celles des produits
...
A partir de là, on peut réécrire la constante d'équilibre $K$ puisqu'on
...
seule l'enthalpie standard de réaction $\Delta H\std$ intervient dans la
...
le $\ph$ est influencé par l'autoprotolyse de l'eau de façon significative. Il faut alors procéder
...
à titrer en fonction du volume de la solution titrante ajoutée au cours du titrage \cite[p.~484-490]{atkins2011principes}.
...
\subsubsection{Solution, soluté et solubilité}
...
On commence par écrire la réaction (débarrassée des ions spectateurs) :
...
\begin{table}[h!]
  \begin{center}
    \begin{tabular}{ll}
      ...
      $\lambda$ & La longueur d'onde d'un rayonnement électromagnétique\\
...
\begin{table}[h!]
  \begin{center}
    \begin{tabular}{|p{0.1\textwidth}|p{0.35\textwidth}|p{0.35\textwidth}|p{0.2\textwidth}|}
      ...
      $E_I$ & \'Energie d'ionisation d'un atome & Absorbée pour relâcher un $e^-$ & \ref{sec:ioni} p.~\pageref{sec:ioni}\\
      $E_N$ & \'Energie de neutralisation & Absorbée pour lier deux atomes déjà ionisés & \ref{sec:neutral} p.~\pageref{sec:neutral}\\
