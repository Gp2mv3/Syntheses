...
Tout vecteur $\fv{u}$ a des coordonnées unique $u_1, u_2, u_3$ dans $\base{\ui}$.
On a
...
\subsection{Produit vectoriel} \subsubsection{Définition}
Soient $\fv{u} = u_1\ui_1 + u_2\ui_2 + u_3\ui_3$ et $\fv{v} = v_1\ui_1 + v_2\ui_2 + v_3\ui_3$.
\begin{eqnarray*}
...
\begin{itemize}
  \item \num{+1} quand $\alpha, \beta, \gamma$ forment une permutation cyclique de 1,2,3 ;
  \item \num{-1} quand $\alpha, \beta, \gamma$ forment une permutation cyclique de 3,2,1 ;
  \item \num{0} dans les autres cas (quand $\alpha = \beta$).
\end{itemize}
...
Pour tout changement de base de $\base{\ui}$ à $\base{\uj}$, il existe une matrice de rotation $A$ tel que
\[ \va{\uj} = A \va{\ui} \]
...
\[ \fv{u} = \va{\uj}^T \wrt{J}{u} = (A\va{\ui})^T \wrt{J}{u} = \va{\ui}^TA^T \wrt{J}{u} = \va{\ui}^T \wrt{I}{u} \]
Donc aussi
\[ \tilde{\fv{u}} = \va{\ui}^T \wrt{I}{\tilde{u}}\va{\ui} = \va{\ui}^T (A^T \wrt{J}{u})^{\sim}\va{\ui} = \va{\ui}^T A^T \wrt{J}{\tilde{u}} A\va{\ui} = \va{\uj}^T \wrt{J}{\tilde{u}} \va{\uj}\]
...
\begin{eqnarray*}
  \fvd{u} &=& \va{\uj}^T \wrt{J}{\dot{u}} + \va{\uj}^T \omegat\wrt{J}{u}\\
...
\begin{itemize}
  \item Le terme $\fvr{u} = \va{\uj}^T \wrt{J}{\dot{u}}$ est la \textit{vitesse relative} du vecteur
    $\fv{u}$ par rapport à $\va{\uj}$. Ce terme se calcule simplement en dérivant chaque composantes du vecteur $\fv{u}$ ;
  \item Le terme $\omegaf \times \fv{u} = \omegaft \cdot \fv{u}$ apparaît parce que le
    repère $\va{\uj}$ a une orientation variable avec le temps par rapport au repère fixe $\va{\ui}$. On appelle ce terme la \textit{vitesse d'entraînement}
...
Souvent, on aime bien exprimer $\omegaf$ dans la base intermédiaire ($\base{\uj}$ dans l'exemple) car ça fait moins de changement de base successif.
...
\begin{itemize}
  ...
  \item $2 \omegaf \times \fvr{u} = 2 \omegaft \cdot \fvr{u}$ est \textit{l'accélération de Coriolis}. Cette accélération est dûe à la fois à la vitesse relative de $\fv{u}$ par rapport à $\va{\uj}$ et à l'orientation variable dans le temps de $\va{\uj}$ ;
  \item $\omegafr \times \fv{u} = \omegaftr \cdot \fv{u}$ resulte de l'accélération angulaire de $\va{\uj}$ ;
...
de roulement sans glissement. Il faut en effet considérer deux points de contacts entre la corde et la
...
\begin{itemize}
    \item ses composantes (exprimées en \si{\kilo\gram\meter\squared}) sont constantes 
        lorsqu'elles sont exprimées dans une base solidaire au corps $C$ ;
    \item il est symétrique ;
    \item il est semi-défini positif ;
    \item
    il existe une base $\base{\uk}$ telle que $\wtr{K}{I}{G}$ (telle que $\ine^G = \va{\uk}^T \wtr{K}{I}{G}$)
    est \textbf{diagonale}, on l'appelle alors la \textbf{matrice principale d'inertie}.
  ...
  \item lorqu'il est exprimé par rapport à $G$, on l'appelle le \textbf{tenseur central d'inertie} ;
  \item on peut passer d'un tenseur d'inertie exprimé dans une base $\va{\uj}$ à un
    tenseur d'inertie exprimée dans une base $\va{\uk}$. Si $\va{\uk} = A\va{\uj}$ :
...
On appelle alors $\st^Q$ un \textbf{pur moment de force} et on le note $\pst$.
...
\footnote{Le plus souvent, on choisit d'utiliser les contraintes de géométrie de manière implicite et celle de roulement sans glissement de manière explicite.}.
...
Il est ici possible que les contraintes choisies ne sont pas indépendantes.
Il existe un critère assez efficace pour le déterminer
\footnote{Si des contraintes implicites faisaient parties de la dépendance, on peut se convaincre que le raisonnement est correct également.}.
...
Pour plus de concision, à l'avenir, $\am^G$ sera raccourci par $\am$ et $\st^G$ par $\st$.
Pour chaque corps $C_i$, les équations de Newton et d'Euler nous donnent
\begin{eqnarray*}
  m\xgdd^i &=& \fv{F}^i\\
  \amd^i &=& \st^i
\end{eqnarray*}
Ce qui nous donne 6 équations en 3D et 3 équations en 2D (deux pour l'équation de Newton et une pour l'éqution d'Euler).
...
On remarque que donner à $a_i$ les unités de vitesse linéaire et à $b_i$ les unités de vitesse angulaire nous donne des \si{\watt} pour les deux termes.
