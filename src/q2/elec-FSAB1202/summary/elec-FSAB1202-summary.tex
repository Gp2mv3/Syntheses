...
\hypertitle[']{Électricité et magnétisme}{2}{FSAB}{1202}
...
\[ \dif\B = \frac{\mu_0}{4\pi} \frac{I \uvt \times \uvr}{r^2} \dif l \] % utilisée par le formulaire, plus facile à utiliser IMO
...
% Ca reste une synthèse, donc non ;-) après on peut faire un notes avec toutes les démos
...
\[ \B = \frac{\mu_0}{2}\cdot\frac{IR^2}{(R^2 + z^2)^{\frac{3}{2}}} \uvz. \]
...
\[ \oint _\Gamma \H \cdot \uvt \dif l = \int _S \J \cdot \uvn \dif S = I_\mathrm{tot} \]
...
\begin{itemize}
  ...
  \item $\mu_r \gg 1$ (matériau ferromagnétique).
\end{itemize}
Dans un électroaimant avec entrefer, la loi d'Ampère devient :

\[ \oint \H\cdot\uvt \dif l \approx LH_m + eH_e = NI\]
...
\[ eH_e = NI \Rightarrow B_e = \mu_0 \frac{NI}{e} \]
...
On peut réécrire cette équation en terme de proportionnalité :
...
Mais comme le $\mu_r$ de l'entrefer est bien plus petit que le $\mu_r$ du matériau
...
leurs sources peut être énoncée en 4 équations.
...
(voir \figuref{tbc}). % Si qqn a le temps il faudrait changer le "superconductrice" en "supraconductrice" sur l'image

$H_c(T)$ est décroissant.
À une certaine température $T > T_c$, quel que soit le champ magnétique $H$,
  ...
  \begin{center}
    \begin{tikzpicture}[scale=0.7]
      \begin{axis}[xmin=0,xmax=5,ymin=0,ymax=50,
          xlabel=$T(\si{\kelvin})$, ylabel=$B_c(\si{\milli\tesla})$]
        %\xlabel{$T$}
