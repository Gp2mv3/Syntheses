...
%FIXME: La condition "sev" est elle inutile pour les autres myprop que l'existence de P_V(x) ?
	\begin{itemize}
...
\begin{mydef}[Matrice orthogonale]
	Une matrice $Q \in \Rnn$ est orthogonale si ses colonnes forment une base \emph{orthonormée} de $\R^n$ par rapport au produit scalaire canonique, ou encore si
	\[ Q \cdot Q^t = I_n \]
\end{mydef}
...
\begin{myprop}[Condition nécessaire et \textbf{pas} suffisante à l'existence d'une limite]
	...
	\begin{itemize}
		...
		\item Certaines fonctions n'ont pas de limite en un point alors que toutes les restrictions à des droites passant par
			ce point admenttent une même limite.
	\end{itemize}
\end{myprop}
...
\begin{mydef}[Extrémum local libre]
	Un extremum local libre de $f$ est un point extrémum local $a \in \newint A$.
\end{mydef}
...
\subsubsection{Calcul d'extrema locaux non-libres}
Un \textbf{extrema local non-libre} est une extrema local à la frontière du domaine.
C'est
\begin{itemize}
...
Un extremum global est soit un extremum \textbf{local libre}, soit un extremum \textbf{local non-libre} (sur la frontière du domaine).
...
Ca rajoutera probablement des frontières, donc des minima locaux non-libres supplémentaire propres à cette restriction.
