...
\begin{myprop}
	Soient $L \dots V \to V$ un opérateur linéaire,
	$\lambda_1, \dots, \lambda_r$ des valeurs propres de $L$ distinctes
	et $x_1, \dots, x_r$ des vecteur propres relatifs, respectivement à $\lambda_1, \dots, \lambda_r$.
	Les vecteurs $x_1, \dots, x_r$ est une famille orthogonale et donc aussi une famille libre.
\end{myprop}
...
\begin{myprop}[Conditions nécessaires et suffisantes pour qu'une matrice soit diagonalisable]
	...
	Si $\mathbb{K} = \mathbb{C}$, la condition $m_a(\lambda_1) + \cdot + m_a(\lambda_r)$ est tout le temps vraie.
...
\begin{mydef}[Forme quadratique]
	...
	et soit $e = \{e_1, \dots, e_n\}$ une base de $V$.
	Une fonction $q: E \to \R$ est une forme quadratique s'il existe
	une matrice symétrique $A \in \Rnn$ telle que, pour tout $v \in V$,
...
\begin{mydef}[Forme quadratique]
	...
	\subparagraph{Remarque}
	$\bar{q}$ est symétrique : $\bar{q}(x,y) = \bar{q}(y,x)$.
\end{mydef}
...
\begin{mycorr}
	De là, on tire que $A_{q,e}$ est l'\emph{unique} matrice symétrique ($A \in \R^{n \times n}$) telle que
	\[ q(x) = _e\!x^t \cdot A_{q,e} \cdot _e\!x \]
\end{mycorr}
...
\begin{mycorr} Soit $V$ un espace vectoriel réel, $q$ une forme quadratique, $e = (e_1, \dots , e_n)$ et $f = (f_1, \dots, f_n)$ deux bases de $V$, alors
	\[ A_{q,f} = (_eI_f)^t \cdot A_{q,e} \cdot _e\!I_f \]
\end{mycorr}
