...
On remarque que la pressionb apparaît encore dans l'expression de $K$, ce sera 
...
La réaction de dissolution d'un acide dans l'eau est la suivante
\[ \ce{HA + H2O <=> A- + H3O+} \]
...
On observe donc que la $\ph$ d'une solution tampon $\approx \pka$ si $[\ce{A-}]_i \approx [\ce{HA}]_i$.
...
\paragraph{Remarque}
Si un sel formé, il faut tenir compte de son caractère acide ou basique.
...
Une \textbf{solution} est une phase liquide homogène qui consiste un un mélange homogène
...
\begin{itemize}
	\item Soluble si $s \geq \unit{0.1}{\mol\per\liter}$ ;
	\item Insoluble si $s \leq \unit{0.001}{\mol\per\liter}$.
\end{itemize}

\subsubsection{Produit de solubilité}

\[ \ce{M_nN_m(s) <=> mM^{n+} + nN^{m-}} \]
...
Voir exemple~12.9, \cite[p.~500]{atkins2011principes}.

\subsubsection{Réactions de précipitation}
Une réaction de \textbf{précipitation} se produit lorsque deux solutions
d'électrolytes forts se mélangent et régaissent pour donner un solide insoluble.
...
Voir exemple~12.10, \cite[p.~502]{atkins2011principes}.
...
Voir exemple~12.12, \cite[p.~505]{atkins2011principes}.
