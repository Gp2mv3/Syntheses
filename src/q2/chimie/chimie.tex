...
{Pierre Godard, Alain Jonas et Bernard Nysten}
...
Où $R = kN_{av} \approx \unit{8,314}{\joule\per\kelvin\mole}$ est appelée la constante
des gaz parfaits.
...
où $q$ est la quantité de chaleur reçue ou donnée par le système, et $w$ et le travail
effectué ou subit par le système. 
...
% TODO : rewrite this subsubsection
...
où $q[\joule]$ est la chaleur fournie et $\Delta T$ est la variation de température.
...
\subsubsection{Enthalpie molaire standard d'un composé chimique}
...
% Cette partie (voir limitation) est un peu floue. Il faudrait :
% - Soit la supprimer et simplement exliquer que ces valeurs
% se trouvent dans des tables ;
% - Soit la retravailler.
...
$$H\std_{m, \ce{H_20 (g)}} = \beth \std(T) \cdot C_{\ce{H_2O}} + \Delta H\std_{\ce{H_2O(g)}}(T)$$
...
macroscopique présentant un grand nombre de micro-états
...
La variation spontanée d'un système isolé va dans le
...
% TODO : add schetch for entropy (see fig 2.11 syllabus)
...
\begin{itemize}
	...
	\item $a$ est \textit{l'activité} du composé. 
		\begin{itemize}
...
$$\Delta G\std_{trans}(T) = \Delta H\std_{trans}(T) - T\Delta S\std_{trans}(T)$$
...
La dernière égalité vient de la conidition d'équilibre : 
$\frac{dG_{p, T}}{d\xi}(\xi_{eq}) = 0$.
...
\subsubsection{Variation de la constant d'équilibre avec la température}
