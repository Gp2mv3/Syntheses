...
\part{Thermochimie}
\section{Les gaz parfait et le premier principe de la thermodynamique}

\subsection{Les gaz parfaits}
...
\subsubsection{Origine de la pression d'un gaz}
...
$$\SI{1}{\bar} = \unit{10^5}{\pascal}$$
% Affiche une bar au lieu d'afficher "bar"
% En utilisant \unit au lieu de \SI, passe une ligne en plus.

La pression peut aussi être donnée par la formule suivante :
...
\subsubsection{Notion de température absolue}
...
$T$ et exprimée en Kelvin ($K$) égale à la température en celius + 273,15.
...
La température et la pression sont reliée par la loi suivante :

$$p = \frac{C}{3} \frac{N}{V} T = k \frac{N}{V} T$$

Où $k \approx \unit{1,381 \cdot 10^{-23}}{\joule\per\kelvin}$ est appelée constante
de Boltzmann.

En modifiant cette formule pour travailler avec le nombre de moles $n$ plutôt qu'
avec le nombre de molécules $N$, on obtient :

$$pV = nRT$$

Où $R = kN_{av} \approx \unit{8,314}{\joule\per\kelvin\mole}$ est la constante
des gaz parfaits.

\subsubsection{Notion de pression partielle}
Lorsqu'on travaille avec des mélanges de gaz, la pression totale est la somme
des pressions partielles dûes à chaque gaz :

$$p_{tot} = x_1p + x_2p + \cdots + x_np$$

Où $x_i = \frac{n_i}{n_1 + \cdots + n_n}$ est la fraction molaire du gaz $i$
dans le mélange de gaz.

\subsection{Premier principe de la termodynamique}

\subsubsection{Tout corps au repos possède une énergie interne}
Tout corps au repos possède une énergie interne, notée $U$, ayant deux orgines, 
cinétique et potentielle :

\begin{itemize}
	\item Origine cinétique : les composants moléculaires des corps sont agités
				de mouvements aléatoires, d'autant plus importants que la température
				est élevée.
	\item Origine potentielle : les molécules interagissent entre elles par le biais
...
\begin{itemize}
	\item C'est une fonction d'état : c'est à dire que $U$ possède une valeur unique
				pour un système donné à une température $T$ et une pression $p$ données
				indépendamment de l'histoire antérieure du système.
	\item C'est une fonction extensive : c'est à dire qu'elle est proportionnelle
				à la quantité de matière.
\end{itemize}

\subsubsection{L'énergie internet d'un gaz parfait ne dépend que de la température}
Dans le cas d'un gaz parfait, puisque les molécules n'interagissent pas entre elles,
la partie potentielle de l'énergie interne est nulle. Par conséquent, $U$ est 
simplement dûe à l'énergie cinétique d'agitation thermique et donc ne dépend que de
$T$. On a alors :

$$U = N\frac{mv_q^2}{2} = \frac{3}{2}NkT = \frac{3}{2}nRT$$

Cette expression est spécifique aux gaz parfaits mono-atomique.

\subsubsection{Le travail permet un transfert coordonné d'énergie}
Il est important de voir que la variation d'énergie interne dûe à un travail ne peut
se faire que parce qu'un grand nombre d'atomes du corps ont été sollicités de la 
même manière. L'addition de leur réponse est donc coordonnée. C'est une caractéristique
de l'énergie transférée sous forme de travail : il faut un déplacement macroscopique
d'une partie du corps pour qu'un travail soit effectué.

On défini par convention le travail comme étant positif quand il correspond à de 
l'énergie reçue par le système sur lequel on l'exerce. Si par contre le travail est
effectué par le système sur l'environnement extérieur, le système perd de l'énergie
et le signe du travail est négatif.

\subsubsection{Travail effectué par ou contre la pression extérieure}
Examinons le travail effectué par ou sur un gaz dans différentes situations de
dilatation ou de compression d'un piston mobile de section $A$ dans un 
cylindre de volume $V$.

\paragraph{Dilatation contre une pression extérieure}
Dans le cas d'une dilatation contre une pression extérieure, $p_{ext}$ < $p_{int}$.
Le travail exercée par l'environnement sur le système est donné par :

$$w = -p_{ext} \Delta V$$

\paragraph{Compression isotherme et réversible}
Isotherme signifie à température constante. Réversible signifie qu'à a la limite
$p_{ext}$, c'est à dire qu'un changement infinitésimal de la pression extérieure 
ou extérieure suffirait pour faire changer la direction du processus. On a alors :

$$w = \int_{V_1}^{V_2} -\frac{nRT}{V} dV = -nRT \ln{\frac{V_2}{V_1}}$$

\subsubsection{La chaleur est un transfert non-coordonné de l'énergie d'agitation}
On peut modifier l'énergie internet d'un gaz en échangeant de l'énergie cinétique
interne par le biais de collisions atomiques. Ce transfert d'énergie cinétique s'
appele la \textit{chaleur}, notée $q$.

Par convention, on décide que la chaleur reçue par un système est positive.

On définit la \textit{capacité calorifique} $C$ d'un système comme le nombre
de Joules de chaleur qu'il faut fournir à un système pour élever sa température
d'un degré Kelvin, en l'absence de travail fourni ou reçu. Elle se mesure en Joule par
Kelvin :

$$q = C \cdot \Delta T$$

On utilise souvent des capacités calorifiques exprimées par kg de matière ou par nombre
de moles.

\paragraph{A volume constant et en l'absence d'autres travaux}
Soit $C_v$ la capacité calorifique à volume constant :

$$q = \Delta U - w = \Delta U = C_v \Delta T$$

\paragraph{A pression constante et en l'absence d'autres travaux}
Soit $C_p$ la capacité calorifique à pression constante :

$$q = \Delta U - w = \Delta U + p\Delta V = C_p \Delta T$$

\paragraph{Lien entre $C_v$ et $C_p$}
Puisque $\Delta U$ est identique pour les deux cas ci-dessus,
on trouve :

$$C_p - C_v = nR$$

\paragraph{Remarque}
Pour les solides et les liquides qui se dilatent très peu lorsqu'on
les chauffe : $C_p \approx C_v$.

\subsubsection{Enoncé du premier principe de thermodynamique}

\textbf{L'énergie interne d'un corps peut uniquement être modifiée
par échange de chaleur ou de travail.}

$$\Delta U = w + q$$

\subsubsection{L'enthalpie}

L'enthalpie $H$ permet de calculer la chaleur échangée lors d'une
transformation à pression constante. L'enthalpie d'un système
est définie de la façon suivante :

$$H = U + pV$$

Il s'agit d'une fonction d'état extensive, exprimée en Joules.

\section{Evolution spontanée et équilibre d'un système}

% TODO

\section{Application 1 : l'équilibre entre un liquide et sa vapeur}

% TODO

\section{Application 2 : les équilibres chimiques}
...
Premièrement, ils faut distinguer les hydracides tels que \ce{HCl} et \ce{HBr}
