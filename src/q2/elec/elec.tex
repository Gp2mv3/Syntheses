...
Considérons une boucle de surface $S$ parcourue par un courant $I$ et un champ magnétique constant $\vec B$.
...
\[ \oiint \vec B \cdot \dif \vec S = 0 = \Phi \]
...
$I_\mathrm{charge}$, $I_\mathrm{displacement}$ et $I_\mathrm{magnetizing}$.
On définit
\[ I = I_\mathrm{charge} + I_\mathrm{displacement} + I_\mathrm{magnetizing} \]
Le premier est le courant électrique que nous connaissons qui est dû à un déplacement de charge.
Le second vaut
\[ I_\mathrm{displacement} = \varepsilon_0\frac{\dif \Phi_E}{\dif t} \]
...
C'est $I_\mathrm{displacement}$ qui entre les deux plaques n'est pas nul qui établit l'égalité.

Par contre, il est très rare qu'on ne puisse pas négliger $I_\mathrm{magnetizing}$.
On a donc la loi d'Ampère généralisée par Maxwell
\[ \oint \B \dif \vec l = \mu_0 \left(I + \varepsilon_0 \frac{\dif \Phi}{\dif t}\right) \]
...
\[ \EMF = \oint \vec E \dif \vec l = -\frac{d\Phi_B}{dt} \]
\paragraph{Conséquences}
\begin{itemize}
...
Pour le vide, $M = 0$ et $\chi = 0$ donc $\mu_r = 1$ et $B = \mu_0 H$.
...
\[ \Phi = \oiint \vec B \cdot \dif \vec S = \iiint \newdiv \vec B \cdot dV \]
