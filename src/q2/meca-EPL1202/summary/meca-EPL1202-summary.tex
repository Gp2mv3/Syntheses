...
\usepackage{../../../eplunits}
\usepackage{../../../eplmeca}
\DeclareMathOperator{\rank}{\mathrm{rang}}

% \begin{document}

\hypertitle{M\'ecanique}{2}{EPL}{1202}
...
où le symbole de Levi-Civita $\epsilon_{\alpha\beta\gamma}$ vaut :
\begin{itemize}
...
être égale à la vitesse de la surface (et ce dans toutes les directions).
...
L'expression qu'on obtient pour la vitesse ne doit pas spécialement être nulle pour tout $t$, il faut identifier les moments pour lesquels le point sera en contact avec le sol et s'assurer qu'à ces moments-là la vitesse sera nulle selon toute direction.
...
Cette égalité ne doit être vérifiée que lorsque le point de la roue $X$ est en contact avec la planche,
...
C'est-à-dire que quels que soient le point $P$ et la base $\base{\uj}$ solidaires au corps $C$, pour tout point $X$ solidaire au corps $C$, les coordonnées de $\vec{PX}$ dans $\base{\uj}$ sont constantes.
...
Tout corps $C$ a un centre de masse unique $G$ défini comme suit
...
La somme des moments de force agissant sur un corps $C$ par rapport à un point $Q$ est notée $\st^Q$.

Si la somme des forces est nulle, $\st^Q$ ne dépend pas du choix de $Q$.
On appelle alors $\st^Q$ un \textbf{moment de force pur}\footnote{C'est essentiellement un anglicisme ; en anglais, le moment d'une force est nommé \emph{torque} ou \emph{moment}, un système de forces dont la résultante est nulle mais dont la résultante des moments ne l'est pas est appelé \emph{couple}, \emph{force couple}, ou \emph{pure moment}, et la résultante des moments elle-même est appelée \emph{torque} en mécanique, incitant aux confusions.} et on le note $\pst$.
...
Par des contraintes implicites liées à la géométrie du système, on réduit nos $3N$ coordonnées à $n$ coordonnées généralisées. À cela s'ajoutent $m$ contraintes explicites indépendantes. On vérifie bien qu'au final on ait $n-m$ degrés de liberté.
...
virtuels de $\Delta \xgd^i$ et $\Delta \omegaf^i$ est égale au produit de cette force avec
...
\begin{mytheo}
    ...
    sur $\Delta \mathcal{P}$,
    elle dispara\^itra de l'équation des puissances potentielles.
\end{mytheo}
...
on ne peut pas dire qu'ils dispara\^itront quand les variations potentielles de vitesse seront compatibles avec la contrainte relative à la force normale.
...
faire un choix de vitesses potentielles non compatible avec les contraintes afin de les
...
Comme nous respectons les contraintes, toutes les forces de contrainte vont dispara\^itre de l'expression.
...
\begin{pmatrix}Q'_1 & Q'_2 & \cdots & Q'_n\end{pmatrix}\begin{pmatrix}\dqp_1\\\dqp_2\\\ldots\\\dqp_n\end{pmatrix}
= (Q')^T \dqp. \]
...
Q' &= \begin{pmatrix}l_1^T&l_2^T&\cdots&l_m^T\end{pmatrix} \begin{pmatrix}\lambda_1\\\lambda_2\\\vdots\\\lambda_m\end{pmatrix}\\
  Q' &= J^T(q, t) \lambda,
\end{align*}
...
\begin{enumerate}
  \item On prend $\dqp = \{\dqp_1, \dqp_2, \ldots, \dqp_n\}$, on exprime les $\Delta \xgd^i$ et les $\Delta \omegaf^i$ en fonction,
    et on met les $\dqp_\alpha$ en évidence dans la formule des puissances potentielles (sans prendre en compte les forces de contrainte) pour faire appara\^itre les $Q'_\alpha$.
...
\begin{itemize}
  \item Le moment fléchissant $M_{\textnormal{f}}$ est positif dans le sens d'une compression des fibres supérieures et d'un étirement des fibres inférieures (concavité vers le haut) ;
...
\begin{enumerate}
  \item On dessine dans un premier temps les couples et forces de réaction aux différentes liaisons et appuis ;
...
\begin{proof}
  ...
  Comme on suppose que les contraintes sont indépendantes, $\rank (J(q, t)) = m$, et donc les $l_j^T$ sont une base de $V$.
  Prenons la projection $v \in V$ de $Q'$ dans $E$, on sait qu'il existe $w \in V^\perp$ tel que
  \[ Q' = v + w. \]
...
\begin{itemize}
  ...
  \item Pour calculer $\am^G$ et $\lm^G$, comme le corps n'a pas de dimension, $\ine^G = 0$\footnote{
    Attention : bien qu'une masse ponctuelle n'ait pas d'inertie par rapport à son \og centre de masse\fg{}, elle a quand même
  ...
  \item Pour calculer $(\amd^G - \lm^G) \cdot \Delta \omegaf$, on a, pour les mêmes raisons que le point précédent,
