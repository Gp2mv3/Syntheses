...
\begin{itemize}
	...
	\item
		...
		ordre~\footnote{L'ordre d'un axe de symétrie est le nombre de d'angle $\theta \in [0; 2\pi[$ tels qu'une rotation de $\theta$ autour de l'axe envoie une forme sur une forme géométriquement identique. Par exemple, la diagonale d'un cube est un axe de symétrie d'ordre 4 de celui-ci.}
	...
	\item
		...
		\[ \ine^P =  \ine^G - m \tilde{\fv{d}}.\tilde{\fv{d}} \]
...
\[ \mathcal{P} = \fo.\dvec{OP} + \st^P.\omegaf \]

\subsection{\'Energie cinétique}
Soit $T$, l'énergie cinétique d'un corps $C$. Elle peut être calculée par la formule suivante
\[ T = \frac{1}{2} \lm . \xgd + \frac{1}{2} \am^G . \omegaf \]
...
\begin{enumerate}
	...
	\item La résultante des moments de force externes agissant sur un corps $C$ est égale à la dérivée temporelle de son moment angulaire
		\begin{eqnarray*}
			\dot{\am}^G &=& \st^G\\
			&=& \ine^G.\omegafr + \omegaft.\ine^G.\omegaf
		\end{eqnarray*}
...
A l'aide des $m$ contraintes, et de l'astuce algébrique, on peut aboutir à l'équation du mouvement.
Seulement, on aimerait bien un moyen de trouver la bonne combinaison linéaire de nos équations qui nous donne l'équation du mouvement.
...
et
\[
\sum_{i=1}^N (m\xgdd^i - \fv{F}^i) . \fv{a}^i +
\sum_{i=1}^N (\dot{\am}^i - \st^i) . \fv{b}^i = 0
...
\[ \Delta \mathcal{P} \eqdef \sum_{i = 1}^N \left(\fv{F}^i . \Delta \xgd^i + \st^i . \Delta \omegaf^i\right) \]
...
Une pour l'équation du mouvement et une pour se débarrasser de la force normale.
...
\subsection{Calcul de l'équation du mouvement}
...
Ce qui nous donne nos équations du mouvement. % il faut encore rajouter les contraintes ici ou pas ?
...
= (Q')^T \dqp \]
Seulement, on sait que si $\dqp$ respecte les contraintes, alors, le fait d'avoir ignorer les forces de contraintes n'a pas d'influence et la formule des puissances potentielles est correcte et donc $(Q')^T \dqp = 0$. Autrement dit,
...
En se débarrassant des $\lambda_j$, on perd $m$ équations et on se retrouve avec $n - m$ équations du mouvement comme pour la méthode des puissances potentielles.
Mais cette fois-ci, nous les avons obtenues sans utiliser d'astuce algébrique.
...
On a alors les $n - m$ équations du mouvement.
