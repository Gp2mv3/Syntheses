...
des pointillés un bord ouvert, des hachures un ensemble <<~remplis~>>, 
...
\begin{myprop}[Condition nécessaire mais \textbf{pas} suffisante à l'existence 
	...
	\begin{itemize}
		...
		\item si $\lim_{x \to a}f_{B_1}(x)$ et $\lim_{x \to a}f_{B_2}(x)$ 
            ...
            calculer le long de chemin générique comme $y = mx + p$ et 
...
\begin{myrem}[Continuité et dérivées directionnelles]
    ...
    (et donc a fortiori partielles) en un point n'implique pas la continuité.
\end{myrem}
...
Il suffit d'\textbf{énumérer} tous ces points et et \textbf{sélectionner} les valeurs extrêmes
...
Si vous vous débrouillez pour que $f$ soit continue dans cet restriction, par le théorème des \textbf{bornes atteintes},
...
\begin{mydef}[Surface orientée]
	...
	sorsqu'on regarde la surface du côté positif, si on marche le long du parcours, la surface est à notre gauche.
\end{mydef}
...
\begin{mydef}[Différentielle exacte]
	...
	si $\vv{F} = (F_1, F_2, F_3)$ est conservatif : $\vv{F}\cdot\dif \vv{x} = \nabla \Phi$.
\end{mydef}
...
$\vv{r} : t \to \vv{r}(t) = x(t) \xunit + y(t) \yunit + z(t) \zunit$ 
avec $t \in [a;b]$.  Et on a donc à calculer
...
\begin{mydef}[Domaine étoilé]
    ...
    (autrement dit si $P$ peut rejoinre n'importe quel point du domaine par une
