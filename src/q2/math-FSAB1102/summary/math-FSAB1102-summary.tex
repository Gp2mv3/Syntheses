...
{Nicolas Cognaux\and L\'eopold Cambier\and Jean-Sébastien Deneil\and Beno\^it Legat\and Antoine Paris}
...
\begin{myprop}
	...
	\begin{itemize}
		...
		\item $E = V \oplus V^{\perp}$ ; % le symbole \oplus signifie déjà qu'ils sont en somme directe
			\item
...
\begin{myprop}[Conditions nécessaires et suffisantes pour qu'une matrice soit diagonalisable]
	...
	Si $\K = \C$, la condition $m_a(\lambda_1) + \dots + m_a(\lambda_r) = n$ est tout le temps vraie.
...
\begin{mydef}[Dérivée directionnelle] Soit $f : A \subseteq \R^n \to \R^m$, $a$ un point intérieur de $A$, $d \in \R^n$ une direction. La dérivée directionnelle de $f$ en $a$ dans la direction $d$ est notée $D_df(a)$ et vaut, si elle existe, la limite suivante
...
\begin{mytheo}[Théorème des accroissements finis (formulation équivalente\footnote{Elle s'obtient
de la formulation précédente en posant $b = a+h$ et en repèrant la position de $c$ dans $]a;b[$ par $c = a +\theta h$.})]
...
\begin{myprop}[Dépendance linéaire de l'intégrant]
...
\begin{mytheo}[Contribution du bord du domaine] Soit $f$ intégrable sur $D$ borné et fermé, alors
...
\begin{mytheo}[Théorème de Poincaré] Soit $D$ ouvert et \emph{simplement} connexe, $F$ un champ vectoriel défini sur $D$, alors on a une 4\ieme{} équivalence aux trois précédentes
