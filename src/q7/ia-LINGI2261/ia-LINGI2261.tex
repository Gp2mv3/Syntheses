...
>>>>>>> 230b861eabb44ce5fcaf8e6b8ee7c43b1dc6d235
...
\subsection{CSP as search problem}
	...
	When choosing variable in order, it's not efficient. To reduce the search, we will improve the way we 
	choose the variable. This is \textbf{variable ordering}, it exists two kind:
	\begin{itemize}
		\item Minimum Remaining Value: we choose the variable with the fewest legal values.
...
There are to techinque to reduce the size of the domain when considering constraint locally:
	\begin{itemize}
...
\paragraph{Arc consistent} All the value in variable domain satisfy the binary constraint
\paragraph{Path consistent} A two-variable set ${X_i,X_j}$ is path consistent with respect to a third variable
...
We divide the the CSP in independent subproblem (partition of variable and constraint) forming k different CSP. 
