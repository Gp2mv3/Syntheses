...
\texbf{WalkSAT} is a local search. 
\begin{enumerate}
...
Graph construct level by level and 
\begin{itemize}
    \item if are in leveloff then no solution
    \item if the goal all the goal literals are non mutex in
     current level, then a solution might exist within the current graph 
     (Givin with extract-solution)
...
wich variable is action at each level. (\textit{true if in the plan, false else})

\begin{enumerate}
    \item initial state : Last level + set of \textbf{goals}
    ...
    \item Goal state : state with level 0
\end{enumerate}
...
\begin{itemize}
    \item Literals increase monotonically
    \item Actions increase monotonically
    \item Mutexes decrease monotonically
    \item Finite number of actions and literals
\end{itemize}
