...
Complete and  optimal with  complexity exponentiel for  for \textit{time}
...
TODO certaine non linear function
...
For opening and ending game states we can precompute all winning states and put them into a lookup table instead of computing them each titme.
...
\subsection{Defining CSP}
	...
	\end{itemize}
	Element of C consits of a pair <scope,rel>, where \textit{scope} is tuple of all the variables involved and
	\textit{rel} is their relation.
	\paragraph{Assigment} It represents the state of an CSP and where each variable have a value in its 			domain.
	\paragraph{Solution} in a CSP is a consistent assignment (an assignment that does not violate any
	constraint)
	\paragraph{Constraint graph} is a graph where the nodes are the variable and the arc are the constraint
	There is diffefent type of CSP , depending on the domain:
	\begin{itemize}
	...
	\begin{itemize}
		\item unary, constraint on only one variable (ex: x $\neq$ green)
		\item binary, constraint on two variable (ex: x+y $\leq$ 12)
		\item high-order, constraint over several variable (ex: x+5y-3z $\leq$ 8)
	\end{itemize}
	We only consider CSP with binary constraints.
	\paragraph{Hypergraph} An hypergraph is graph with ordinary nodes(circle) which are the variable and the
	constraint, and hypernode(square) which represent the n-ary constraint. 
\subsection{CSP as search problem}
	\begin{itemize}
	...
	\end{itemize}
	\paragraph{Commutativity} order the of application of any given set of action has no effect on the 
	outcome.
	The search tree have a depth of n (number of variable) and the number of state = $O(d^n)$ with d the 
	size of the domain.	With CSP search algorithms should only consider a single variable at each node.
	\paragraph{Backtracking search} It's a depth first search that choose values for one variable at a time and 
	backtracks when a variable has no legal values left to assign.
	When choosing variable in order, it's not efficient. To reduce the search, we can improve the way we 
	choose the variable. This is called \textbf{variable ordering}, it exists two kind:
	\begin{itemize}
		\item Minimum Remaining Value: we choose the variable with the fewest legal values in its domain.
		\item Degree Heuristic: Choose the variable involved in the largest number of constraints (it reduce 
		the branching factor)
	\end{itemize}
        Once a variable has been selected the algorithme need to choose which value to assign. We can also improve that by choosing i
	in which order the values are going to be tried. A solution is the least constraining value, the value that rules out the fewest 
	choice for the neigboring variables in the constraint graph.

	To be more efficient in the search, a good idea is to progate the information, it is done with Forward 
	checking.
	\paragraph{Forward Checking} When X is assigned a value v:
	\begin{itemize}
		\item Look at each unassigned variable T connected to X (with binary constraint).
		\item Remove from T's domain the value inconsistent with the value choosen for X.
		\item With MRV, failure occurs if a domain becomes empty.
	\end{itemize}

\subsection{Constraint Propagation}
The objective is to reduce the search space. To achieve that, we will try to find an equivalent CSP to the 
original one with smaller domains of variables.
There are two technique to reduce the size of the domain when considering constraint locally:
	\begin{itemize}
		\item Arc consistency consider constraint between two variables (remove uncompatible value in 
		domain)
		\item Path consistency consider constraint between n variables.
	\end{itemize}

\paragraph{Node consistent} All the value in a variable domain satisfy the variable unary constraint
\paragraph{Arc consistent} All the value in variable domain satisfy the variable binary constraint.
...
A two-variable set ${X_i,X_j}$ is path consistent with respect to a third variable
$X_m$ if, for every assignment ${X_i=a,X_j=b}$ consistent with the constraint on ${X_i,X_j}$, there is an 
assignement to $X_m$ that satisfies the constraints on ${X_i,X_m}\quad{X_m,X_j}$	
\paragraph{bound consitency} IT is not always possible to represent the domain of each variable as a set. Instead domains are represented with a lower and upper bound and are managed by bound propagation
\paragraph{Global constraint} Constraint involving an arbitrary number of variable and ad hoc specialized
method (ex: Alldiff(X1,X2,..,X8))

\paragraph{Intelligent Backtracking} Instead of backing up to the preceeding variable, we back up to a 
variable that might fix the problem.\\
To achieve that we keep a conflict set that keep the assignement that are in conflict with some value. (With 
backjumping we go to the most recent variable in conflict set)

\subsection{Local Search for CSP}*
The initial state assing a value to every variable and the search changes a value of one variable at a time.
When choosing a new value the most obivious heuristic is to select the value that result in the minimum number of conflicts.
This is called Min-conflict
\subsection{Structure of the problem}
We divide the the CSP in independent subproblem (partition of variable and constraint) forming k different CSP. This can speed up things. 
To achieve that, we consider the strong connected component of the constraint graph as a subproblem
...
>>>>>>> 5cf2733297a1c8f7af731ab8fbe3d1ce9ef5a0eb
TODO
...
\section{Chapitre 10 : Planning}
\subsection{What is planning ?}
Input :
\begin{itemize}
\item Set of world state
\item action operators
\item initial world state
\item goal
\end{itemize}
Output:
\begin{itemize}
\item Ordering of action
\end{itemize}
Difficulties:
\begin{itemize}
\item Size of search tree
\item search method
\item difficult to introduce problem decomposition}
\end{itemize}

\subsection{PDDL Language}
Planning domain definition language allows us to express all the actions with one action schema.

\paragraph{State} The states are representated by a conjontion of fluent, Rich \land Know \land happy. The closed world assumption means that when 
we have no information about a fluent, it is false.

\paragraph{Actions} Actions are described by a set of action schemas defining Action(s) and result(s,a) where s is the state and a the action. The result is specified by what has changed, everything that didn't change is left unmentioned.
...
Action( fly(p, from, to)
	...
	effect : \lnot At(p,from) \land At(p,to)
\end{align}

An action a is applicable in on a state s if the preconditions are satisfied by s. In more details, an action can be executed
on a state S if s entails the preconditions of a. s \models q if and only if every positive litteral q is in s and every negative literal q is not 
in s.

\paragraph{Result}
A result is a state which result in the application of a action on a state.
...
\paragraph{Goal}
Partially specified state, it is like a precondition a conjunction of litteral positive or negative.
A state satisfies a goal is it contains all the atoms of the goal.

\subsection{Forward search vs Backward search}

\paragraph{Forward search}
Forward planning is prone to explore many irrelevant actions especially if uninformed. The second problem is that the state space is usually very large, without a accurate heuristic solving the problem is hopeless.

\paragraph{Backward search}
We start from the goal and apply the actions backward until finding a sequence of steps that reaches the initiale state. In general backward search work only when we know how to regress from a state to its predecessor which is not always easy. However with the PDDL representation it is easy to make regress actions.

Backward search keep the branchin factor lower than forward for most problems but it is harder to find good heuristics.

\subsection{Heuristics for planning}

There are two ways we can relax these kind of problems. By adding more edges to the graph, making easier to find path or by grouping node together.

First, adding edge : 
\begin{itemize}
\item removing precondition from action
\item delete all negative effect from actions result
\end{itemize}

Second, reduce number of state by mapping many state to one, this is done by ignoring some fluent.

A key idea to define heuristics is decompostion, dividing the problem into multiple subproblem and solving each part independently.

\subsection{Planning graph}
TODO : add a figure

Planning graph can be used to give better heuristics. A planning graph is a directed graph organized into levels. The first level $S_0$ consist of nodes representin each fluent that holds in $S_0$ then $A_0$ consist of nodes for each possible action applicable on state $S_0$.

$S_i$ countains all the literals that could hold on time $i$ depending on an action executed at time $i-1$. It is possible to $p$ and $\lnotp$ to both be present in $S_i$. $A_i$ countains all actions that could have their precondition satisfied at time $i$.

We can add mutex link on a planning graph to represent impossibility of certain choices between each other. There are different kind of mutex link.
...
\item competiting need : one of the precondition of one action is mutually exclusive with the procondition of the other action.

\item Mutex literal : one is the negation of the other on the same level.
\end{itemize}
