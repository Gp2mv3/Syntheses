...
\paragraph{Universal Instantiation} For any sentence $\alpha$, variable v ,and ground term g:
	\begin{itemize}
		\item $\frac{\forall v\quad \alpha}{subst({v/g},\alpha)}$ , It means that v can be replace with any 
		instance.
		\item Ex: from $\forall x\quad King(x)\wedge Greedy(x)\Rightarrow Evil(x)$ we can infer $King(
		Richard) \wedge Greedy(Richard) \Rightarrow Evil(Richard)$
	\end{itemize}
\paragraph{Existential instantiation} For any sentence $\alpha$, variable v ,and new constant symbol k:
	\begin{itemize}
		\item $\frac{\exists\quad\alpha}{subst({v/k},\alpha)}$ , It means that v can be replace with a new 
		symbol.
		\item Ex: from $\exists x\quad mother(Richard,x)$ we can infer 
		mother(Richard,ANewMother) , ANewMother is a Skolem constant.
	\end{itemize}
With these two instantiation, we can reduce FOL to proposition symbols. The problem is that with function 
symbols, there are infinetely many ground term (ex:Father(Father(John))).
\subsubsection{Reduction technical issue}
\paragraph{Theorem Herbrand} If a sentence $\alpha$ is entailed by a FOL KB, ti is entailed by a finite subset 
of the propositionalized KB.
\paragraph{Application} For n = 0 to $\infty$ do 
	\begin{itemize}
		\item create a propositional KB by instatntiating with depth-n terms. (Father(a),Father(Father(a)),...)
...
\paragraph{Unification} Substitution that make different logical expression look identical.
	\begin{itemize}
		\item knows(John,x),knows(y,Bill) by unifying we have the substitution $\theta$ = {x/Bill,y/John}
	\end{itemize}
\paragraph{Most General Unifier} The substitution that makes the least commitment about the binding 
variable	
\paragraph{Standardize apart} If two sentences share variables, we can't unified them. To avoid name clashes,
we rename one of the variable. 
\subsection{Forward chaining}
	...
	\end{itemize}
\paragraph{Generalized Modus Ponens} $\frac{p'_1,p'_2,...,p'_n,(p_1\wedge p_2\wedge ...\wedge p_n 
\Rightarrow q)}{q\theta} \quad where \quad p'_i\theta=p_i\theta$ for all i and $\theta$ is a subsitution.
GMP used with KB of definite clause(only one positive litteral) and there is the assumption that all variable
are universally quantified.  It is sound
\subsubsection{Analysis}
\paragraph{Soundness} It only derive sentences that are entailed,because GMP does.
\paragraph{Completeness} It answer every query whose answers are entailed by the KB, but may not 
terminate because of the semidecidability of entailment with definite clauses.
The FC have three sources of ineffiency:
	...
	\end{itemize} 
To avoid repetition, we use incremental forward chaining where some rules generate new information which
may permit unification of existing rules. And rule generate preexisting information that we do not revisit them
when unifying.	 
\paragraph{Rete Algorithm} It preprocess the set of rules in the KB to construct a sort of dataflow network in
each node is a literal from a rule premise. (see pg 341)
\paragraph{Magic Set} To avoid generating irrelevant facts, we rewrite the rule set using information from the
goal, so that only relevant variable bindings (belonging to a magic set) are considered during forward 
inference. 
...
Start with the premises of the goal
	...
	\end{itemize}
The BC for FOL is a function as a depth-first  on a And-Or tree. The tree is a bit different as the arc are OR and the node are a list of goals.
See Prolog from slide 9.38
\subsection{Resolution}
Every sentence of FOL can be converted into an inferentially equivalent CNF sentence. Different technics are 
used:
	\begin{itemize}
		\item Eliminate implications
		\item Move $\neg$ inwards
		\item Standardize variables
		\item Move quantifiers left
		\item Skolemization (see below) 
		\item De Morgan
		\item ...
	\end{itemize}
\paragraph{Skolemization}  Remove existential quantifier by replacing variable with brand new constants.(ex 
$\exists x \quad Q(x)$ becomes Q(A) where A is unique). For more complex sentence, we use function 
symbol (Skolem Functions) to indicate the specific value.
\paragraph{}
...
= {x/b,z/A}$$
To prove that a sentence $\alpha$ is a logical consequence of KB we show that KB $\wedge\neg\alpha$ is 
unsatisfiable. We derive until we get an empty clause (as with propositional logic).
Answer for a goal can be either a Yes/no or a substitution.

\paragraph{Propertes of Resolution} It is sound and complete (see pg 356 for proof) 
...
\paragraph{Complexity}  The complexity of the algorithm is in O(N*b) where N is the number of actions and 
