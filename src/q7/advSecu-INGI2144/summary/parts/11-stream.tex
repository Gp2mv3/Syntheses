    ...
    \item To destroy the linearity:
        \begin{itemize}
            ...
            \begin{center}
            	\begin{tikzpicture}
            		...
            		\node [draw,shape=rectangle,above left = of func] (lfsr1) {LFSR1};
            		\node [right = of func] (keystream) {keystream};
            		\node [draw,shape=rectangle, left = of func] (lfsr2) {LFSR2};
            		\node [below left = of func] (dot) {\ldots};
            		\node [draw,shape=rectangle, below = 0.5cm of dot] (lfsrn) {LFSRn};
            ...
            \begin{center}
                ...
                \node [draw,shape = rectangle, fit = (a) (a) (d) (a)] (func) {f};
                \node [below = 0.5 cm of func] (keystream) {keystream};
		...
		\begin{center}
			\begin{tikzpicture}
				...
				\node [draw,shape=rectangle, above left = of func] (lfsrc) {LFSR clocking};
				\node [draw,shape = rectangle, left = of func] (lfsr) {LFSR};
				\node [below left = of func] (tmp) {output $a_i$};
				\node [below right = of func] (tmp1) {discard $a_i$};
				
				\draw [->] (lfsrc) -| (func) node [near start,above] {$s_i$};
				\draw [->] (lfsr) -- (func) node [near start,above] {$a_i$};
				\draw [->] (func) -- (tmp) node [midway,above] {$f(s_i) = 0$};
