...
\begin{myexem}
  ...
  with the relation $y = w^Tx + \text{noise}$.
  ...
  \begin{itemize}
    \item an overfitting $w$ that has very high values of $w$ that compensates each other but that minimises very well the errors $|w^Tx_i - y_i|$,
    \item a reasonable $w$ with low values of $w$ that minimize quite well the errors $|w^Tx_i - y_i|$
  \end{itemize}
  ...
  Beforehand, they decide the value of $w$ hence Bob will be able to compute $w^Tx_i$ and Alice only has to transmit $y_i-w^Tx_i$ which needs much less bits than transmitting $y_i$.
...
\begin{align*}
  \delta E
  & = (\grad E)^T \delta w + \frac{1}{2} (\delta w)^T H (\delta w) + \bigoh(\|\delta w\|^3)\\
  & = (\grad E)^T \delta w + \frac{1}{2} \left(\sum_{i=1}^n H_{ii}(\delta w_i)^2 + \sum_{i=1}^n\sum_{j=1}^n H_{ij} (\delta w_i) (\delta w_j) \right) + \bigoh(\|\delta w\|^3).
\end{align*}
where $\grad E$ is the gradient of $E$ at $w$ and $H$ is the hessian of $E$ at $w$.

After having trained the network, we know that $w$ is such that $(\grad E)^T \delta w = 0$.
...
They show that if we minimize $\delta E \approx \frac{1}{2} (\delta w)^T H (\delta w)$ subject to $\delta w_k = -w_k$, we find
the minimum
\begin{align*}
