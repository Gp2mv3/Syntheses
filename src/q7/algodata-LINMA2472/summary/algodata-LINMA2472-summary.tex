...
Networks can take several forms and represent all kinds of elements (economic, biological, geographical, social, technological, ...). We will start by looking at social networks. The latter have some properties: 
...
\begin{itemize}
    ...
    $$\mathbb{E}(\sharp\{\text{edges}\}) = \Big( \begin{array}{c} n \\ 2 \end{array}\Big) p \sim n^2p \Rightarrow \begin{array}{c} 
    p \ll p^* \quad \text{then} \quad \mathbb{E}(\sharp\{\text{edges}\}) \ll 1 \\
    p \gg p^* \quad \text{then} \quad \mathbb{E}(\sharp\{\text{edges}\}) \gg 1
    \end{array}
    $$
\end{itemize}

Note that we do not have heterogeneous degrees or density or many triangles : $\mathbb{E}(\sharp\{\bigtriangledown\}) = \Big( \begin{array}{c} n \\ 3 \end{array} \Big) p^3 = \Theta\big((pn)^3\big) = \Theta\big(\bar{d^3}\big)$
...
\subsubsection{Watts-Strogatz (small world)}
...
In this case we try to simulate the emergence of hubs (high degree nodes). ``Richer get richer" (Matthew's effect) 
...
$$\text{Prob(deg = }k\text{)} \propto \frac{1}{k^3}$$
...
\newpage
\subsubsection{Stochastic block}
...
\begin{itemize}
    ...
    \begin{equation*}
        C_u = \frac{2T(u)}{d_u (d_u - 1)} \quad \text{where} \quad T(u) = \sharp\{\text{nodes linked to u} \}
    \end{equation*}
...
The idea behind the algorithm is the following [Complexity O(m) and Space O(n+m)]: 
...
Assume that we are in a simple graph and that every node has a scalar label (eg. age, degree, salary,..). We can pick a random edge and call the extremities X and Y : the assortativity coefficient of the label is Corr$(X,Y)$. (Newman)

\item \textbf{Binary labels}

If instead of labels we have binary categories (gender, smoker, ..) and want to know how many of our friend belongs to a certain one, we replace the label by -1 and 1 and compute, with m the nb of edges and $\mathcal{L}$ the label:
$$
BA_\mathcal{L} = 
\frac{
    \frac{
...
\begin{itemize}
    ...
    \item \textbf{Louvain method}: (Blondel, Guillaume, Lambiotte, Lefèvre, 2008)
...
Compartmental models: population is divided in compartments based on their health status. (Bernoulli in 1760 then $20^{th}$)\\
...
\href{https://github.com/RomainGrx/LINMA2472-Homeworks/blob/main/miscellaneous/animations/cascade.gif}{Animation here.} With a set A0 of activated nodes at the start, at every step if a node u becomes activated it makes a single attempt to try to infect its neighbours v which succeeds with probability p. (independent of past trials). The process stops when no more nodes can be infected. We can perform link percolation: random removal of links in a graph, and study the size of the connected
...
\textbf{1}. Reducing the risk of contagion: adoption of guidelines\\
\textbf{2}. Limiting the possible transmission paths by vaccination or limitation of social contacts. \\
...
There are a lot of parameters such as : \textit{Number of layers}, \textit{Number of neurons}, \textit{Optimizer }(\textit{SGD}, \textit{Adam}, \textit{RMSProp}, ...), \textit{Initial weights}, \textit{Activation functions}, \textit{Learning rate}, ...
...
\begin{itemize}
    ...
    \item $\phi$ : $\inputspace \rightarrow \featurespace$ the function that maps an instance $x_i \in \inputspace$ to $h \in \featurespace$ (Encoder)
    \item $\psi$ : $\featurespace \rightarrow \inputspace$ the function that maps $h \in \featurespace$ to $x' \in \inputspace$ (Decoder)
\end{itemize}
...
$$h = \phi(x)$$
$$x' = (\psi \circ \phi)(x)$$
$$\hat{\phi}, \hat{\psi} = \argmin_{\phi,\psi} ||x - (\phi \circ \psi)(x)||^2$$
...
\begin{figure}[H]
    \centering
    \includegraphics[width=.6\textwidth]{res/cbow.png}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=.6\textwidth]{res/skip.jpeg}
\end{figure}
...
Node2Vec is an algorithmic framework for representational learning on graphs. Given any graph, it can learn continuous feature representations for the nodes, which can then be used for various downstream machine learning tasks (Community detection, layouts, ...).
...
\begin{itemize}
    ...
    \item update $u := v_k$ and go to step 3
\end{itemize}


Given a graph $G=(V, E, W)$, the random walk jump probabilities are given as follows:
...
\begin{itemize}
    ...
    \begin{itemize}
        \item $\dfrac{1}{p}$ : if an edge leads to the previous node;
        \item $\dfrac{1}{q}$ : if an edge leads to an unexplored node;
        \item $1$: else.
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item \textbf{Pseudonymization} = removal of direct identifiers (name, phone number, address, social security number…) and just give each person a unique id that cannot be linked to them.
\end{itemize}
...
$$Entropy = -\sum_{i=1}^{k} \frac{\#C_i}{N} \log\frac{\#C_i}{N}$$

\vspace{1em}

Where: N is the amount of rows in the dataset D;
$C_1$,..., $C_k$ are the equivalence classes; $\#C_i$ indicates the number of rows that belong to $C_i$. The higher the entropy, the more information is contained in D. 
...
Unstructured data is non-tabular, non-categorical data, often text. E.g.Tweets, Emails, Search quries,... . This data can be very sensitive, yet useful and difficult to anonymize. 
...
in the dataset. $P(\text{output = y | Bob} \in D] \approx P(\text{output = y | Bob} \notin D] $.
...
The \textbf{global sensitivity of a function f} captures the magnitude by which a single individual’s data can change the function f in the worst case, and therefore, the uncertainty in the response that we must introduce in order to hide anyone’s participation. Adding noise according to Lap($\Delta f / \epsilon$) prevent averaging attack!\\ 
