...
\begin{enumerate}
 ...
 \item \textbf{Motivation comme trait de personnalité/culturel} :
     ...
     \begin{itemize}
         \item[$to$] On peut donc pas rentrer dans la logique où il
...
\section{L'autorité et l'ordre de l'interaction}
	...
	employé.
	\paragraph{La persuasion} Si ces décisions sont remises en question, il peut tenter de persuader des 
	...
	\subsection{L'autorité comme légitimité}
		...
		\paragraph{Les sources de la légitimité} D'ou vient la légitimité de l'autorité? %Je me suis plus inspire des notes de Quentin pour cette partie.
		\begin{description}
			\item[Légitimité personelle] C'est à dire les qualité, les compétences, le pouvoir,....
			\item[Légitimité d'accesion] L'accès à la position c'est fait des les règles d'usage (un entretien, un 
			vote électorale,...).
			\item[Légitimité de la position] Reconnaissance publique de la position (ex: le beau père qui n'est pas obéi par ses belles filles $\to$ 'T'es pas notre père''
			\item[Légitimité de représention] Incarne un image valorisante du collectif. Raconte un bon récit
			de la communauté.
			\item[Légitimité de rôles et d'action] Rempli les différentes fonctions pratico-sociales et de 
			délivrer des résultats attendu (ex:nouvel entraineur d'un équipe de foot qui perd tout le temps)
			\item[Légitimité de consentement] La légitimité par inférence $\to$ je suis nouveau dans un 
			boite, je sais pas qui est le bosse mais comme les autres obéissent à un gars ça doit être lui.
		\end{description}
		Si l'une de ces légitimités est remise en question, l'autorité s'amenuise.
