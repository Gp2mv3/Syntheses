...
{Nicolas Houtain\and Gorby Nicolas Ndonda Kabasele}{Charles Pecheur}
...
\paragraph{FSP}
Compose concurent processes with \textit{Disjoint Parallel Composition} ($||$).
...
\begin{description}
    ...
    (here, you can explain the two buffers). If an action is hidden, it is replaced by a
    ...
    (\textit{P$<<$A gives A higher priority than P}).

        If A has an high priority, whenever an action A is enabled other
    actiosn are disabled  including \textit{tau} (if the process has to
...
same trace but they are different as  P4 may deadlock after a. With trace
...
states of $E_1$ must simulate all states of $E_2$ )
...
equivalent to  $E_2$ iff there exists a strong bisimulation R such that
$R(E_1,)$
...
execution  paths  (ex: After A  becomes  true, Until  A becomes true,...).
...
infinite traces, reachability is not enough, we use \texit{Strongly connected
component} or \texit{terminal sets}.
