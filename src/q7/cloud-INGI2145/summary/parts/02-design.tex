...
\begin{tabular}{m{4cm}m{10cm}}
    Strong consistency :&
\begin{description}
    \item[Majority quorum] Always write to and read from a majority of nodes. At least one node knows the most recent value. tolerate up to $\lceil$N/2$\rceil$ - 1 crashes. But have to read/write $\lfloor$N/2$\rfloor$ + 1 values.
