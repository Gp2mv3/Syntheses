...
For example, the cone $\{(x,y) \in \R^2 | x \geq 0\}$ is not pointed and we have the whole line $x = 0$ in $K$.
...
\begin{mytheo}[Strong duality for conic optimization]
  ...
  \[ \sprod{x^*}{c} = \sup_{y\text{ admissible}} b^Ty. \]
\end{mytheo}
...
\begin{mydef}[Lorentz cone]
  ...
  \[ \mathbb{L}^n \eqdef \left\{(x_0, x_1, \ldots, x_n) \in \R^{n+1} | x_0 \geq \|(x_1, \ldots, x_n)\|_2 = \sqrt{x_1^2 + \cdots + x_n^2}\right\}. \]
\end{mydef}
