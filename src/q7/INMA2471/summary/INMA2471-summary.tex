...
which will a regular self-dual cone.
...
With the scalar product $\sprod{x}{y} = y^Tx$, the regular cone $\mathbb{L}^n$ is self-dual.
The standard form becomes
\begin{align*}
  \max b^Ty & & \min c^Tx\\
...
\begin{mytheo}
  ...
  \begin{itemize}
    ...
    \item $n(x)$ is admissible (i.e. $x + n(x) \in \dom f$
...
\begin{mydef}[Self-concordant functions]
  ...
  \[ \grad^3 f(x)[h,h,h] \leq 2 (\grad^2 f(x)[h,h])^{3/2} \]
  for all $x \in \dom f$ and for all $h$.
\end{mydef}

\begin{myrem}
  Since $f$ is convex, $\grad^2 f(x)[h,h]$ is positive so we can take its square root.
\end{myrem}
...
\begin{mytheo}
  ...
  $\frac{n(x)}{1 + \delta(x)}$ is admissible and
...
Here are self-concordant barriers for the 3 usual regular self-dual cones and others ($Q \succeq 0, p \geq 1$)
\begin{align*}
...
\begin{mytheo}[Universal barrier]
  Let $K \subseteq \mathbb{R}^n$ be a cone.
...
For the rest of this section, we will assume that both the primal and the dual have a strictly admissible solution.
...
But thanks to the weak duality, since $x^*(\mu)$ and $y^*(\mu)$ are also admissible for the inital problem, we know that
\begin{align*}
...
If we are at a ``distance'' $\delta_\mu(x) \leq \tau < 1$ of $x^*(\mu)$, we have the relation (TODO: do we have the same for $y$ ?)
\[ b^Ty^* - b^Ty^*(0) \leq \mu\nu + (\tau+\sqrt{\nu})\frac{\mu\tau}{1-\tau} \]
but if $\nu - \sqrt{\nu} \geq \tau$ (which is true for $\nu \geq 2$), it can be simplified to
\[ b^Ty^* - b^Ty^*(\mu) \leq \frac{\nu\mu}{1 - \tau} \]
...
\begin{algorithm}
  \caption{Short step barrier iteration for a given precision $\epsilon$ for $b^Ty - b^Ty^*$, an intial admissible solution $y_0$ and initial $\mu_0$}
...
\begin{algorithm}
  \caption{Long step barrier iteration for a given precision $\epsilon$ for $b^Ty - b^Ty^*$, an intial admissible solution $y_0$ and initial $\mu_0$}
  \label{algo:longstep}
  \begin{algorithmic}
    \STATE $\tau = 1/4$
...
For such $f$, we can use the gradient's method
\[ x_{k+1} = x_k - h_k \grad f(x_k). \]
We know that
\[ f(x_{k+1}) - f(x_k) = (\frac{L}{2} h_k^2 - h_k) \|\grad f(x_k)\|_2^2 \]
...
\[ f(x_k) - f(x^*) \geq \frac{3L}{32} \frac{\|x_0 - x^*\|_2^2}{(k+1)^2} \]
...
\[ f(x_k) - f(x^*) \geq 2L \frac{\|x_0 - x^*\|_2^2}{(k+1)^2}. \]
