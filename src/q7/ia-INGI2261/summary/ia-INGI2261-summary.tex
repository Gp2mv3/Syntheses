...
{Yves Deville}
$$$$
This file is based on the book ``Artificial Intelligence, a Modern Approach'' by \textit{Stuart Russel} and \textit{Peter Norvig}

\section{Chapter 1 : Introduction}

\textbf{What is Artificial Intelligence?}
An AI could be a system that \dots
\begin{itemize}
   \item thinks like a human?
    \item acts like a human?
    \item thinks rationally?
    \item acts rationally?
\end{itemize}
Thought is the process which leads to doing an action, while an act is the behaviour, the action itself.

\subsection{Turing test approach}
An \textbf{AI} is ``successful'' if it can make a human think that they're speaking with another human.
...
\begin{description}
    ...
    \item[Percept] : Refers to the agent's perceptual inputs at any given
        instant

    \item[Percept sequence] : Complete history of an agent's percepts
...
\begin{description}
    ...
    \item[Rationality] : ``doing the right thing'' \dots but what \textbf{is} the
        right thing for a computer? Requires a \textit{performance measure}

    \item[Rational agent] : For every possible percept sequence, a
   ...
   \item The rationality of an action depends on:
        \begin{enumerate}
            \item A performance measure that defines the criterion of success
            \item The agent's prior knowledge of the environment
            \item The actions that the agent can perform
            \item The agent's current percept sequence
        \end{enumerate}
    ...
    \item Note : Rationality $\neq$ Omniscience (expected performance $\neq$ actual performance).

    \item[Omniscience agent] : knowing the \textbf{real} result of its action.

    \item[Autonomy] :  An automous agent learns what it can do to compensate for partial or incorrect prior
    knowledge (but shouldn't only rely on its prior knowledge).
