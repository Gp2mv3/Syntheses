...
alétoire est un signal dont on ne peut pas
...
Gibbs, plus d'informations dans~\cite[pp.~169]{haykin2003}.
...
\begin{itemize}
  \item pour les systèmes continus, il faut que les pôles $d_k$ soient
    strictement à gauche de l'axe imaginaire, $\Re(d_k) < 0$. En effet,
		...
		\[ \laplace^{-1}\{\frac{1}{s-d}\} = e^{dt}u(t) = h(t). \]
  ...
  \item pour les systèmes discrets, il faut que les pôles $d_k$ soient
    strictement à l'intérieur du cercle unité, $|d_k| < 1$. \`A nouveau,
		...
		\[ \transfz^{-1}\{\frac{1}{1-dz^{-1}}\} = d^nu[n] = h[n]. \]
...
\begin{figure}[ht]
	...
	l'étape de mesure est idéale. $r$ désigné le signal de sortie ``cible'',
	$e = r-y$ est l'erreur commise par la système, $v$ est un bruit
	apparaissant à la sortie de $G(s)$.}
	\label{fig:feedback}
\end{figure}

On a du coup $Y = GH(R - Y)$ d'où 
$(1+GH)Y = GHX$ et donc
\[ T(s) \eqdef = \frac{Y(s)}{R(s)} = \frac{G(s)H(s)}{1+G(s)H(s)}. \]
...
\[ Y(s) = \underbrace{\frac{GH}{1+GH}}_{T(s)}X + \underbrace{\frac{1}{1+GH}}_{S} V(s) \approx R(s). \]
L'objectif lors de la conception d'un système avec feedback sera
donc de choisir $H(s)$ afin de rendre $T(s)$ proche de $1$ et $S$
