...
\end{tabbing}
\end{bluebox}\\\\
Ces matrices correspondent à une rotation entre une base initiale (ex. inertielle) et finale (ex. attachée au corps) : \[\hat{X} = A Î \]
Si on a une séquence de modification (non-infintésimale) du type 1$\rightarrow$2$\rightarrow$3, on aura $[ \hat{x} ] =A^{(3)}(\theta_3) A^{(2)}(\theta_2)A^{(1)}(\theta_1)[Î]$
...
\>$\frac{d \overrightarrow{OM}}{dt} =\vec{v}_M = \vec{v}_p+\mathring{\vec{p}}_M+\vec{\omega}\times\vec{p}_M$ \> avec \>$\mathring{\vec{p}}_M = [\vec{x}_\alpha]^T\dot{p}_M$\\
