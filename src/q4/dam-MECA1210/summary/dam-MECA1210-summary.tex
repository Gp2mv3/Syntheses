...
\section{Définitions prélimilaires}
\begin{itemize}
\item Mécanique: Etude de la relation entre les mouvements (et \og non-mouvements \fg{}) de systèmes matériels soumis à des forces et des couples
\item Cinématique : Etude du mouvement « pour lui-même » en vue de:
\begin{enumerate}
\item l’analyse de mécanismes (ex: cardan ) $\rightarrow$ calculer (f(t)) les déplacements, vitesses et accélérations de tout point/corps
\item la synthèse de mécanismes $\rightarrow$ trouver le \og meilleur \fg{} mécanisme capable de réaliser une tâche donnée.
\end{enumerate}
$\rightarrow$ Si pas de dépendance temporelle $\Leftrightarrow$ Géométrie (étude des configurations)

\item Statique : Etude des systèmes en équilibre.
\item Pseudo-statique : Etude des systèmes dont la "dynamique " peut être négligée (mouvements lents et/ou masses + inerties faibles) $\rightarrow$ recours aux équations de la statique.
\item Dynamique :
\begin{enumerate}
	\item directe : consiste à déterminer le mouvement du système, connaissant les sollicitations (forces et couples) qui s’y appliquent. La dynamique directe est par exemple utilisée pour prédire le mouvement des véhicules et en analyser le comportement.
	\item inverse : consiste à déterminer les forces et/ou les couples à appliquer à un système dont c’est le mouvement qui est imposé. La dynamique inverse est souvent utilisée en robotique ou en biomécanique pour dimensionner les moteurs d’un robot manipulateur ou estimer les forces musculaires du corps humain en mouvement. L’étude nécessite évidemment de connaître le mouvement (position, vitesse et accélération de tous les corps), mesuré par un jeu de capteurs adéquats (ex. caméra rapide, gyroscope, accéléromètre).
\end{enumerate}

\item Géométrie : l’étude des configurations instantanées des corps (il n’y a plus de dépendance temporelle). C'est-à-dire que l'on ne regarde qu'une position initiale et finale, sans se soucier de ce qu'il y a comme transformation entre les 2 états.
\end{itemize}

\begin{figure}[H]
\centering
\includegraphics[width=.7\linewidth]{defintions.png}
\caption{Liens entre les différentes définitions}
\label{def}
\end{figure}
...
\noindent équations du mouvement de rotation par rapport au centre de masse G (Euler): $L^G = I^G . \dot \omega + \omega \times I^G .\omega$

\noindent équations du mouvement de translation (Newton): $F=m . \ddot R$

Deux types de transformation finie utiles:

\noindent les translations, elles sont commutatives

\noindent les rotations, elles ne sont commutatives que si on utilise le même axe de rotation!
...
\end{tabbing}
\end{bluebox}\\\\
Ces matrices correspondent à une rotation entre une base initiale (ex. inertielle) et finale (ex. attachée au corps) : \[\hat{X} = A \hat{I} \]
Si on a une séquence de modifications (non-infintésimales) du type 1$\rightarrow$2$\rightarrow$3, on aura $[ \hat{x} ] =A^{(3)}(\theta_3) A^{(2)}(\theta_2)A^{(1)}(\theta_1)[\hat{I}]$

\begin{orangebox}
Mouvement général d'un solide = {\color{orange}Séquence} de déplacements* {\color{orange}hélicoïdaux} autour\\
d'{\color{orange}axes hélicoïdaux instantanés} successifs\hspace{2 cm} \scriptsize{*(ou mouvements si infinitésimal)}
...
\>$\fdif{\overrightarrow{OM}}{t} =\vec{v}_M = \vec{v}_p+\mathring{\vec{p}}_M+\vec{\omega}\times\vec{p}_M$ \> avec \>$\mathring{\vec{p}}_M = [\vec{x}_\alpha]^T\dot{p}_M$\\
...
\begin{orangebox}
"le {\color{orange}vecteur vitesse angulaire} d’un solide est aligné avec son {\color{orange}axe hélicoïdal instantané}"
\end{orangebox}\\\\
