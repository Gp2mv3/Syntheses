...
\paragraph{Marché}
Commençons par voir ce qu'est un marché.
Un marché est simplement un groupe de consommateurs et de producteurs
...
\begin{description}
  \item[Coût]
    ...
    Le coût est constitué d'un coût fixe qu'on note $C_F$ et d'un coût
    variable $C_V(q)$.
    Le coût fixe est un coût à payer lorsqu'on qu'on commence à produire
...
Il ne prenent pas l'équation du prix en fonction de la quantité
...
Seulement, le producteur ne produira pas s'il est en perte.
En effet, s'il a un coût fixe non nul,
$\pcompt(q) = pq - C_F - C_V(q)$ peut être négatif.

Le producteur suit donc la démarche suivante.
Il regarde le prix sur le marché,
il calcule $q^*$ tel que $C_m(q^*) = p$.
Si $pq^* - C_F - C_V(q^*) < 0$, il ne produit rien pour n'avoir aucun coût fixe
et a un profit nul.
Sinon, il produit $q^*$ et a un profit de $pq^* - C(q^*)$.
...
Donc par exemple, si une firme travaille dans le software avec
$\pcompt = 100$ mais que si elle travaillait dans le hardware elle aurait
...
on arrive plus un optimum collectif
(mais rien ne dit qu'on est pas à un optimum de Pareto).
Celui qui subit un déséconomie externe va perdre par rapport à l'optimum
