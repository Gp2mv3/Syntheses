...
\hypertitle{Conception orientée objet et gestion de données}{4}{INFO}{1225}
{Beno\^it Legat\and Benjamin de Wergifosse}
{Kim Mens}

\part{Gérer une base de données avec le langage \sql{}}
Une base de données permet de stocker des informations de manière structurée
et facilement accessible.
Les données y sont organisées en tables selon un thème,
une \strong{collection d'entités} (ex. Clients, Réservations,\dots).
...
elle constitue une \strong{clef primaire}.
...
on nomme la colonne de \strong{clef étrangère}.

Une base de données se gère grâce au langage \sql{} qui possède une syntaxe particulière.
Nous allons voir les principales requêtes permettant d'accéder aux données,
de les modifier et d'en ajouter.
D'autre part, un \strong{système de gestion de bases de données} (SGBD)
...
\begin{itemize}
  ...
  \item \lstinline|REAL|, c'est un nombre à virgule flottante sur 8 bytes
    selon la norme IEEE;
  \item \lstinline|TEXT|, c'est un \textit{string}, stocké en utilisant
    l'encodage de la base de données (\texttt{UTF-8}, \texttt{UTF-16BE} ou \texttt{UTF-16LE});
  \item \lstinline|BLOB|, pour \textit{Binary Large OBject}, c'est un blob de données,
...
\end{lstlisting}
ainsi que supprimer une table
\begin{lstlisting}
DROP TABLE jedis;
\end{lstlisting}

\section{Sélection dans une base de données}
...
IS        Comparison to [NOT] NULL, [NOT] TRUE and [NOT] FALSE
...
croissant (mot clef \lstinline|ASC|).

\subsection{Exemples de requêtes}
L'aspect le plus intéressant de \sql{} est celui des références.
On peut par exemple obtenir les \textit{jedis} sans sabre \textit{laser} :
\begin{lstlisting}
SELECT name
FROM jedis
...
\end{lstlisting}
mais aussi le nom des \textit{jedis} ayant un sabre \textit{laser} rouge
\begin{lstlisting}
SELECT J.NAME
...
\end{lstlisting}
on sélectionne les couleurs de sabres \textit{lasers} de \textit{jedis} de moins
de \SI{1.2}{\meter}.
...
\end{lstlisting}
pour lequel on peut aussi définir un caractère d'échappement
...
On peut aussi insérer dans une table
des données obtenues par un \lstinline|SELECT|.
Supposons qu'on veuille mettre dans une table les noms des \textit{jedis} qui ont
...
Ou supprimer tous les \textit{jedis} du côté obscur
\begin{lstlisting}
DELETE j FROM jedis j, blades b
WHERE j.blade_id = b.id AND b.color = 'red';
\end{lstlisting}

Ou supprimer ceux qui n'ont plus de sabre \textit{laser}
\begin{lstlisting}
DELETE FROM jedis
WHERE j.blade_id IS NULL;
\end{lstlisting}

\part{Modélisation de données ORM et \textit{mapping} relationnel}
...
ORM (\textit{Object-Role Modeling}) en est la première.

Dans un premier temps,
il est important de définir \strong{l'univers de discours} (UoD, pour \textit{Universe of Discourse}).
...
un ensemble de couples \lstinline$(TITRE, valeur)$.
...
La base de données est donc un ensemble de tables basées
sur un ensemble de relations $n$-aires.
Le \strong{schéma de la base de données} n'est
autre que l'ensemble des schémas des différentes relations.

\section{L'\textit{Object-Role Modeling}}
...
Selon qu'un fait élémentaire fasse intervenir plus ou moins d'objets
on dira qu'il est \strong{unaire} (1),
\strong{binaire} (2) ou \strong{$n$-aire} ($n$).
Voici quelques exemples de faits élémentaires :
\begin{itemize}
  \item unaire : Destination (numéro) 0475151230 \emph{est gratuite};
  \item binaire : Opérateur (nom) ``Proximus'' \emph{a} Préfixe (numéro) 0478;
  \item binaire : Communication (numéro) 11 \emph{a} Durée (minutes) ``$1:38$'';
  \item binaire : Facture (numéro) 0648979136879 \emph{coûte} Montant (euro) 289;
  \item binaire : Jedi (nom) Yoda \emph{mesure} Taille (mètre) \num{0.66};
  \item ternaire : Étudiant (NOMA) 54511100 \emph{a obtenu} Côte (chiffre) 18
    \emph{pour} Cours (code) ``LSINF1225''.
\end{itemize}
Il est à noter qu'un fait élémentaire ne peut pas être divisé (il est atomique) et n'utilise aucun connecteur ni quantificateur logique (et, ou, non, si, pour chaque, il existe,\dots).
...
(ajout d'une boîte sur la ligne de relation).
\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.7\textwidth]{schema_conceptuel.png}
...
s'assurer de l'exactitude de celui-ci. On parle de \strong{populer} la relation.
...
\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.6\textwidth]{contraintes.png}
...
Elle se dessine comme une flèche en pointillé d'une boîte
à celle obligatoire (un peu comme l'unicité externe).

\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.9\textwidth]{schema_conceptuel_complet.png}
...
Le principe consiste à choisir un objet clef du schéma conceptuel
et de le transcrire sous forme de tableau.
Dans ce tableau doivent se retrouver toutes les contraintes du schéma conceptuel.
Ainsi,
\begin{itemize}
  ...
  \item les colonnes sont mises entre crochets si elles sont optionnelles;
  \item les deux clefs primaires de tableaux différents
    (si toutefois les données sont dans deux tableaux différents)
    sont reliées par une flèche pour une
    contrainte de sous-ensemble.
\end{itemize}

La clef primaire doit exister et être unique, c'est donc
\begin{itemize}
  \item soit une colonne obligatoire et unique;
  \item soit plusieurs colonnes obligatoires avec une unicité externe.
\end{itemize}
...
la flèche venant de la clef étrangère doit pointer vers une clef primaire.
...
\begin{figure}[!htb]
  ...
  \caption{Exemple de schéma relationnel.}
  \label{schema_relationnel}
\end{figure}

La figure~\ref{conceptuel_to_relationnel} illustre un exemple un peu plus
élaboré du passage d'un schéma conceptuel vers un schéma relationnel, par la procédure \strong{Rmap}.

\begin{figure}[!htb]
\centering
\setlength{\lineskip}{\medskipamount}
{\includegraphics[width=0.8\textwidth]{c2r1.png}}\hfill
{\includegraphics[width=0.8\textwidth]{c2r2.png}}
\caption{Exemple du passage d'un schéma conceptuel vers un schéma relationnel.}
\label{conceptuel_to_relationnel}
\end{figure}

\section{Le mapping relationnel (\textit{Relational Mapping})}
...
\begin{enumerate}
  ...
  \item tous les types de faits avec des rôles fonctionnels (unicité simple)
    ...
    avec comme clef l'identifiant de ce type d'objet.
\end{enumerate}
...
la modification de celle-ci a un important coût sur le reste du développement,
...
La phase de \emph{fabrication} reprend différentes itérations telles que :
\begin{itemize}
...
\section{Les récits utilisateurs (\textit{user stories})}
Dans un premier temps,
il faut définir ce qu'on appelle les
\strong{récits utilisateurs} (\textit{user stories}).
Ceux-ci définissent les besoins,
...
Généralement, un récit est une courte description d'un besoin
...
\end{quote}
Il est important de ne pas trop détailler les récits
...
Typiquement, un récit prend 2 jours à implémenter.
Voici deux exemples de récits utilisateurs.
\begin{itemize}
...
Le mot clef \og INVEST \fg{} caractérise un bon récit utilisateur.
\begin{description}
  \item[Indépendant] Il faut rendre chaque récit
    le plus indépendant possible des autres.
  \item[Négociable] Un récit n'est pas fixé une fois pour
    toutes, la conversation avec le client peut amener à le modifier.
  \item[Valuable] Chaque récit a
    sa propre valeur en fonction de son importance.
  \item[Estimable] À chaque récit est associé un
    \og coût \fg{} d'implémentation.
  \item[Small]Un récit ne doit pas
    être trop grand (ni trop petit) sinon il peut être divisé.
  \item[Testable] On peut tester si un récit est réalisé ou pas.
\end{description}
...
\strong{diagramme de classes \textit{Unified Modeling Language}} est assez simple.
...
``visibilité \strong{nom}(nomArg: Type=valeur,\dots) : Type''.
...
\begin{figure}[!htb]
 ...
 %\end{tikzpicture}
  \includegraphics[width=0.9\textwidth]{diagclasses.png}
...
\end{description}
Des \strong{contraintes} sur les attributs sont notées entre accolades.
...
\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.6\textwidth]{diagramme_classes_addon.png}
...
on retrouve diverses informations telles qu'un nom (et le sens de lecture),
...
\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.6\textwidth]{diagramme_classes_association.png}
...
\og dérivent \fg{} d'une classe mère, ou sont un sous-type, une sorte de, etc\ldots\
...
\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.6\textwidth]{diagramme_classes_generalisation.png}
...
Représentées par une flèche en pointillé,
elles sont ornées d'un mot clef qui décrit la relation
...
\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.6\textwidth]{diagramme_classes_relations.png}
...
La différence avec l'association est floue et on ne l'utilisera donc que très rarement.
...
un élément A d'une base de données, l'autre élément de la relation, B,
...
Par exemple,
l'association Etudiant-Enseignant-Salle peut donner lieu à la classe-association Cours.

Lors de la réalisation d'un diagramme de classes UML,
il est important de penser orienté objet,
et de bien penser à toutes les responsabilités et
...
par opposition à sa \emph{structure statique}, le diagramme de classes.
Contrairement au diagramme de classes qui se basait sur les cartes CRC,
le diagramme de séquence se base sur les récits
utilisateurs pour se structurer.
Typiquement, un diagramme de séquence décrit le comportement d'un
seul scénario et ne regroupera donc qu'un ou quelques récits utilisateurs.
...
\begin{figure}[!htb]
  ...
  \caption{Composants d'un diagramme de séquence.}
  \label{diagramme_sequences}
\end{figure}

Chaque participant est représenté par une \og boîte de ligne de vie \fg{}.
Une collection d'un type de participants sera représentée par une double boîte.
...
Une ligne de vie possède une ou plusieurs barres d'activation
...
La manière de procéder est illustrée
par la figure~\ref{diagramme_sequences_objet}.
\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.5\textwidth]{diagramme_sequence_objet.png}
  \caption{Création et destruction d'objets dans un diagramme de séquence.}
...
\begin{figure}[!htb]
  \centering
  \setlength{\lineskip}{\medskipamount}
  \subcaptionbox{La structure conditionnelle \texttt{loop}.}{\includegraphics[width=0.48\textwidth]{diagramme_sequence_cadres.png}}\hfill
  \subcaptionbox{Les structures conditionnelles \texttt{loop}, \texttt{alt} et \texttt{opt}.}{\includegraphics[width=0.48\textwidth]{diagramme_sequence_cadres2.png}}
  \caption{Exemples de cadres itératifs dans un diagramme de séquence.}
  \label{diagramme_sequences_cadres}
\end{figure}
Les trois opérateurs usuels sont les suivants :
\begin{description}
  \item[alt] représente la condition \texttt{if-else} où le \texttt{else} est
    une garde séparée du \texttt{if} par des pointillés;
  \item[loop] représente la boucle \texttt{for};
  \item[opt] représente la condition \texttt{if}.
\end{description}
