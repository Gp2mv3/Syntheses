...
\begin{mydef}
  Un ensemble $X$ est convexe si et seulement si, $\forall x, y$
  \begin{align*}
    x, y \in X \Rightarrow x + (1-\lambda) y \in X & 0 < \lambda < 1.
  \end{align*}
\end{mydef}

\begin{myprop}
  Si
  \[ X = \{x \in \Rn | c_i(x) = 0 \forall i \in \eqset
  \land c_i(x) \geq 0 \forall i \in \inset\}, \]
  on a une condition \emph{suffisante} permettant de savoir si $X$ est
  convexe.
  $X$ est convexe si
  \begin{itemize}
...
Il existe des cas pathologiques pour lesquels sa complexité est exponentielle
...
Comme on fait une borne inférieure, il faut que les inégalité soient $\leq$.
Donc
\begin{itemize}
...
\begin{mytheo}[Dualité d'un problème sous forme général]
  \begin{align*}
    \min c^T x & & \max b^Ty\\
...
Il se peut même que $f$ soit borné et qu'il n'y pas de minimum global
comme pour $f = \arctan$.
...
\end{itemize}
Ces deux items sont donc des conditions suffisantes pour être un minimum
...
Si $f$ est \emph{bornée}, de classe $\dset^1$,
les $p_k$ sont des directions de \emph{descentes} vérifiant
les conditions de \emph{Wolfe} et
que $\grad f$ est \emph{Lipschitsien} et continu.
...
La méthode du gradient consiste à prendre $p_k = -\grad f(x_k)$
...
\begin{mytheo}
  ...
  \begin{proof}
    ...
    \begin{enumerate}
      \item $g(0) > 0$ ($g(0) = \gradn f(x)^T\gradn f(x) = ||\gradn f(x)||^2 > 0$).
...
\begin{mytheo}
  ...
  \begin{description}
    ...
    \item[$w_1$] $f(x_k + \alpha_kp_k) \leq f(x_k) + c_1\alpha_k \grad f(x_k)^Tp_k$.
    \item[$w_2$] $\grad f(x_k + \alpha_k p_k)^T p_k \geq c_2 (\grad f(x_k)^T p_k)$.
    \item[Lip] $\|\grad f(y) - \grad f(x)\| \leq L\|x - y\|$.
  \end{description}
  \begin{proof}
    Bornon la longueur $\alpha_k$ (par le bas)
    \begin{align*}
      \grad f(x_k + \alpha_k p_k)^T p_k - \grad f(x_k)^T p_k & \geq (c_2 - 1) \grad f(x_k)^T p_k\\
      (\grad f(x_k + \alpha_kp_k) - \grad f(x_k))^T p_k & \geq (c_2 - 1) \grad f(x_k)^Tp_k > 0
    \end{align*}
    Combinons avec (Lip).
    \[ \| \grad f(x_k + \alpha_kp_k) - \grad f(x_k) \| \leq L \alpha_k \|p_k\| \]
    Par Cauchy-Schwartz
    \[ (\grad f(x_k + \alpha_kp_k) - \grad f(x_k))^T p_k \leq \|\grad f(x_k + \alpha_kp_k - \grad f(x_k)\| \|p_k\| \]
    d'où
    \[ \alpha_k \geq \frac{(c_2-1)\grad f(x_k)^Tp_k}{L\|p_k\|^2} \]
    Par ($w_1$), commme $c_1 \grad f (x_k)^T p_k$, on a
    \[ f(x_{k+1}) - f(x_k) \leq c_1 \frac{(c_2-1)}{L} \left(\frac{\grad f(x_k)^T p_k}{\|p_k\|}\right)^2 \]
    En sommant de 0 à $k-1$, on obtient
    \[ f(x_k) - f(x_0) \leq \frac{c_1(c_2-1)}{L} \sum \left(\frac{\grad f(x_k)^T p_k}{\|p_k\|}\right)^2 \]
    Mais $f(\xopt) - f(x_0)$ est fini et $f(\xopt)-f(x_0) \leq f(x_k) - f(x_0)$.
    On conclut que
    \[ \sum_{k=0}^\infty \frac{(\grad f(x_k)^T p_k)^2}{\|p_k\|^2} \leq \frac{f(x_0) - f(\xopt)}{c_1(1-c_2)/L} \]
    Puisque
...
\begin{mytheo}
  ...
  ($\alpha_k = -\alpha_k B_k^{-1} \grad f(x_k)$ $\forall k =\grad f(x_k?)-\grad f(x_k)$).
