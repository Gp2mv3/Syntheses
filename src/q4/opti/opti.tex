...
\begin{mydef}[Convexité d'une fonction]
...
\subsection{Calcule d'un premier sommet}
...
\begin{myprop}[Condition de complémentarité]
...
%\begin{mytheo}
%  Si $x^*$ est un min local de $\min_{x \in \Rn}f(x)$ alors
%  $\gradn f(x^*) = 0$.
%  \begin{proof}
%    Soit un point $x$ tel que $\gradn f(x) \neq 0$.
%    On va montrer que si $x$ n'et pas un min local.
...
%
%\begin{myexem}
%  $a^T x \geq b$ ou $c^Tx \geq d$
%  $\Leftrightarrow$
%  $a^Tx y \geq yb$ et
%  $c^Tx (1-y) \geq (1-y) d$ et $y \in \{0, 1\}$
%\end{myexem}
%
...
%    Mais $f(x^*) - f(x_0)$ est fini et $f(x^*)-f(x_0) \leq f(x_k) - f(x_0)$.
%    On conclut que
%    \[ \sum_{k=0}^\infty \frac{(\grad f(x_k)^T p_k)^2}{\|p_k\|^2} \leq \frac{f(x_0) - f(x^*)}{c_1(1-c_2)/L} \]
%    Puisque
