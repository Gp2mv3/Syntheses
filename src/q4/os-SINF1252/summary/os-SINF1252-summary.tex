...
\begin{itemize}
  ...
  \item Soit il s'agit d'un module du programme (dans le même répertoire ou dans un répertoire définit en utilisant la variable d'environnement \texttt{GCC\_INCLUDE\_DIR} ou \lstinline|CPATH|) : \\
    \texttt{\#include "lemodule.h"}.
\end{itemize}
...
Cette taille doit être de type \texttt{size\_t}, il est important de toujours utiliser \lstinline|sizeof| lors du calcul de la taille d'une zone mémoire à allouer.
\lstinline|malloc| retourne un pointeur du type \texttt{(void *)} qui peut être casté en un pointeur du bon type.
...
\begin{itemize}
  \item Les arguments qui ont été passés via la ligne de commande.
    L'OS met dans \lstinline|argc| le nombre d'arguments et place dans \texttt{char *argv[]} tous les arguments passés avec dans \texttt{argv[0]} le nom du programme qui est exécuté.
...
La plupart d'entre elles sont initialisées par le système ou via les fichiers qui sont chargés automatiquement au démarrage de l'interpréteur comme le fichier \texttt{/etc/profile} et le fichier \lstinline|.profile| du répertoire utilisateur.}.
...
\noindent\begin{tabularx}{\textwidth}{|p{0.08\textwidth}|p{0.45\textwidth}|p{0.05\textwidth}p{0.1\textwidth}|p{0.185\textwidth}|}
  ...
  & Admin & \lstinline|hostname| & \texttt{damien-laptop}\\
  \hline
  ...
  & User & \lstinline|passwd| & \texttt{/bin/bash} \\
  \hline
  ...
  & & & \texttt{/home/damien} \\
  \hline
  \texttt{PRINTER} & Nom de l'imprimante par défaut qui est utilisée par \lstinline|lp|. & & & \\
  \hline
  ...
  & User\footnote{par exemple en ajoutant \texttt{PATH=\$PATH : \$HOME/local/bin : .} dans le fichier \lstinline|.profile|.
Dans cet exemple le répertoire \texttt{/local/bin} sera ajouté ainsi que le répertoire courant symbolisé par le ``\lstinline|.|''.} & & \texttt{/usr/local/sbin: /usr/local/bin: /usr/sbin: /usr/bin: /sbin: /bin: /usr/games}\\
  \hline
\end{tabularx}
...
Cette erreur est signalée lors de la compilation par \lstinline[language=bash]|gcc| sous la forme d'un \textit{warning} : \texttt{warning : function return address of local variable}.


\section{Organisation des ordinateurs}
\begin{wrapfigure}{r}{0.3\textwidth}
  \vspace{-0.5cm}
  \includegraphics[width=0.3\textwidth]{img/vanNeumann}
...
\begin{wraptable}{r}{0.4\textwidth}
  \begin{tabular}{|c|c|c|}
    ...
    \hline
    \texttt{long int} & 4 & \lstinline[language={[x86masm]Assembler}]|l| \\
    \hline
    \texttt{void *} & 4 & \lstinline[language={[x86masm]Assembler}]|l| \\
    \hline
  \end{tabular}
...
Par exemple : \texttt{leal 4(\%esp) \%edx} placera dans le registre \lstinline[language={[x86masm]Assembler}, mathescape]|$\texttt{\%}$edx| l'adresse de son argument source, c'est-à-dire l'adresse contenue dans \texttt{\%esp+4}.
...
Les premiers langages de programmation et les langages tels que le BASIC ou le FORTRAN disposent d'une construction similaire avec l'instruction \lstinline[language={[x86masm]Assembler}]|goto|.
Cependant, l'utilisation de l'instruction \lstinline[language={[x86masm]Assembler}]|goto| dans des programmes de haut niveau rend souvent le code difficile à lire et de nombreux langages de programmation n'ont plus de \lstinline[language={[x86masm]Assembler}]|goto|\footnote{Le \clang{} permet encore l'utilisation de \lstinline[language={[x86masm]Assembler}]|goto| (contrairement à \java{}) mais son utilisation est fortement déconseillée !}.
Lors de l'exécution d'un \lstinline[language={[x86masm]Assembler}]|goto|, le programme saute directement à l'exécution de l'instruction qui suit le label indiqué.
...
\begin{itemize}
  \item utiliser le qualificatif \texttt{\_\_thread} devant une déclaration de variable (supporté par \lstinline[language=bash]|gcc| avec la librairie threads POSIX);
...
\begin{wrapfigure}{r}{0.4\textwidth}
  \vspace{-0.5cm}
  \includegraphics[width=0.5\textwidth]{img/memthread}
...
Le \textit{\textbf{contexte}} d'un thread est composé de l'ensemble des contenus des registres qui sont nécessaires à son exécution (y compris le contenu des registres spéciaux tels que \lstinline[language={[x86masm]Assembler}, mathescape]|$\texttt{\%}$esp|, \lstinline[language={[x86masm]Assembler}, mathescape]|$\texttt{\%}$eip| et \texttt{\%eflags}).
...
\begin{wrapfigure}{r}{0.45\textwidth}
  \vspace{-0.5cm}
  \includegraphics[width=0.55\textwidth]{img/appelsbloquant}
...
En \clang{}, un mutex est représenté par une structure de données \texttt{pthread\_mutex\_t} qui est définie dans \lstinline|pthread.h| et qu'il faut initialiser avec \texttt{pthread\_mutex\_init} et détruire avec \texttt{pthread\_mutex\_destroy} afin de libérer les ressources qui y sont associées.
...
\begin{itemize}
  ...
  \item une fonction \lstinline|post| qui permet d'incrémenter la valeur du sémaphore ou de débloquer un thread en attente (s'il y en a un) : (\texttt{int sem\_post(sem\_t *sem);});
  \item une fonction \lstinline|wait| qui permet de tester la valeur du sémaphore, si elle est $>0$, décrémente le sémaphore et continue, sinon bloque le thread et attend qu'un autre thread fasse un \lstinline|post| : (\texttt{int sem\_wait(sem\_t *sem);}).
\end{itemize}

Dans le cadre de ce cours nous nous limitons au sémaphores POSIX.
Pour pouvoir les utiliser il faut importer \lstinline|semaphore.h| qui contient les différentes signatures (dont celles reprises dans la liste ci-dessus) et la structure de données de type \texttt{sem\_t}.
...
\lstinline[language=bash]|gcc --static -g -o executable libname.a exemple.o|. L'argument \texttt{--static} force le compilateur à inclure le code de la librairie dans l'exécutable. \\
...
lors de la terminaison d'un processus en utilisant \texttt{\textbf{int} atexit(\textbf{void} (*function)(\textbf{void}))}.
 ...
 père doit utiliser l'appel système \lstinline|waitpid| via \texttt{\textbf{pid\_t} waitpid(\textbf{pid\_t} pid, \textbf{int} *status, \textbf{int} options)}.
...
\begin{enumerate}
  ...
  \item vérifie le type de fichier : par convention, le début d'un fichier contient une séquence d'octets ou de caractères qui indiquent le type de fichier dont il s'agit\footnote{Ce type de fichier peut être testé avec la commande \lstinline|file|.} :
    \begin{enumerate}[label=\alph*)]
      ...
      \item soit le fichier est en langage interprété et débute alors par \lstinline[language=bash]|\#!| suivi du nom complet de l'interpréteur à utiliser et de ses paramètres éventuels à utiliser.
    \end{enumerate}
\end{enumerate}
...
\begin{tabularx}{\textwidth}{l|p{0.615\textwidth}|c}
  ...
  \hline \hline
  \texttt{SIGALRM} & Une alarme définie par la fonction \lstinline|alarm| ou \lstinline|setitimer| a expiré. & Terminaison. \\
  \hline
  ...
  \hline
  \texttt{SIGTERM} & Utilisé par défaut par \lstinline|kill| pour demander la fin du processus. & Terminaison. \\
  \hline
...
Les bits de permission sont gérés par l'OS mais il peut parfois être utile de les modifier pour certaines pages. Cela se fait avec \texttt{\textbf{int} mprotect(\textbf{const void} *addr, \textbf{size\_t} len, \textbf{int} prot)} où \lstinline|prot| peut prendre les valeurs \texttt{PROT\_NONE}, \texttt{PROT\_READ}, \texttt{PROT\_WRITE}, \texttt{PROT\_EXEC} ou une disjonction logique de celles-ci. Si la protection demandée est plus \textit{faible} que l'actuelle, l'OS produira une \texttt{Segmentation Fault}.
...
\lstinline|mmap| prend entre autres un argument qui spécifie si les pages du fichier sont mappées dans chaque processus (avec \texttt{MAP\_PRIVATE}) ou si plusieurs processus peuvent accéder et modifier la page (avec \texttt{MAP\_SHARED}).
...
On peut utiliser \lstinline|msync| qui permet de demander si cette écriture doit être faite (avec le drapeau \texttt{MS\_SINC} il attend la fin, tandis qu'avec \texttt{MS\_ASYNC} il ne fait que démarrer l'écriture). \\
