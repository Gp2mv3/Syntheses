...
Attention : Il ne faut pas compter sur les \texttt{free} implicites.
...
\paragraph{1e solution : Répartition d'adresses entre \textit{SRAM} et \textit{DRAM}}
...
La 2e famille d'instructions importante sur un processeur contient les instructions qui permettent d'effectuer les opérations arithmétiques et logiques.
...
En \clang{}, un mutex est représenté par une structure de donnée \texttt{pthread\_mutex\_t} qui est définie dans \texttt{pthread.h} et qu'il faut initialiser avec \texttt{pthread\_mutex\_init} et détruire avec \texttt{pthread\_mutex\_destroy} afin de libérer les ressources qui y sont associées.
...
En \clang{}, il existe des variantes à \texttt{execve} : \texttt{execl}, \texttt{execp}, \texttt{execle} et \texttt{execv}. Celle-ci diffèrent de \texttt{execve} par leur manière de spécifier les arguments. \\
