...
\question{1}
Consultez la page de manuel de chmod(1) ou d’autres sources pour déterminer
...
Avec chmod, il y a aussi le "bit" X qui permet de ne viser que ce bit de permission sur les répertoires.
Par exemple, \lstinline|chmod -R u-x, u+X|
...
\item t : est-ce que les fichiers et dossiers contenus à l'intérieur peuvent être supprimés ou renommés par les autres utilisateurs que ceux du groupe ou du propriétaire (ex : /tmp/).
...
Si oui, utilisez chmod(1) pour supprimer cette autorisation de lecture par défaut dans votre répertoire.
...
\question{4}
Le répertoire /tmp est particulier car il est destiné à pouvoir contenir des fichiers de n’importe quel utilisateur.
...
\question{5}
La commande lsof(8) permet de lister les fichiers ouverts par un processus.
A votre avis, comment cette commande fonctionne-t-elle ?

\begin{solution}
Elle regarde dans le dossier spécial /proc/[pid]/fd pour chaque processus, la liste des file descriptors et donc des fichiers ouverts.
\end{solution}

\question{6}
Lorsqu’un exécutable est lancé avec execve(2), il hérite des descripteurs de fichiers qui étaient ouverts dans le processus qui a exécuté execve(2).
Comment feriez-vous en utilisant uniquement l’appel système open(2) pour déterminer combien de descripteurs de fichiers étaient ouverts au démarrage de votre programme ?

\begin{solution}
Comme les descripteurs de fichier sont attribués séquentiellement, on peut appeler open(), récupérer sa valeur de retour et soustraire 1.
...
open(2) renverra quand même l'ancien descripteur de ce fichier, alors qu'il reste d'autres fichiers ouverts.
Parfois, les réponses ne sont pas correcte.)
\end{solution}

\question{7}
Est-il possible dans un processus d’exécuter \lstinline|int fd1 = open("f",O\_RDWR)| ;
et juste après \lstinline|int fd2 = open("f",O\_RDONLY);| ?
Si oui, quelles seront les valeurs des descripteurs de fichiers retournés lors des appels système.
Si non, quelles seront les valeurs de errno?

\begin{solution}
C'est parfaitement possible.
En pratique, le premier file descriptor référera à un open file object sur lequel on peut lire et écrire,
et le deuxième file descriptor référera à un autre open file object, avec ses propres propriétés et son propre file offset, sur lequel on ne pourra que lire.
Les deux descripteurs de fichier seront évidemment différents.
\end{solution}

\question{8}
Dans un shell, on exécute la commande \lstinline|/bin/echo "1234" > ./test.txt|.
Quels sont les appels systèmes effectués par le shell avant l’exécution de /bin/echo ?

\begin{solution}
Déjà, le shell se fork.

Dans le fils, on ferme le file descriptor correspondant à stdout, on ouvre le fichier 'test.txt'
(logiquement, le file descriptor renvoyé sera 1, correspondant à la sortie standard).

(Alternative : ouvrir le fichier 'test.txt' dans un file descriptor fdtest,
puis dupliquer avec dup2 : \lstinline|dup2(fdtest, STDOUT_FILENO)|
(STDOUT\_FILENO est d'abord fermé, puis devient une copie de fdtest)
(dup2 renvoie STDOUT\_FILENO comme nouveau file descriptor, ou -1 s'il y a une erreur).)

Enfin, appel à execve : le processus va alors croire qu'il écrit sur le terminal, dans stdout, alors qu'il écrit dans un fichier.
\end{solution}

\question{9}
Dans un shell, on exécute la commande \lstinline|./a.out < in.txt > out.txt|.
...
\question{10}
Dans un shell, on exécute la commande \lstinline$cat < in.txt | grep sinf1252$.
...
\question{11}
Dans un shel bash(1), lorsque l’on suffixe une commande avec 2>\&1,
...
\begin{solution}
Syntaxe : \lstinline|commande > out.txt 2>&1| ou \lstinline$commande 2>&1 | grep$
...
\question{12}
Comment se comporte l’appel système write(2) lorsque le disque sur lequel on souhaite écrire des données est plein ?
Peut-on distinguer cette erreur d’un problème d’écriture sur le disque ?

\begin{solution}
L'appel échoue avec le code d'erreur ENOSPC dans errno.
\end{solution}
...
\question{14}
Le programme \href{https://sites.uclouvain.be/SystInfo/notes/Exercices/html/_downloads/writeperf.c}{/Programmes/src/writeperf.c}
permet de tester les performances des écritures dans write sur un système de fichiers.
Compilez-le et exécutez le avec différents tailles de buffers passés à write(2).
Mesurez le temps d’exécution avec la commande time(1posix) et comparez le temps qu’il faut pour écrire 100MB en blocs de 1 MB ou en blocs de 1 KB.
Faites de même en activant la synchronisation avec le paramètre -s.
...
\question{1}
En utilisant signal(2), écrivez une fonction C \lstinline|sig_ignored| qui prend comme argument un numéro de signal et retourne \lstinline|true| si le signal passé en argument est ignoré par le processus et \lstinline|false| sinon.
...
On peut donc se permettre de temporairement remplacer le handler actuel par SIG\_DFL avec signal()
(pas SIG\_IGN car ça a des conséquences fâcheuses),
...
puis de nouveau appeler signal() pour remplacer le SIG\_DFL par l'"ancien" handler (en vrai, celui de départ),
et donc vérifier que ce qu'on reçoit soit bien SIG\_DFL.
On peut alors comparer le handler de départ à SIG\_IGN.

Note au niveau des signaux : par défaut, très peu de signaux sont ignorés (voire même aucun) :
ils apparaissent tous comme étant SIG\_DFL, même ceux qui ont le comportement SIG\_IGN.
La fonction va donc uniquement retourner les signaux ayant changé le handler vers l'ignorage (ou l'ignoration).

Autre petit soucis, affecter SIG\_IGN à un signal dont le comportement par défaut est similaire à SIG\_IGN n'est pas équivalent à lui affecter SIG\_DFL, et l'inverse est aussi vrai.
...
pire, il n'est même plus possible d'attendre ses enfants, car ils se terminent directement, et waitpid échoue avec un code d'erreur ECHILD.
\end{solution}
...
\question{4}
Le programme \href{https://sites.uclouvain.be/SystInfo/notes/Exercices/html/_downloads/alarm.c}{/Programmes/src/alarm.c}
que nous avons utilisé pour illustrer l’appel système alarm(2) contient une condition de course (\emph{race condition} mal traduit) qui peut dans certains cas empêcher l’interruption de l’appel système read(2).
...
\href{https://sites.uclouvain.be/SystInfo/notes/Exercices/html/_downloads/alarm3.c}{/Programmes/src/alarm3.c} ?

\begin{solution}
Il faut utiliser un jump au lieu d'une condition if sur le booléen.

Il est possible que le système, très chargé, nous fasse attendre entre l'appel à alarm et l'appel à read, suffisamment longtemps que pour que le signal soit délivré avant l'appel à read, que le handler s'exécute (incorrectement), et que le read se lance alors, attendant l'utilisateur, et ne pouvant plus être interrompu par un signal.

Première solution (du syllabus) : il faut empêcher l'exécution de read si l'alarme a déjà été reçue. Pour cela, on utilise un sigsetjmp encadrant les deux appels systèmes, et on utilise un siglongjmp dans le handler.
...
Si à un moment donné on se retrouve de nouveau dans l'instruction sigsetjmp, ce sera nécessairement à la suite de l'appel à siglongjmp(env, 1), et donc l'alarme aura expirée et on n'aura pas à enregistrer une nouvelle alarme ou à commencer la lecture.

L'utilisation de la variable expired dans le code de alarm3.c, ne résout rien :
il est tout à fait possible que le processus soit mis en pause entre la vérification du if et l'appel à read, et donc que le read s'exécute après que le signal ait été délivré.
En fait, il est vraiment nécessaire que le programme repasse dans cette portion de code comprise entre alarm et read inclus tous les deux :
il faut en effet pouvoir ne pas exécuter read si jamais l'alarme se déclenche, mais il est possible qu'elle arrive avant l'appel à read.
Avec les sigjmp, le cas d'une interruption à mi-chemin fait que read n'est pas exécuté.

Petite note : en fait, il est tout à fait possible que le programme plante pour une raison extérieure.
En effet, si quelqu'un d'extérieur envoie un signal SIGALRM à notre processus, juste après que l'on ait enregistré le handler, et juste avant qu'on exécute sigsetjmp, alors le handler sera appelé, et siglongjmp plantera car le buffer d'environnement n'aura pas encore été initialisé.
Mais bon, on peut considérer cela comme un acharnement à provoquer un bug plutôt qu'un bug réel.

Autre petite note : siginterrupt permet de gérer le redémarrage automatique ou non de certains appels systèmes lors de l'arrivée d'un signal.
Ici, on désactive le redémarrage des appels systèmes après récupération d'un signal.
\end{solution}

\question{5}
La page de manuel signal(7) définit les signaux suivants :
\begin{itemize}
	\item SIGSYS
	\item SIGPIPE
\end{itemize}
...
Ca peut arriver aussi si on appelle directement les appels systèmes avec syscall.
...
\begin{itemize}
	...
	Exemple : cat file.txt | ./buggy : si buggy se crash dès le début, la redirection de cat va planter pour lui éviter du travail inutile.
\end{itemize}
A noter que le signal n'est envoyé que si le file descriptor correspondant au pipe en lecture est fermé chez tout le monde : le père et le fils.
Tant que le fils ne s'exécute pas, son file discriptor reste ouvert, peu importe le temps d'attente.
...
\begin{solution}
SIGCHLD : utilisé par wait et waitpid par exemple.
...
\end{lstlisting}
Cette fonction utilise l’appel système write(2) qui peut modifier la variable errno en cas d’erreur.
Modifiez-la de façon à ce que toute modification à errno n’affecte pas l’utilisation de errno dans le programme en cours d’exécution.

\begin{solution}
Déjà, faire un write dans un \emph{signal handler} est une mauvaise idée :
ça va prendre beaucoup trop de temps.

Or, errno risque d'être modifié par le write.
Mais aussi par une autre fonction.

Donc, on doit copier errno avant d'exécuter write, puis réaffecter errno.
...
\question{9}
Dans les programmes
\href{https://sites.uclouvain.be/SystInfo/notes/Exercices/html/_downloads/process-sem-before.c}{/Programmes/src/process-sem-before.c} et
\href{https://sites.uclouvain.be/SystInfo/notes/Exercices/html/_downloads/process-sem-after.c}{/Programmes/src/process-sem-after.c},
on a choisi de créer le sémaphore dans un processus avec sem\_open(3) et de le supprimer avec sem\_unlink(3) dans un autre processus.
...
Faut juste faire attention que lorsqu'on appelle sem\_unlink, le sémaphore ne peut plus être ouvert par un autre processus (mais il reste ouvert chez tout ceux qui l'ont déjà).
Donc il faut appeler sem\_unlink après que tous les processus qui pourraient en avoir besoin l'aient ouvert.
Par exemple, on l'ouvre, puis on fork tous les processus, puis on attend qu'ils l'aient tous ouvert (on pourrait par exemple leur demander de poster sur ce sémaphore), puis on unlink.
Si les processus ne partagent pas de lien de parenté, c'est un peu plus compliqué.
\end{solution}

\question{10}
Un processus P ouvre le fichier fichier avec l’appel système open(2). Celui-ci est associé au descripteur fd. Ensuite, le processus utilise fork(2) pour créer un processus fils.
\begin{itemize}
	\item représenter les open-file objects utilisés
	\item le père et le fils peuvent-ils lire sur fd ?
	\item le processus fils utilise l’appel système chmod(2) pour rendre le fichier inaccessible en écriture et lecture. Après cette opération, le père et le fils peuvent-ils lire sur fd ?
	\item le père ferme le descripteur fd. Après cette opération, le père et le fils peuvent-ils lire sur fd ?
	\item le fils exécute unlink(2) sur le fichier. Après cette opération, le père et le fils peuvent-ils lire sur fd ?
\end{itemize}

\begin{solution}
\begin{itemize}
	\item L'open file object (file description, qui contient entre autre l'offset) est stocké dans le noyau, et est donc partagé, offset compris (du moins s'il a été ouvert dans un père ; deux processus différents ouvrant le même fichier auront bien des open file objects différents). Les deux file descriptors sont de simples entiers, chaque processus a son propre entier, mais ils valent la même chose.
	\item Les deux processus peuvent lire sur fd (du moins si on l'a demandé dans open).
	\item Ca ne fait rien : ils l'ont déjà ouvert (open) avant, les permissions ne sont plus vérifiées par la suite.
	\item Le père ne peut plus (vu que son fd est invalide pour son processus), le fils oui (c'est son fd, même s'il partage le file description).
	\item Unlink va supprimer le fichier du disque, mais uniquement après que tous les processus l'ont fermé. Donc, le père n'y a pas accès, le fils non plus (il appelle d'abord close), et donc le fichier est probablement supprimé. Si le père n'avait pas closé avant, il aurait néanmoins toujours eu accès au fichier, mais le fils pas non plus.
\end{itemize}
\end{solution}

\question{11}
Même question que ci-dessus, mais en remplaçant le fichier par un sémaphore nommé et open(2) par sem\_open(3) ?
...
\begin{itemize}
	...
	\item Toute personne l'ayant ouvert peut le modifier, et ils sont conservés par fork.
	\item Il n'y a pas de chmod pour les sémaphores, si ? %FIXME
	\item Le père ne peut plus se servir du sémaphore, le fils oui.
	\item Lorsque le fils fermera son sémaphore, celui-ci sera supprimé du système. En attendant, il persiste en mémoire, pouvant être utilisé par tous les processus l'ayant ouvert avant l'appel à unlink.
\end{itemize}
...
\question{12}
Le programme \href{https://sites.uclouvain.be/SystInfo/notes/Exercices/html/_downloads/miniprojet.c}{/Programmes/src/miniprojet.c}
utilise des sémaphores nommés pour synchroniser la production/consommation entre deux processus.
Le processus père crée le fichier my\_file.txt y écrit une valeur.
...
Après avoir terminé le programme avec Ctrl-C vous verrez qu’il reste des sémaphores dans /dev/shm/.
...
\question{1}
L’appel système shmget(2) supporte également le drapeau IPC\_EXCL en plus du drapeau IPC\_CREAT.
Expliquez à quoi ce drapeau peut servir et dans quel cas il peut être utile.

\begin{solution}
Utilité de IPC\_CREAT : créer la nouvelle clé si elle n'existe pas ;
sans ce drapeau, si la clé n'existe pas, l'appel échoue.

Utilité de IPC\_EXCL : fait échouer l'appel système si la clé existe déjà ;
assure que l'appel crée la clé. Il faut spécifier IPC\_CREATE aussi.
\end{solution}
...
\question{3}
Lorsque l’on utilise l’appel système shmat(2) avec \lstinline|NULL| comme deuxième argument, le système d’exploitation choisit l’adresse à laquelle le segment de mémoire va être attaché.
Cela pose des difficultés si l’on veut stocker des pointeurs en mémoire partagée.
Un étudiant propose de d’abord allouer la zone mémoire avec malloc(3) et d’ensuite attacher le segment de mémoire à cet endroit.
...
Un dernier problème se passe lorsqu'on détruit la zone de mémoire partagée (via shmdt ou via munmap) :
...
Compilez les programmes \lstinline|svshm_create|, \lstinline|svshm_attach| et \lstinline|svshm_rm| pour créer, attacher et supprimer un segment de mémoire partagée.
Les commandes ipcs(1) et ipcrm(1) permettent de visualiser et supprimer les segments de mémoire partagée.
...
\begin{itemize}
	\item Avec -c, crée la zone. Si -c n'est pas mis, plante si elle n'existe pas (équivalent de IPC\_CREATE)
	\item Avec -x en plus (ou même uniquement), si la clé existe déjà, ça plante (équivalent de IPC\_EXCL)
...
\begin{itemize}
	...
	\item L'argument R n'a pas été observé, vu que le programme de test ne fait rien d'autre qu'attacher la page.
...
C'est à peu près la même commande que ipcrm(1).
...
on constate que le segment reste en mémoire, mais avec un "DEST" à côté (sa destruction est planifiée),
...
Encore mieux, tant que le segment est en mémoire, même s'il est marqué de DEST, un nouveau processus peut toujours attacher cette zone de mémoire partagée ;
