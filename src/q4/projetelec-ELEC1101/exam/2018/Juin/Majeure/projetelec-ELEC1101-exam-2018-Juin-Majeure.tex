...
\begin{figure}[H]
    \centering
    \centerline{\includegraphics[width=1\linewidth]{tableau_signaux.png}}
    \caption{Tableau des signaux}
    \label{fig:tab}
\end{figure}

\section{Quel est le nom typique du bloc "???" sur la figure 8? Donner son implémentation circuit.}
...
\section{Dessiner sur le signal en sortie de la réception directe sur la fig 9 l'allure de la sortie du détecteur de crête}
...
Le seuil de basculement de la diode est de $\approx 0.7Volt$, ce qui empêche la détection de signaux de faible amplitude. De plus, la crête sera systématiquement 0.7 volt moins élevé que $Vmax$. Utiliser une super diode (qui a un seuil de basculement valant $\frac{\approx 0.7}{gain de l'oaop}$) résous ce problème.
...
Le schéma bloc de la figure 8 ne traite pas le cas ou des réflexions multiples sont captés.
...
\section{À l’aide des graphes de la figure 2, dimensionner le détecteur de crête.}
\begin{solution}
