...
\vspace{-0.5cm}
\begin{solution}
Le produit GBP dans un TL084 et TL082 est typiquement de $3MHz$ (cf. datasheet). Vu que le signal est à une fréquence de 40kHz, le gain maximum par amplificateur est d'environ $\frac{3 000 000}{40 000}=75$. Un seul amplificateur ne convient donc pas. Deux amplificateurs en série ne conviennent pas non plus, car le produit de leur gain est également inférieur à 10 000 ($75*75<10 000$). Il faut trois amplificateurs en série pour pouvoir amplifier sans dépassé le produit GBP. La réponse est donc la F.
\end{solution}
...
\begin{solution}
Le LM319 va trigger sur le flanc descendant du signal d'entré. Il n'est pas retriggerable.
Le NE555 n'est pas non plus retriggerable et ce déclanche également sur le flanc descendant. 
La seul différence entre les deux montages et que le montage avec NE555 va pouvoir prendre plus régulièrement des impulsions en entrée.
...
{\centering Inserer mise en contexte}

\section{Quelle est le nom typique du bloc ? sur la figure xx? Donner son implémentation circuit}
...
\section{Dessiner sur la fig xx l'allure de la sortie du détecteur de crête sur le graphe xx}
...
Le seuil de basculement de la diode est de 0.7Volt, ce qui empêche la détection de signaux de faible amplitude. Utiliser une super diode (qui a un seuil de basculement valant $\frac{0.7}{gain de l'oaop}$) résout ce problème.
\end{solution}
...
\section{A l’aide des graphes de la figure 2, dimensionner le détecteur de crête.}
