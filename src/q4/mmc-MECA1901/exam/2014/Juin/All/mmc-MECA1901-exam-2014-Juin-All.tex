...
\hypertitle{Mécanique des milieux continus}{4}{MECA}{1901}{2014}{Juin}{All}
...
\[
	\mathrm{Tr}(\bm{\sigma})=\sigma_{ii}
\]
Elle est utilisée par exemple dans l'équation locale de conservation de la masse dans le cas d'un milieu incompressible:
\[
...
La façon la plus simple de garantir systématiquement cette inégalité, pour toutes les valeurs de $\nabla T$, compte tenu du fait que $T > 0$, est de choisir $\textbf{q}$ comme suit (c'est la loi de Fourier pour le flux de chaleur) :

\begin{equation}
\textbf{q} = - k \nabla T
\end{equation}

Où $k$ est une constante positive appelée la \textit{conductivité thermique}.
