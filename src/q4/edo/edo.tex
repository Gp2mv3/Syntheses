...
\begin{align*}
  v'(t) & = -A(t)^*[u(t)] + f(t) && \forall t \in [a,b]\\
  (\alpha|v(a)) & = (\beta|v(b)) &&
...
\[ A = PDP^{-1}. \]
$P$ est alors la matrix qui transforme $v$ en ses coordonnées dans
la base $v_1, \dots, v_n$, $D$ a sur sa diagonale les valeurs propres
$\lambda_1, \dots, \lambda_n$ et $P$ permet de passée des coordonnées
...
Le théorème fondamental de l'algèbrene nous assûre malheureusement
...
\begin{itemize}
  ...
  \item La mauvaise c'est que que comme $m_g(\lambda_i) \leq m_a(\lambda_i)$,
...
une fois qu'on a exprimer $v$ dans une base de vecteurs propres.
...
$\newnull(A - \lambda_i I) = m_a(\lambda_i)$.
