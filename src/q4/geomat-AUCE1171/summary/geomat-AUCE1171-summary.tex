...
Comme le sable qui, lui, peut éjecter/capter facilement de l'eau de ses pores, l'eau peut drainer à travers le sol la pression interstitielle ne changera pas après application de la charge (figure~\ref{fig:load}). On parle alors de sol \textit{drainé}. L'argile, quant à lui, dans un premier temps, ne peut pas éjecter d'eau de ses pores, la pression interstitielle va donc augmenter. La contrainte effective, elle ne change pas. Avec le temps, l'argile va petit à petit éjecter l'eau de ses pores et rejoindre le même état que le sable. A court terme, l'eau ne peut pas drainer à travers l'argile, le sol est dit \textit{non-drainé}, mais à long terme, celle-ci en est capable, le sol est donc dit, comme le sable, \textit{drainé}.
...
Une autre information donnée par le forage est l'indice de qualité RQD (\textit{Rock Quality Designation}). Celui-ci est le rapport de la somme des carottes de plus de \unit{10}{\centi\meter} de longs et de la longueur de passe. Celui-ci est exprimé en \% (tableau~\ref{rqd2}).
