...
\usepackage{tikz}
\usepackage{enumitem}
\usepackage{physics} % pour avoir Re et Im au lieu de R et I en fraktur, pour les parties réelles et imaginaires.
...
\begin{enumerate}[label=(\alph*)]
	...
	La partie réelle est coefficients $X[k]$ est un signal pair, et la partie imaginaire est un signal impair.
...
\draw (0.1, 1.5) -- (-0.1, 1.5) node[left] {$\frac{\pi}{j}\pi$};
\end{tikzpicture}
\end{center}
...
\begin{enumerate}[label=(\alph*)]
	...
	% Axes
	\draw[thick, ->, >=latex] (-3, 0) -- (3, 0) node[below right] {$f$};
	\draw[thick, ->, >=latex] (0, -4) -- (0, 4) node[above right] {$\Im(X(jf))$};
