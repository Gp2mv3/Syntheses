...
\hypertitle{Signaux et systèmes}{4}{FSAB}{1106}{2017}{Juin}{All}
{Jean-Martin Vlaeminck}
{Luc Vandendorpe et Vincent Wertz}

\section{LV1}
Soit un système linéaire permanent (c-à-d invariant dans le temps) de réponse impulsionnelle $h(t)$ et de réponse en fréquence $H(\imagj \omega)$.
...
\begin{solution}
On a
\[ x(t) = \sin (\omega_0 t + \phi) = \frac{\e^{\imagj (\omega_0 t + \phi)} - \e^{-\imagj (\omega_0 t + \phi)}}{2\imagj } \]
et donc, la transformée de Fourier du signal est
\[ x(t) \leftrightarrow X(\imagj \omega) = \frac{\pi}{\imagj } \e^{\imagj \phi} \delta(\omega-\omega_0) - \frac{\pi}{\imagj } \e^{-\imagj \phi} \delta(\omega+\omega_0)\]
Dès lors,
\begin{align*}
Y(\imagj \omega) &= H(\imagj \omega) \cdot X(\imagj \omega) \\
&= H(\imagj \omega) \cdot \left( \frac{\pi}{\imagj } \e^{\imagj \phi} \delta(\omega - \omega_0) - \frac{\pi}{\imagj } \e^{-\imagj \phi} \delta(\omega + \omega_0) \right) \\
&= H(\imagj \omega_0) \frac{\pi}{\imagj } \e^{\imagj \phi} \delta(\omega - \omega_0) - H(-\imagj \omega_0) \frac{\pi}{\imagj } \e^{-\imagj \phi} \delta(\omega + \omega_0) \\
&= \abs{H(\imagj \omega_0)} \frac{\pi}{\imagj } \e^{\imagj \phi} \e^{\imagj  \arg(H(\imagj \omega_0))} \delta(\omega-\omega_0) - \abs{H(-\imagj \omega_0)} \frac{\pi}{\imagj } \e^{-\imagj \phi} \e^{\imagj \arg( H(-\imagj \omega_0))} \delta(\omega+\omega_0) \\
&= \abs{H(\imagj \omega_0)} \frac{\pi}{\imagj } \e^{\imagj \phi} \e^{\imagj  \arg(H(\imagj \omega_0))} \delta(\omega-\omega_0) - \abs{H(\imagj \omega_0)} \frac{\pi}{\imagj } \e^{-\imagj \phi} \e^{-\imagj \arg( H(\imagj \omega_0))} \delta(\omega+\omega_0) \\
&= \abs{H(\imagj \omega_0)} \left( \frac{\pi}{\imagj } \e^{\imagj  (\phi + \arg(H(\imagj \omega_0)))} \delta(\omega-\omega_0) - \frac{\pi}{\imagj } \e^{-\imagj (\phi + \arg(H(\imagj \omega_0)))} \delta(\omega+\omega_0) \right)
\end{align*}
dont la transformée inverse est
\begin{align*}
Y(\imagj \omega) \leftrightarrow y(t) &= \abs{H(\imagj \omega_0)} \frac{\e^{\imagj  (\omega_0 t + \phi + \arg(H(\imagj \omega_0))} - \e^{-\imagj  (\omega_0 t + \phi + \arg(H(\imagj \omega_0)))}}{2\imagj } \\
&= \abs{H(\imagj \omega_0)} \sin (\omega_0 t + \phi + \arg(H(\imagj \omega_0)))
\end{align*}
un résultat bien connu, et relativement logique : le signal de départ est déphasé de $\arg(H(\imagj \omega_0))$, et est multiplié par $\abs{H(\imagj \omega_0)}$.
...
\begin{enumerate}[label=(\alph*)]
	...
	\begin{itemize}
		\item La transformée de Fourier discrète d'un signal $a[n]$, de durée finie $N$, (TFD, ou DFT en anglais) peut s'interpréter comme l'échantillonnage de la DTFT (transformée de Fourier en temps discret) de ce signal $a[n]$. En notant $a_{\textnormal{P}}[n]$ le signal obtenu en inversant la DFT\footnote{Au facteur $N$ près, c'est la même expression que pour la série de Fourier.}, cet échantillonnage a comme conséquence de rendre le signal $a_{\textnormal{P}}[n]$ périodique, de période $N$. Comme il y a $N$ échantillons sur une période de $2\pi$, on écrit $A[k] = X\left(\e^{\imagj  \frac{2k\pi}{N}}\right)$, avec donc des échantillons espacés de $\frac{2\pi}{N}$ dans la fréquence $\Omega$.
		\item Étant donné un signal $a(t)$, son échantillonnage $a_{\textnormal{e}}(t)$ et $a_{\textnormal{e}}[n]$ à la période d'échantillonnage $T$ donne une transformée de Fourier $A_{\textnormal{e}}(\imagj \omega)$ périodique de période $\frac{2\pi}{T}$, qui est une mise à l'échelle de la transformée de Fourier en temps discrète $A_{\textnormal{e}}(\e^{\imagj \Omega})$, de période $2\pi$. Les fréquences $\omega$ et $\Omega$ sont reliées par la relation $\omega = \Omega/T$.
		\item Enfin, la transformée de Fourier $A(\imagj \omega)$, utilisant des fréquences en radians par seconde, peut être convertie en une transformée de Fourier $A(\imagj f)$ utilisant des hertz via $\omega=2\pi f$.
	\end{itemize}
	...
	Une autre propriété sympathique est que $\Im(X[mN]) = 0$ : la DFT est purement réelle à chaque multiple de $N$.

	% TODO compléter par d'autres propriétés, s'il y en a.

	\item En reprenant la définition, et en se souvenant que $N = 8$ est pair,
	\begin{align*}
	Y[k] &= \sum_{n=0}^{N-1} y[n] \e^{- \frac{2 \pi \imagj  k n}{N}} \\
	&= \sum_{n=0}^{N-1} x[n] (-1)^n \e^{- \frac{2 \pi \imagj  k n}{N}} \\
	&= \sum_{n=0}^{N-1} x[n] \e^{- \pi \imagj  n} \e^{- \frac{2 \pi \imagj  k n}{N}} \\
	&= \sum_{n=0}^{N-1} x[n] \e^{- \frac{2 \pi \imagj  k n + \pi \imagj  N n}{N}} \\
	&= \sum_{n=0}^{N-1} x[n] \e^{- \frac{2 \pi \imagj  (k + \frac{N}{2}) n}{N}} \\
...
%	\end{align*}
	...
	\begin{align*}
	h[n]=(-1)^n \longleftrightarrow H[k] &= \sum_{n=0}^{N-1} h[n] \e^{-\frac{2\pi \imagj  k n}{N}}
	= \sum_{n=0}^{N-1} \e^{-\imagj  \pi n} \e^{-\frac{\imagj  2 \pi k n}{N}}
	= \sum_{n=0}^{N-1} \e^{-\imagj  \pi \frac{N + 2k}{N} n} \\
	&= \frac{1 - \e^{-\imagj  \pi (N+2k)}}{1 - \e^{-\imagj \pi \frac{N+2k}{N}}}.
	\end{align*}
	...
	\begin{align*}
	\e^{-\imagj \pi\frac{N+2k}{N}} = 1 \Leftrightarrow
	\pi\frac{N+2k}{N} &= 2m\pi \quad m\in\mathbb{Z} \\
	N+2k &= 2mN \\
	k &= (2m-1)\frac{N}{2}
	\end{align*}
	autrement dit, lorsque $k=\frac{N}{2}, \frac{3N}{2}, \dots$. Dans ces cas, on a
	\begin{equation*}
	H\left[\frac{N}{2}\right] = \sum_{n=0}^{N-1} (-1)^n \e^{-\frac{2 \pi \imagj  N n}{2 N}}
	= \sum_{n=0}^{N-1} (-1)^n \e^{- \pi \imagj  n}
...
\section{LV3}
On génère un signal périodique $x(t) = \sin (2 \pi f_0 t)$. Ce signal est d'abord échantillonné par un échantillonneur\footnote{Sans blague ?} qui fonctionne à la fréquence $f_{\textnormal{e}}$. Ensuite, le résultat de l'échantillonnage est interpolé par un filtre réel, fonctionnant en temps continu de type passe-bas idéal et de fréquence de coupure $f_{\textnormal{e}}/2$ (il laisse donc passer toutes les fréquences $f$ telles que $\abs{f} \le f_{\textnormal{e}}/2$). %FIXME < ou <= ?
...
\begin{center}
	...
	\draw (2.5, -1.2) node[] {$f_{\textnormal{e}}$};
...
\end{center}
\begin{enumerate}[label=(\alph*)]
	\item Représentez graphiquement le spectre du signal obtenu en sortie du filtre analogique lorsque $f_0 = f_{\textnormal{e}}/4$. Justifiez.
	\item Pareil lorsque $f_0 = 7f_{\textnormal{e}}/8$. Justifiez.
	\item Représentez graphiquement le spectre du signal obtenu en sortie du filtre analogique lorsque, à $f_{\textnormal{e}}$ fixé, on augmente $f_0$ de $0$ jusque $3f_{\textnormal{e}}/2$. Justifiez.
\end{enumerate}

\begin{solution}
L'échantillonnage va provoquer une réplication, tous les $f_{\textnormal{e}}$ (ou $\omega_{\textnormal{e}} = 2 \pi f_{\textnormal{e}}$), du spectre du signal de départ. Le filtre passe-bas va ensuite éliminer l'ensemble de ce spectre qui se trouve en dehors de l'intervalle $[-f_{\textnormal{e}}/2, f_{\textnormal{e}}/2]$ ; cela va éliminer une partie des réplicants, mais va peut-être aussi éliminer une partie du spectre du signal de départ, ou va laisser des réplicants.
...
\[ X(\imagj \omega) = \frac{\pi}{\imagj } \delta(\omega - 2\pi f_0) - \frac{\pi}{\imagj } \delta(\omega + 2 \pi f_0) \]
et chacun des spectres restera d'ailleurs purement imaginaire, sans partie réelle, qui ne sera donc pas représentée.

Lors de l'examen, seul le spectre en sortie du filtre passe-bas était demandé ; les autres spectres ne devaient pas être dessinés. Également, le spectre était demandé, pas son module, ce qui explique le signe des impulsions de la figure suivante\footnote{Remarquez que la graduation sur l'axe est de $\pi/\imagj $ : le pic de gauche a en fait une partie imaginaire positive.}.
\begin{center}
\begin{tikzpicture}
\draw[thick, ->, >=latex] (-3, 0) -- (3, 0) node[below right] {$f$};
\draw[thick, ->, >=latex] (0, -1.6) -- (0, 2) node[above right] {$X(\imagj f)$};
\draw[thick, Green, ->, >=stealth] (1, 0) -- (1, 1.5);
\draw (1, 0) node[below] {$f_0$};
\draw[thick, Green, ->, >=stealth] (-1, 0) -- (-1, -1.5);
\draw (-1, 0) node[above] {$-f_0$};
\draw (0.1, 1.5) -- (-0.1, 1.5) node[left] {$\frac{\pi}{\imagj }$};
\end{tikzpicture}
\end{center}

\begin{enumerate}[label=(\alph*)]
	\item Lorsque $f_0 = f_{\textnormal{e}}/4$, l'hypothèse du théorème d'échantillonnage est satisfaite (la fréquence d'échantillonnage $f_{\textnormal{e}}$ est strictement supérieure au double de la fréquence $f_0$), et donc le signal de départ peut être reconstruit à partir du signal en sortie du filtre passe-bas ; il n'y a pas de repli spectral.
	\begin{center}
	...
	% Axes
	\draw[thick, ->, >=latex] (-3, 0) -- (3, 0) node[below right] {$f$};
	\draw[thick, ->, >=latex] (0, -4) -- (0, 4) node[above right] {$X(\imagj f)$};
	\draw (0.05, 3.14) -- (-0.05, 3.14) (0, 3.14) node[above left] {$\frac{\pi}{\imagj }f_{\textnormal{e}}$};
	\draw (-0.05, -3.14) -- (0.05, -3.14) (0, -3.14) node[below right] {$\frac{-\pi}{\imagj }f_{\textnormal{e}}$};
	% Zone du filtre
	...
	\draw[thin, Green, dashed, -latex] (2.25, 0) -- (2.25, 3.14);
	% Coordonnées
	\draw (-0.25, 0) node[above] {$-\frac{f_{\textnormal{e}}}{4}$};
	\draw (0.25, 0) node[below] {$\frac{f_{\textnormal{e}}}{4}$};
	\draw (0.75, 0) node[above] {$\frac{3f_{\textnormal{e}}}{4}$};
	\draw (1.25, 0) node[below] {$\frac{5f_{\textnormal{e}}}{4}$};
	\end{tikzpicture}
	\end{center}
	\item Cette fois, l'hypothèse du théorème de Shannon n'est plus satisfaite, et il y a repli spectral (\emph{aliasing}) ; après le filtre passe-bas, il est impossible de reconstituer le signal de départ. En fait, le signal s'est transformé\footnote{Il a pris une autre forme, une sorte d'alias.} en $y(t) = -\sin (2 \pi f_{\textnormal{e}} / 8)$.
	\begin{center}
	\begin{tikzpicture}[x=1.5cm, y=0.5cm]
	% Axes
	\draw[thick, ->, >=latex] (-3, 0) -- (3, 0) node[below right] {$f$};
	...
	\draw[thin, Green, dashed, -latex] (2.125, 0) -- (2.125, -3.14);
	% Coordonnées
	\draw (-0.875, 0) node[above] {$-\frac{7f_{\textnormal{e}}}{8}$};
	\draw (0.875, 0) node[below] {$\frac{7f_{\textnormal{e}}}{8}$};
	\draw (0.125, 0) node[above] {$\frac{f_{\textnormal{e}}}{8}$};
	\draw (1.875, 0) node[below] {$\frac{15f_{\textnormal{e}}}{8}$};
	\end{tikzpicture}
	\end{center}
	\item Pour $0 < f_0 < f_{\textnormal{e}}/2$, le spectre du signal $x(t)$ ne subit aucun repli spectral, et ressort indemne du filtre passe-bas.
	% TODO insérer figure
	% c'est fait non?
	\begin{center}
	\begin{tikzpicture}[x=1.5cm, y=0.5cm]
	...
	Pour $f_0 = f_{\textnormal{e}}/2$, les deltas se superposent parfaitement, et s'annulent ; le spectre du signal disparait en sortie de l'échantillonneur (ce qui est logique, le signal étant nul à chaque échantillon), et donc le signal en sortie du filtre passe-bas est nul.

	Pour $f_{\textnormal{e}}/2 < f_0 < f_{\textnormal{e}}$, il y a repli spectral, le spectre du signal original sortant de la fenêtre du filtre passe-bas, et les diracs des répliques adjacentes entrant dans la fenêtre. Le signal en sortie du filtre passe-bas n'est plus le même que le signal de départ, et la fréquence de ce signal diminue (les diracs sortant du filtre se rapprochent de $0$).\footnote{En notant $f_1$ la fréquence de ce signal, on peut en fait constater que $f_1 = f_{\textnormal{e}} - f_0$.}
	% TODO insérer figure
	\begin{center}
	\begin{tikzpicture}[x=1.5cm, y=0.5cm]
	% Axes
	...
	Pour $f_0 = f_{\textnormal{e}}$, le signal ressortant de l'échantillonneur est de nouveau nul, pour les mêmes raisons que $f_0 = f_{\textnormal{e}}/2$.

	Pour $f_{\textnormal{e}} < f_0 < 3f_{\textnormal{e}}/2$, il y a toujours repli spectral, mais les diracs des répliques s'éloignent de $0$, et la fréquence du signal de sortie\footnote{Égal à $f_2 = f_0 - f_{\textnormal{e}}$.} augmente.
	% TODO insérer figure
	\begin{center}
	\begin{tikzpicture}[x=1.5cm, y=0.5cm]
	% Axes
...
\begin{solution}
\begin{enumerate}[label=(\alph*)]
	\item Les pôles de la fonction de transfert sont en $1$, en $-\frac{3}{4} + \imagj \frac{\sqrt{7}}{4}$ et en $-\frac{3}{4} - \imagj \frac{\sqrt{7}}{4}$. Comme $1$ est à droite de l'axe imaginaire, et que le système est causal, le système n'est pas stable BIBO (il faudrait que tous les pôles soient à gauche de l'axe imaginaire, donc à partie réelle négative).
	...
	Le critère de Routh-Hurwitz stipule que le nombre de racines à partie réelle positive est égal au nombre de changements de signe des nombres de la première colonne. Comme on veut déterminer l'ensemble des $K$ tels que toutes les racines ont des parties réelles négatives, il faut que tous les nombres de la première colonne soient strictement positifs, et donc il faut que
	\[
	\begin{cases}
