...
\begin{enumerate}[label=(\alph*)]
	...
	\begin{itemize}
		\item La transformée de Fourier discrète d'un signal $a[n]$, de durée finie $N$, (TFD, ou DFT en anglais) peut s'interpréter comme l'échantillonnage de la DTFT (transformée de Fourier en temps discret) de ce signal $a[n]$. Cet échantillonnage a comme conséquence de rendre le signal $a[n]$ périodique, de période $N$. Comme il y a $N$ échantillons sur une période de $2\pi$, on écrit $A[k] = X\left(e^{j \frac{2k\pi}{N}}\right)$, avec donc des échantillons espacés de $\frac{2\pi}{N}$ dans la fréquence $\Omega$.
	...
	ce qui semble très plausible : comme on échantillonne un signal $x(t)$, en prenant $N$ échantillons séparés par un temps $T$, pour une longueur totale de $NT$, et que la DFT nous renvoie également $N$ échantillons, on peut s'attendre à ce que ces échantillons soient répartis sur la distance $\frac{1}{NT}$ en fréquence.
...
\draw[thick, ->, >=latex] (0, -2) -- (0, 2) node[above right] {$X(jf)$};
...
\begin{enumerate}[label=(\alph*)]
	...
	\end{tikzpicture}
	\end{center}
	\item Cette fois, l'hypothèse du théorème de Shannon n'est plus satisfaite, et il y a repli spectral ; après le filtre passe-bas, il est impossible de reconstituer le signal de départ. En fait, le signal s'est transformée, par \emph{aliasing}, en $y(t) = -\sin (2 \pi f_e / 8)$.
	\begin{center}
	\begin{tikzpicture}[x=1.5cm, y=0.5cm]
	% Axes
	\draw[thick, ->, >=latex] (-3, 0) -- (3, 0) node[below right] {$f$};
