...
\newcommand{\laplacu}{\mathcal{L}_{\text{u}}}
\newcommand{\transfz}{\mathcal{Z}}
\newcommand{\transzu}{\mathcal{Z}_{\text{u}}}
...
On nomme $T_\text{s}$, la période entre deux évaluations.
En effet, passer d'un signal continu $x(t)$ à un signal discret
$x[n] = x(nT_\text{s})$ est appelé de l'échantillonnage qui en anglais se dit
``sampling''. D'où le $T_\text{s}$ qui réfère à la période de ``sampling''.
...
\[ x(t) = x_\text{e}(t) + x_\text{o}(t). \]
On peut alors prouver que
\[ x_\text{e}(t) = \frac{1}{2}[x(t) + x(-t)], \]
\[ x_\text{o}(t) = \frac{1}{2}[x(t) - x(-t)]. \]
...
simplement $x(t) =  \sum_{n=-\infty}^{+\infty} x[n] \delta(t-nT_\text{s})$, c'est à dire
qu'on aurait, en notant $\Omega = \omega T_\text{s}$,
\begin{align}
  \label{eq:tfd}
  X(j\omega) & = \int_{-\infty}^{+\infty} \sum_{n=-\infty}^{+\infty} x[n]
  \delta(t-nT_\text{s}) e^{-j\omega T_\text{s}n} \dif t\\
  \nonumber
  & = \sum_{n=-\infty}^{+\infty} x[n] e^{-j\Omega n}.
\end{align}
Seulement, on peut remarquer que
\begin{align*}
  X(j(\omega+2\pi/T_\text{s})) & = \sum_{n=-\infty}^{+\infty} x[n]
  e^{-j(\Omega+2\pi) n} \dif t\\
  & = \sum_{n=-\infty}^{+\infty} x[n]
  e^{-j\Omega n} \dif t\\
  & = X(j\omega).
\end{align*}
La transformée de Fourier serait donc périodique de période $\frac{2\pi}{T_\text{s}}$.
...
évidente à saisir.
Lorsqu'on a un signal $x[n]$, parler de $f_\text{s}$ ou $T_\text{s}$ est un peu superflu.
En effet, avec par exemple un signal $x(t) = \sin(2\pi f t)$.
On a $x[n] = \sin\left(2\pi \frac{f}{f_\text{s}} n\right)$.
On voit bien ici qu'un même signal $x[n]$ peut être produit par $(f,f_\text{s}) = (2,2)$
ou par $(f,f_\text{s}) = (4,4)$ par exemple.
Lorsqu'on calcule la tranformée de Fourier discrète, on veut une transformée de Fourier
indépendante de $f_\text{s}$.
Même si, lorsqu'on connait $f_\text{s}$, on peut facilement repasser à la transformée de Fourier
classique en remplaçant $\Omega$ par $\omega T_\text{s}$.
...
\begin{description}
  \item[transformée de Fourier discrète]
    La transformée de Fourier est périodique de période $\frac{2\pi}{T_\text{s}}$
    lorsque $x$ est discret avec une période de discrétisation $T_\text{s}$.
...
$x$ est discret et de période $N$, sa période en continu est donc $NT_\text{s}$
car $T_\text{s}$ est la distance entre deux valeurs discrètes.

La transformée de Fourier a donc une période de discrétisation de $\omega_0 = \frac{2\pi}{NT_\text{s}}$
et une période de $\omega = \frac{2\pi}{T_\text{s}}$.
En prenant $\Omega = \omega T_\text{s}$ (et donc $\Omega_0 = \omega_0 T_\text{s}$)
...
est indépendant de $T_\text{s}$ et donc de $T = nT_\text{s}$.
On lui donne un vecteur de $N$ éléments, il nous renvoie $N$
éléments.
Il peut trouver $\Omega_0 = \frac{2\pi}{N}$ et utiliser
l'équation~\eqref{eq:sfd} sans se soucier de $T_\text{s}$.
On voit à la \sectionref{sfd} où le $T_\text{s}$ se simplifie.
À nous donc de nous souvenir quel $T_\text{s}$ on a utilisé.
On sait alors que $\omega_0 = \Omega_0 f_\text{s}$ et donc on peut
calculer
...
Malgré le surplus calculatoire, cela permet notamment d'appliquer la FFT sur un signal d'une longueur qui est une puissance de $2$, ce qui rend possible l'utilisation d'algorithmes particulièrement efficaces.
...
Lorsqu'on échantillonne un signal continu $x(t)$ en $x_\text{s}(t)$,
...
\[ \sum_{n=-\infty}^{+\infty} \delta(t-nT_\text{s}). \]
...
\[ \frac{1}{2\pi}
\frac{2\pi}{T_\text{s}}\sum_{k=-\infty}^{+\infty}\delta(\omega-2\pi k/T_\text{s})
= \frac{1}{T_\text{s}}\sum_{k=-\infty}^{+\infty}\delta(\omega-2\pi k/T_\text{s}). \]
On va donc prendre le spectre de $x(t)$ et on va le reproduire tous les
$2\pi/T_\text{s} = \omega_s$ en le divisant par $T_\text{s}$.
...
il faut donc que $\frac{2\pi}{T_\text{s}} > 2\omega_\mathrm{max}$ pour que les
reproductions de signal ne se chevauchent pas (on parle
sinon de repli spectral, ou d'\emph{aliasing} en anglais).
Comme $\omega_s = \frac{2\pi}{T_\text{s}}$, on a en fait $f_\text{s} > 2f_\mathrm{max}$.
...
et de remultiplier le signal par $T_\text{s}$.
...
qui vaut $T_\text{s}$ pour $-\pi/T_\text{s} \leq \omega < \pi/T_\text{s}$ et 0 sinon;
ce qui fait une convolution par $\sin(\pi t/T_\text{s})/(\pi t/T_\text{s})$ en temporel.
...
en fréquentiel par une fenêtre qui faut $T_\text{s}$ pour
...
$x_\text{s}(t)$ n'est pas nécessairement périodique.
Pour que $x_\text{s}(t)$ soit périodique, il faut et il suffit que
$T_\text{s}$ soit un multiple rationnel de la période de $x(t)$.
...
\begin{align*}
  X_{\text{u}}(s) & = \int_{0^-}^{+\infty} x(\tau) e^{-s\tau} \dif \tau.
\end{align*}
...
\begin{align*}
  ...
  s^2X_{\text{u}}(s) - sx(0^-) - x'(0^-)
\end{align*}
et pour des systèmes stables et causaux,
\begin{align*}
  x(0^+) & = \lim_{s\to\infty} sX_{\text{u}}(s)\\
  \lim_{t \to \infty} x(t) & = \lim_{s\to 0} sX_{\text{u}}(s).
\end{align*}
...
\begin{align*}
  X_{\text{u}}(z) & = \sum_{n=0}^{+\infty} x[n] z^{-n}.
\end{align*}
...
\begin{align*}
  ...
  z^{-1}X_{\text{u}}(z) + x[-1]\\
  \transzu\{x[n-2]\} & =
  z^{-2}X_{\text{u}}(z) + z^{-1}x[-1] + x[-2]\\
  \transzu(x[n-k]) & =
  z^{-k}X_{\text{u}}(z) + z^{-k+1}x[-1] + \ldots + z^{-1}x[-k+1] + x[-k]
\end{align*}
et pour des systèmes stables et causaux,
\begin{align*}
  \lim_{z \to\infty} X_{\text{u}}(z) & = x[0^+]\\
  \lim_{z\to 1} (z-1)X_{\text{u}}(z) & = \lim_{n \to \infty} x[n].
\end{align*}
