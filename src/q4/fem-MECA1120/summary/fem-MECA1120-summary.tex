...
mais seulement le premier est complet à l'ordre 2 en $\x$ \cite[p.~15]{legat2015fem}.

\begin{myrem}
  On distingue les sommets qui sont des éléments du maillage aux n\oe{}uds qui sont des points
  où est définie une valeur nodale de l'approximation~\cite[p.~5,8]{legat2015fem}.
...
\begin{myrem}
  Le degré de précision d'une méthode porte sur $fJ_e$, pas uniquement sur $f$!
\end{myrem}
...
On pourrait choisir que $\U = \{v \in \U_s \suchthat v(\x) = t(\x), \forall \x \in \Gamma_D\}$ et $\hU = \{v \in \U_s \suchthat v(\x) = 0, \forall \x \in \Gamma_D\}$ mais on ajoutera souvent d'autres restrictions~\cite[p.~22]{legat2015fem}.
...
\section{Techniques de résolution des systèmes linéaires creux}
