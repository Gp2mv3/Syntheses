...
Au final, les transformations courantes sont les suivantes.
...
entre $\hat{p}$ et $p$ pour nous simplifier la vie.
...
Ici on a mis $p$ pour \eqref{eq:p} et $\hat{p}$ pour \eqref{eq:pab} pour montrer
qu'on a le choix.
...
Ensuite, il nous suffit calculer un intervalle de confiance de $1-\alpha$ en utilisant le fait que $H_0$ est vrai,
