...
{Gaetan Cassiers\and Antoine Paris\and Philippe Verbist \and Pablo Laviolette}
{Claude Oestges}
...
Dans un matériau \textbf{linéaire}, \textbf{homogène}, \textbf{isotrope} et à réponse \textbf{instantanée}, le champ de déplacement électrique ou densité de flux électrique est donné par
\begin{equation}
	\D = \perm \E.
...
Le champ d’induction électrique $\D$ représente la façon dont le champ électrique $\E$ influe sur l’organisation des charges électriques dans un matériau donné, notamment le déplacement des charges (d'où la notation $\D$) et la réorientation des dipôles électriques.
...
On peut ainsi aussi écrire cette équation sous la forme locale (quand le volume tend vers 0)
\begin{equation}
	\divn \D = \rho_v
	\label{eq:gauss-dif}
\end{equation}
...
déplacer \textbf{une} charge unitaire positive d'un point à un
...
On peut réecrire cette équation sous forme locale (quand le chemin fermé se résume à un point)
\begin{equation}
	\rotn \E = 0
...
%\subsection{Energie potentielle magnétique} TODO ?
...
induite\footnote{Même sans support!} (emf ou $\EMF$) qui peut établir un courant sur un chemin fermé. Un champ magnétique variable crée donc un champ électrique.
\begin{equation}
	\EMF = - \fdif{\Phi}{t}.
	\u{\volt}
\end{equation}
...
Des charges électriques accélérées sont responsables de l’apparition d’une onde électromagnétique.
...
Ex :  lorsqu’un signal sinusoïdal est envoyé sur une ligne de transmission, il subira un délai.\\
 Les lignes usuelles sont uniformes, càd leur configuration transverse est indépendante de $z$.
...
On dérive pour obtenir l'équation des télégraphistes:
...
\begin{myrem}
Si $Z_0 = Z_L$ (ligne adaptée) , alors l'onde réfléchie $V_-$ est nulle et
\begin{equation}
Z_{in} = Z_0.
\end{equation}
On parle alors de \emph{matching} d'impédance. Le but étant
en général de transmettre un maximum de puissance à la charge,
il est en général souhaitable de réduire l'onde réfléchie. On retrouve aussi  $V_- = 0$ lorsque la ligne est infinie.
\end{myrem}
...
(typiquement, impulsions ou échelons). $Z_c$ est le rapport des amplitudes des ondes de tension et de courant se déplaçant dans un même sens.
...
(voir figure \ref{fig:tem}). Pour une onde TEM il faut au moins 2 conducteurs.  Ex : pas d’onde TEM à l’intérieur d’un tube creux en CEP ($\vec{E}=0$).
...
\begin{myprop}
Les composantes d'une onde TEM obéissent aux équations de la statique dans le plan transverse.
Dans le plan transverse (en tout $z$ donné), représentation par un circuit localisé (mais uniquement valable pour une longueur $dz$ infinitésimale).
\end{myprop}
