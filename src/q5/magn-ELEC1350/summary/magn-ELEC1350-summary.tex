...
\section{Équation du télégraphe}
...
\end{myrem}
\begin{myrem}
L'étude de telle lignes prend tout son sens lorsque l'on s'intéresse au transfert d'un signal sur une longue distance, ou, plus précisément, lorsque la distance parcourue est de l'ordre de grandeur de la longueur d'onde du signal. La référence aux télégraphes s'explique par le fait que historiquement, ils ont été parmi les premiers à ce confronter à ce type de problème (transfert d'une information sur une longue distance).
\end{myrem}
...
\begin{equation}
\beta \triangleq \frac{\omega}{v_p}= \frac{2\pi}{\lambda}
\end{equation}
...
\begin{equation}
\Gamma(z) = \frac{V_0^- e^{j\beta z}}{V_0^+ e^{-j\beta z} = \frac{Z(z)-Z_0}{Z(z)+Z_0}}
\end{equation}
\end{mydef}
...
\begin{equation}
Z_{in} = Z_0 \frac{Z_L cos(\beta l)+jZ_0sin(\beta l)}{Z_0 cos(\beta l) + j Z_L sin (\beta l)}
\end{equation}
\end{mydef}
