...
\begin{description}
    \item[ADSL] : Asymmetric Digital Subscriber Lines (le principe est d'avoir un gros lien down pour le particulier et un petit lien en up)
    \item[ARP] : Address Resolution Protocol
    \item[CRC] : Cyclical Redundancy Check
...
\subparagraphe{Loose source} Option IPv6 pour préciser la routes à prendre (on spécifie l'addresse des routeurs). Pas utilisé car problème de sécurit'é.
...
Potentiellement, tout les node IPv6 sont capable de captuer des frames destinés à un 
...
\begin{enumerate}
    \item Envoi un multicast \textbf{ICMPv6 Neighbor Solicitation} (NA) avec son adresse IPv6
    \item Le receiver répond avec un \textbf{ICMPv6 Neighbor Advertisement} avec son adresse IPv6 et MAC
    \item En recevant le ICMPv6 NA, il stock le lien IPv6-MAC dans sa \textbf{NDP table}
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Il vérifie son unicité en envoyant un \textbf{ICMPv6 NS} unicast
    \item Pour connaître le \textit{prefix IPv6 subnet}, le router envoi
        ...
        \textbf{ff02::} \footnote{Correspond à tout les hosts disponible} contenant : 
        \begin{itemize}
...
via \textbf{ff01::} \footnote{Correspond à tout les routers disponible} pour
...
\paragraph{Note: } Les routers envoient périodiquement (30sec) une ou plusieurs RIP response
reprenant les distances vectors qui résume la routing table.
...
L'inter-area routing est fait en échangeant des distance vector protocol.
...
\subsubsection{The point to point protocol}
	...
	PPP utilise le bit stuffing ou le character stuffing selon l'environnement où le protocole est utilisé. Les champs Address et Control sont présentes pour des raisons de compatibilité. Le champ Protocol (16 bits) contient l'ID du type de protocole utilisé pour le transport de la trame PPP. 
...
\subsubsection{Ethernet}
   ...
   \end{center}
	...
	\end{center}
      ...
      \begin{figure}[!ht]
	\begin{center}
	    \includegraphics{switch_mac_address_table.png}
