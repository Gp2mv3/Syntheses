...
L'inter-area routing est fait en échangeant des distance vector protocol. Pour prévenir les routeur d'une
area le cout pour atteindre un routeur d'un autre domain. 
\paragraph{LAN}
Quand les routeurs boot dans un LAN, ils élisent un routeurs (Designated Routeur). Pour ne pas devoir 
échanger des HELLO packet entre tout les routeurs. Les routeurs peuvent seulement échanger des HELLO
avec le DR. C'est lui qui représente le LAN.

\paragraph{Shortest path}
Les protocoles interdomaines choississent les plus court chemin. Il peut arriver qu'il y ait deux
chemins qui aient le même coût. Dans ce cas deux approches:
\begin{itemize}
	\item On choisit toujours le même chemin et n'utilise pas les autres.
	\item On utilise les différents chemin:
	\begin{itemize}
		\item On peut faire un Round Robin mais cela causerait d'utiliser de chemins différent pour les 			
		paquets d'un même flow.
		\item On calcule une hash value à partir du tuple de paquet (SRC,DEST,PortSRC,PortDest) et le Next 
		header. La même interface (chemin) sera utiliser pour les paquets du même flow.
	\end{itemize}}
\end{itemize}
