...
\begin{description}
  \item[port-address table] Si on a un \textbf{tree-shapped network} il suffit d'inspecter les packets
    reçu pour créer sa table de forwarding sans risquer d'avoir de loop.
    The problem with a tree-shapped network is that if a link fails, the network is split in two since there is no redundancy.
    If the network is not a tree, we can also use port-address table but we need to use a distributed algorithm to ensure
