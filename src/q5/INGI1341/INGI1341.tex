...
The transport layer send \textbf{semgment} as PDU.
...
There is three different service on the transport layer : connectionless service,
connection-oriented service en request-response service.

\paragraph{Note: } All transport layer service on a connectionless network layer service.
...
\begin{itemize}
    \item \textbf{Reliability} : Elle n'est garantie que lorsque la connection est terminé avec un
        ``gracefully``, sinon des pertes peuvent être observé
\end{itemize}

\paragraph{\textbf{Connection}}

\subparagraph{Refus:} Soit de la part du destinataire, soit du provider.
