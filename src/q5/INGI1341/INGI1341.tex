...
\begin{itemize}
	...
	\item If there are many concurrent client on a server, we can add server to reduce load. 
...
\begin{itemize}
    ...
    \item \textbf{Using popular bittorent service} : Split the file into different block
        and client need all block to have file. There is a \textbf{metadata} file that
...
\item[-] \textbf{Deterministic Medium Acces Control (DMAC) } : TODO page 94
\end{itemize}
...
we know that if a frame of length $2\tau$ has a collision, the sending host \textbf{will sense it} (as any other hosts by the way).
We therefore enforce $2\tau$ as the minimum frame size.

$\to$ Since Ethernet has not many transmission errors, CSMA/CD does \textbf{not uses acknowledgment} to avoid the collisions they would cause.
...
\begin{description}
    ...
    \item[DIFS] : time where a device must wait until the channel has been idle \textbf{after theprevious frame was received correctly}
    \item[EIFS] : time where a device must wait until the channel has been idle \textbf{after theprevious frame was corrupted}
...
and are identified by a port number and a network adress (\textit{IPv4 or IPv6})
