...
\begin{solution}
    ...
    \begin{align*}
        \frac{\mathrm{D}\:}{\mathrm{D}t}\int_{\Omega(t)}\phi\: \mathrm{d}v &=\int_{\Omega(t)}\left(\frac{\mathrm{D}\phi}{\mathrm{D}t}+\phi \nabla\cdot\mathbf{v}\right) \: \mathrm{d}v\\
        &=\int_{\Omega(t)}\left(\frac{\partial\phi}{\partial t}+ \mathbf{v}\cdot \nabla\phi + \phi\nabla\cdot\mathbf{v}\right) \: \mathrm{d}v\\
        &=\int_{\Omega(t)}\left(\frac{\partial\phi}{\partial t}+\nabla \cdot (\phi \mathbf{v}) \right) \: \mathrm{d}v\\
        &=\int_{\Omega(t)}\frac{\partial\phi}{\partial t} \: \mathrm{d}v + \oint_{\partial\Omega(t)}\phi(\mathbf{v}\cdot \unit{n})\:\mathrm{d}s
    \end{align*}
    
    \item 
        \begin{enumerate}
            \item S'il n'est pas libre de se déformer, c'est que les déformations infinitésimales sont nulles. (Pour rappel, $\epsilon_{ii} \triangleq \frac{||\mathrm{d}x^{(i)}||-||\mathrm{d}X^{(i)}||}{||\mathrm{d}X^{(i)}||}$ et $\epsilon_{ij}\triangleq \frac{1}{2}\left( \frac{\partial u_{i}}{\partial x_{j}}+\frac{\partial u_{j}}{\partial x_{i}}\right)$). On impose donc que $\epsilon_{ij} = 0$. Il reste uniquement le terme de dilatation thermique. On obtient:
        \begin{align*}
        \sigma_{ij} &=2\mu\epsilon_{ij}+\lambda \epsilon_{ij}\delta_{ij}-(3\lambda+2\mu)\alpha (T-T_0)\delta_{ij}\\
    ...
    \item Il faut respecter le principe d'admissibilité thermodynamique. On doit ainsi respecter l'ingéalité de Clausius-Duhem que l'on peut simplifier: $$ 0 \geq \frac{1}{T} \mathbf{q}\cdot \nabla T $$
    ...
    Pour un fluide, on a:
    \begin{align*}
        e &= \int c_v \: \mathrm{d}T \longrightarrow \frac{De}{Dt} = c_v\frac{DT}{Dt}\\
        S &= \int \frac{c_v}{T} \:\mathrm{d}T \longrightarrow \frac{DS}{Dt} = \frac{c_v}{T}\frac{DT}{Dt}\\
...
\begin{enumerate}
    ...
    $$\frac{v_r}{r} + \frac{1}{r} \frac{\partial v_r}{\partial r} = 0\quad \Longrightarrow \quad v_r(r)=\frac{A}{r} \quad ,A\in \Re$$
