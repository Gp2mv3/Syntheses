...
\begin{table}[H]
	...
	\begin{tabular}{ccc}
		...
		\multirow{5}{*}{Symboles logiques} & Quantificateurs & $\exists$ et $\forall$ \\
		 ...
		 & & $\true$ et $\false$ \\
		\multirow{3}{*}{Symbole non logiques} & Symboles de prédicat & $P$, $Q$, $R$,\ldots (avec arité $\ge 0$) \\
...
\begin{mydef}[Modèle en logique des prédicats]
	...
	Formellement,
	si on a un ensemble de formules $B = \{p_1, \ldots, p_n\}$,
	une interprétation $I$ pour $B$ est modèle si et seulement si
	\[
	\VAL{I}(p_i) = \true\,, \quad \forall p_i \in B\,.
	\]
\end{mydef}
...
par rapport aux preuves formelles en logique propositionnelle.
\begin{itemize}
