...
\begin{description}
    ...
    our forwarding table, without risking making a loop.
     \begin{enumerate}
          ...
          from wich the packet was received. It's called \textbf{broadcasting}
          (\textit{drawback}).
     \end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Authority} wich is the IP address or DNS server where the
    file is located.
...
un \textbf{delayed acknowledgment strategy} qui assure d'envoyer un ack a chaque fin
...
\paragraph{LAN} Dans une LAN Chaque host est identifié par une addresse datalink layer
(\textbf{MAC addresse}).

LAN supporte broadcast and multicast datalink layer addresses. Une frame envoyé a l'addresse
...
IPv4 ne pourra bientot plus supporter toute les addresses, d'ou le besoin
...
\paragraph{ }
Une allocation \textbf{hierarchique} des allocations d'addresse permet de minimiser
le nombre de route connu par les router. Celui ci connait donc la route pour
certains block d'addresse!
(\textit{$2^{128}$ groupé dans $2^{64}$ subnet!})

Deux type d'allocation d'addresse :
\begin{itemize}
    \item \textit{provider independent} (PI) : Pour compagnies connecté a au moins deux ISP.
        Ca correspond à recevoir un \textbf{block d'addresse} indépendant du ISP.
    \item \textit{provider aggregatable} (PA) : Est dépendante du provider.
\end{itemize}

Le désavantage des \textbf{PA} addresses est que lorsque tu utilises un block d'addresse PA
et que le provider change, il change aussi toute les addresses utilisés.
...
l'addresse est celle qui est employé.

Note : ::/0 match avec tout le monde et est donc la \textit{default route}.

\paragraph{\textbf{Unique Local Unicast}}
L'addresse ULA est un fc00::/7 et est similaire à une IPv4 addresse.
L'addresse n'est pas garantie unique et un router ne forwarde pas
une ULA.
...
\begin{itemize}
    \item Messages d'erreurs
        \begin{enumerate}
            \item Destination unreachable (0: no route, 1: firewall refuse, 2: sender use link-local addresse to reach global unicast addresss, 3: address unreachable, 4: port unreachable)
            \item Packet too big
            \item Time exceeded
...
Chaque interface sur un device est identifié par une MAC addresse unique. \footnote{MAC address block are allocated to manufacturers}.

Grace à cette unicité, deux hosts connecté sur une LAN a donc une addresse unique!
...
packet lui est destiné ou non (\textit{sur base d'une addresse logique}).

Potentiellement, tout les node IPv6 sont capable de captuer des frames destinés à different
autre multicast addresse.

\subsubsection{Interactions between IPv6 and datalink layer}

\paragraph{Dans une LAN  sans internet}, les hosts prennent une  addresse IP grace au
\textbf{link-local addresse} : on prend fe80::/64 avec la MAC addresse.

\paragraph{Si  la  LAN  est  connecté à  internet},  on  utilise  \textbf{Neighbor
Discovery  Protocol} (partie  de ICMPv6)  pour connaître  l'addresse des
autres

\paragraph{Neighbor Discovery Protocol (NDP)}
Attention les addresses IPv6 ont été configuré \textbf{manuellement}.
\begin{enumerate}
    \item Envoi un multicast \textbf{ICMPv6 Neighbor Solicitation} (NS) avec l'addresse IPv6 recherché
    \item Le receiver répond avec un \textbf{ICMPv6 Neighbor Advertisement} avec son addresse IPv6 et MAC
...
De plus, le ICMPv6 NA est stocké temporairement dans la cache et quand il expire il faut revalider l'addresse.

\paragraph{Duplicate Address Detection algorithm} permet de ne pas avoir deux addresses IPv6
identiques.
Pour cela il envoit un \textbf{ICMPv6 NS} en unicast à sonn addresse, et si il ne reçoit pas de réponse elle est donc unique.

\paragraph{\textbf{Automatically configure IPv6 address}}
There is two protocol : SLAAC and DHCPv6.

\subparagraph{Stateless Address Auto-Configuration (SLAAC) }
\begin{enumerate}
    \item Un device crée sa \textbf{link-local addresse} avec fe80::/64 et 64bits
    ...
    \item Pour connaître le \textit{prefix IPv6 subnet}, le router envoi
        ...
        \end{itemize}
    \item IPv6 addresse = Prefix + 64 bits d'identification
    \item Envoi un \textbf{ICMPv6 NS} a son addresse IPv6 pour s'assurer de l'unicité
\end{enumerate}
