...
\begin{description}
    ...
    \item[\difs{}]: \textit{DCF Interframe Space}. If a station detects the medium has been continuously idle for a duration of \difs{} microseconds,
    ...
    Note that the last transmission need also be succesful
    (that is, no corruption),
    as the station would need to wait for a duration of \eifs{} until it can again transmit any frame
    (whether it be the old one or a new one).
...
We add a DLE in front of each DLE contained in the data of the frame.
...
(it has to discard packets to old packets
and have maximal throughput because of that).
...
\begin{itemize}
        ...
        \tiem a multiplexing technique that permits differenciating applications on the host.
\end{itemize}
...
It's possible to implement the prefence of a cheap link rather than another via
...
\paragraph{ }
BGP convergence isn't always guaranteed and verifiant global convergence is
...
special characters:
\begin{itemize}
    \item[-] \textsc{END}: ath the beginning and end of a packet.
...
which \textbf{TCP/IP header compression} techniques were indispensable.
(\textit{Headers already took a lot of time.})

    $\to$ Consecutive segment redundance is exploited to compress.
...
\begin{enumerate}
    ...
    \item the \textit{Link Control Protocol}: used to negociate options and to
    ...
    of the network layer, it allows negociating the protocol-specific options
    (\textit{IPv4's NCP negociates the IPv4 address}).
\end{enumerate}
...
negociate a maximum packet size.
...
to the Internet is done via the Extensible Authentification Protocol (EAP)
...
developpped specifications for PPP frame transport on other networks than
point-to-point.}
\end{center}
...
\begin{itemize}
    ...
    \item \textbf{Frame format}:
        ...
        The beginning of an Ethernet frame begins with a premable used by the
        physical layer to synchronise clocks.
    \item \textbf{Adresses}:
        \begin{itemize}
...
\begin{figure}[!ht]
    ...
    \caption{A hierachical Ethernet network made of hubs.}
    \end{center}
\end{figure}
...
\begin{itemize}
    ...
    \SI{10}{Mbps} links).
        ...
        $\to$ to preserve CSMA/CD: slot time = \SI{5.12}{\micro\second}.
\end{itemize}

\paragraph{\textbf{Ethernet switches}}
Another solution to get better performances in LAN ethernets is by making hubs
intelligent.
...
\begin{table}[ht]
    ...
    \end{tabular}
    \end{center}
    \caption{BPDU transmission in fucntion of port state.}
\end{table}
...
\begin{itemize}
    \item Bridges listen in on the blocked port, but if the network topology is
...
\begin{itemize}
    ...
    maximum age is reached, a failure has occured and the STP has to be
    recomputed.
\end{itemize}
...
exchanged frame's header. This header is inserted directly after the MAC
...
The radio spectrum is a limited resource shared by all. It's reglemented by
...
connectionless services like Ethernet. The acknowledgments are used to minimise
...
\section{Algorithms}
   \subsection{MAC Address Learning by bridges}
	  \lstset{keepspaces=true, keywordstyle=\color{red!70},
    ...
    \subsection{Additive increase/multiplicative decrease in TCP}
	  \lstset{keepspaces=true, keywordstyle=\color{red!70},
       commentstyle=\color{blue!60}}
	  \begin{center}
	  \framebox{\begin{minipage}{0.9\linewidth}
