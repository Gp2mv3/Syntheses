...
\section{Caractériser un ensemble de mesures}
L'ensemble de données qui nous intéresse est la
\textit{population}. Le sous-ensemble sélectionné
pour étudier cette population est l'\textit{échantillon}.

\begin{mydef}
La \textit{moyenne} d'un échantillon de $n$ mesures
$y_1, y_2, \dots, y_n$ est donnée par

\[ \overline{y} = \frac{1}{n} \sum_{i=1}^n y_i.\]

La moyenne de la population correspondante est notée $\mu$.
\end{mydef}

\begin{mydef}[Quantile]
Pour une valeur $p \in (0,1)$, $q_p$ est une valeur telle 
qu'une proportion $p$ de l'échantillon est inférieure à
elle et une proportion $1-p$ est supérieure à elle.

On appelle la \textit{médiane} le quantile $q_{0.5}$ 
\end{mydef}

\begin{mydef}
La \textit{variance} d'un échantillon de $n$ mesures
$y_1, y_2, \dots, y_n$ est donnée par

\[ s^2 = \frac{1}{n-1} \sum_{i=1}^n (y_i - \overline{y})^2.\]

La variance de la population correspondante est notée $\sigma^2$.
\end{mydef}

\begin{mydef}
L'\textit{écart-type} d'un échantillon de mesures
$y_1, y_2, \dots, y_n$ est donné par

\[  s = \sqrt{s^2}. \]

L'écart type de la population correspondante est noté $\sigma$.
\end{mydef}

\begin{myrem}
Pour une distribution des mesures approximativement normale,
on a 
\begin{itemize}
	\item $\mu \pm \sigma$ contient 68\% des mesures ;
	\item $\mu \pm 2\sigma$ contient 95\% des mesures ;
	\item $\mu \pm 3\sigma$ contient presque toutes les mesures.
\end{itemize}
\end{myrem}

\begin{mydef}
Le coefficient de variation d'un échantillon est donné
par
\[ \text{cv} \eqdef \frac{s}{\overline{x}}. \]
Le coefficient de variation est indépendant des unités
dans lesquels le mesures ont été effectuées. Pour
comparer des données avec des différentes unités, il
faut donc utiliser le coefficient de variation et non
l'écart-type.
\end{mydef}

\begin{mydef}
Le \textit{range} est simplement la valeur maximale $-$
la valeur minimale parmis les mesures.
\end{mydef}

\begin{mydef}
L'\textit{écart interquartile} est une mesure de dispersion
donnée par

\[ IQR = q_{0.75} - q_{0.25}. \]

Une donnée est considérée comme aberrante (\textit{outliers}) si
elle se trouve en-dessous de $q_{0.25}-1.5\times IQR$ ou
au-dessus de $q_{0.75}+1.5\times IQR$.
\end{mydef}
