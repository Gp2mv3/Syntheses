...
\item $E(X^2)=\int_0^2 y^2f_Y(y) \dif y=\int_0^2 \dfrac{3}{2}y^3-\dfrac{3}{4}y^4 \dif y=\left[ \dfrac{3}{2}\dfrac{y^4}{4}-\dfrac{3}{4}\dfrac{y^5}{5}\right]_0^2=6-\dfrac{24}{5}=\dfrac{6}{5}$
...
\begin{align*}
F_{U_{(n)}}(u)&=\Pr(U_{(n)}\leq u)=\Pr(U_{(1)}\leq u)...\Pr(U_{(n)}\leq u)\\
&= [F_X(u)]^{n_1}[F_Y(u)]^{n_2}\\
...
\begin{enumerate}
    ...
    \end{align*}
    \item Soit $X \sim Bi(200,0.08)$ avec $p=0.08$ et $n=200$.    On cherche $\Pr(X<20)$. Comme le calcul est beaucoup plus long, on va l'approximer par le \textit{Central Limit Theorem} qui nous dit:
    ...
    $$\Pr(X=x)=\frac{C^{x}_{r}\cdot C^{n-x}_{N-r}}{C^n_N} \textrm{ pour } x =0,1,...,min(n,r)$$
    \begin{align*}
