...
{Simon Demaré\and Cynthia Laureys}
...
\begin{enumerate}
\item \begin{enumerate}
\item On sait que $X, Y \simind \gammad(\alpha_i,\beta)$ avec $i$ valant respectivement 1 et 2. En outre, on connait $\textbf{U}=g(\textbf{X})=\begin{cases} U=X+Y \\V=\dfrac{X}{X+Y}\end{cases}$. Pour déterminer la distribution de $\textbf{U}=(U,V)$, il s'agit donc ici d'appliquer une \textit{One-to-one transformation} définie comme suit:
...
\begin{align*}
	...
	&= \dfrac{x^{\alpha_1-1}e^{-x/\beta}}{\beta^{\alpha_1} \gammad(\alpha_1)}\dfrac{y^{\alpha_2-1}e^{-y/\beta}}{\beta^{\alpha_2} \gammad(\alpha_2)} && \textrm{ pour } x,y \geq 0\\
	&= \dfrac{x^{\alpha_1-1}y^{\alpha_2-1}e^{-(x+y)/\beta}}{\beta^{\alpha_1}\beta^{\alpha_2} \gammad(\alpha_1)\gammad(\alpha_2)}&& \textrm{ pour } x,y \geq 0\\
\end{align*}
...
\begin{align*}
    ...
    &=\left. (x+y) \dfrac{x^{\alpha_1-1}y^{\alpha_2-1}e^{-(x+y)/\beta}}{\beta^{\alpha_1}\beta^{\alpha_2} \gammad(\alpha_1)\gammad(\alpha_2)} \right|_{\textbf{x}=g^{-1}(\textbf{u})} && \textrm { pour } u,v \geq 0\\
    &= u \dfrac{(uv)^{\alpha_1-1}(u-uv)^{\alpha_2-1}e^{-u/\beta}}{\beta^{\alpha_1}\beta^{\alpha_2} \gammad(\alpha_1)\gammad(\alpha_2)} && \textrm { pour } u,v \geq 0\\\end{align*}
...
$$U \Perp V \Leftrightarrow f(u,v)=f_U(u)f_V(v)$$ où $f(u,v)=\dfrac{1}{\beta^{\alpha_1}\beta^{\alpha_2} \gammad(\alpha_1)\gammad(\alpha_2)} \left[u^{\alpha_1-\alpha_2-1}e^{-u/\beta}\right]\left[v^{\alpha_1-1}(1-v)^{\alpha_2-1} \right]$
...
\begin{solution}
  \begin{enumerate}
    ...
    \item
      ...
      Il est à savoir que pour une distribution $Z$ quelconque, $\bar{Z}$ est un estimateur de de $\mu_Z$
      avec
    ...
    \item
      On suggère $\bar{Y}$ comme test statistique.

      Comme $\beta = \mu_Y$, on sait que
      \[ \sqrt{n}\frac{\bar{Y}-\beta}{\beta} = \sqrt{n}\frac{\bar{Y}-\mu_Y}{\sigma_Y} \sim \N(0,1). \]
      La $p$-value vaut
      \begin{align*}
        \Pr(\bar{Y} \geq \bar{y})
        & = \Pr(\sqrt{n}\frac{\bar{Y}-\beta}{\beta} \geq \sqrt{n}\frac{\bar{y}-\beta}{\beta})
      \end{align*}
      qui se calcule à l'aide de la valeur $\sqrt{n}\frac{\bar{y}-\beta}{\beta}$ et des tables.
    \item
      On a
      \begin{align*}
        0.95
    ...
    \item
      ...
      Par le CLT, asymptotiquement on a $T \sim \N(\beta,\beta^2/n)$.
      Dès lors, comme la transformation $t \mapsto \sqrt[\alpha]{-\ln(1-\pi)t}$ est strictement monotonique,
      \begin{align*}
        f_{U}(u)
        & = \left.\frac{f_T(t)}{\frac{1}{\alpha}(-\ln(1-\pi)t)^{1/\alpha-1}}\right|_{t = -u^\alpha/\ln(1-\pi)}\\
        & = \left.\frac{\frac{\sqrt{n}}{\beta\sqrt{2\pi}}\exp\left[-\left(\frac{n}{2\sigma^2}(t-\beta)^2\right)\right]}{\frac{1}{\alpha}(-\ln(1-\pi)t)^{1/\alpha-1}}\right|_{t = -u^\alpha/\ln(1-\pi)}\\
        & = \frac{\alpha\sqrt{n}}{\beta\sqrt{2\pi}}u^{\alpha-1}\exp\left[-\left(\frac{n}{2\sigma^2}(-u^\alpha/\ln(1-\pi)-\beta)^2\right)\right].
      \end{align*}
  \end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
    \item Soit $X \sim \bin(12,0.8)$ avec $p=0.08$ et $n=12$. On sait que
    ...
    \begin{align*}
    \Pr(X< 20)&\approx P\left(\frac{X-np}{\sqrt{np(1-p)}} < \frac{20.5-np}{\sqrt{np(1-p)}} \right)\\
    &= P\left(\frac{X-16}{\sqrt{16\cdot 0.92}} < \frac{20.5-16}{\sqrt{16 \cdot 0.92}} \right)\\
    &= 1- P\left(Z \geq 1.17 \right)\\
    &=0.879
    \end{align*}
