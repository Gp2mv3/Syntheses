...
\item $E(Y|X=1/2)=\int_1/2^2 y f(y|x)dy = \int_1/2^2 y \frac{f(x,y)}{f_X(x)}dy = 12$ min $30$ où $f_X(x) = \int_x^2 f(x,y)dy$
...
On rejette $H_{0}$
La p-valeur est la valeur $p=P(T>t=2.23)$ où $T \sim t_{n_a+n_b-2}$. Cette valeur n'est pas dans les tables néanmoins l'on trouve $P(T>2.101)=0.025$ donc on peut conclure que $p>0.025$ (et $p<0.05$).
