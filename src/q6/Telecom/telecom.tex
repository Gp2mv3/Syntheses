...
\fancyhead[R]{Synthèse}
\pagestyle{fancy}

\title{LELEC1930\\Introduction aux télécommunications\\Synthèse}
\date{}
\author{Syméon Malengreau\ \ \ Nicolas Houtain\ \ \ Nicolas Kabasele\\https://bitbucket.org/nhoutain/telecom}
...
Deux types de canaux de communications: \textit{filaires} (ligne téléphonique, cable coaxial, ...) et \textit{sans fils}. Trois éléments caractérisent ces canaux:
\begin{description}
...
\item[La Distortion] est l'ensemble des modifications indésirables d'un signal~\cite{DistWiki}.
\end{description}

Il existe deux moyen de représenter les informations par des signaux:
\begin{itemize}
...
\begin{description} 
  \item [Note]
Les différents systèmes de télécommunications utilisent des bandes de fréquence bien précise pour chaque tâche.
\end{description}

La modulation permet permet de transposer un signal autour d'une fréquence définie. 
...
\begin{description}
\item[Le Multiplexage] permet de faire passer plusieurs informations sur un seul support de fréquence. Les différents symboles sont combiné grace à un multiplexeur. 
...
Avec la radio les sons gauche et droit sont envoyé ensemble sous un signal $S_1 = G+D$ de façon à ce que les radios mono puissent le recevoir et un autre signal est envoyé pour le stereo $S_2 = G-D$. Le signal stereo peut être recréé par l'opération suivante:
\begin{eqnarray*}
G &=& S_1 + S_2\\
D &=& S_1 - S_2
\end{eqnarray*}
...
Pour la télévision le procédé est assez différent, la luminance (niveau de gris) des lignes sont envoyé avec une pulsation de synchronisation (\textbf{synchronizing pulse}) entre chaque ligne.

Aussi pour rendre les images fluides pour la vue humaine on entrelace les lignes, c'est-à-dire que l'on envoi d'abord la moitié des lignes (une ligne sur deux), et puis on envoi les lignes qui reste (\textbf{1 image = 2 trames}).

Pour l'envoi des couleurs, l'idée est la même qu'avec la radio, on va garder l'envoi de la luminance pour que les vieilles télévision puisse toujours lire l'image et l'on va rajouter d'autre signaux pour pouvoir avoir la couleur. 
Soient $RGB$ les couleurs (red, green, blue) et $YUV$ les signaux (luminance et chrominance).
\begin{eqnarray*}
Y &=& 0.3R + 0.59G + 0.11B\\
U &=& 0.493 (B-Y)\\
V &=& 0.877 (R-Y)
\end{eqnarray*}
De nos jours la télévision numérique a remplacé la télévision analogique et les images sont désormais envoyé en contenu binaire.

\subsection{Numérisation}

La \textbf{numérisation} va permettre de passer d'un signal analogique à un signal numérique. On va pour cela \textbf{échantilonner} le signal (capturer des valeur à interval fixe). Les avantages des signaux numériques sont:
\begin{itemize}
...
\end{itemize}
Il faut faire tres attention lors de l'échantillonage des erreurs possible, en effet choisir un interval de temps trop large pour capturer les valeurs peut causer des erreurs. (On interpretera un signal d'une manière différente).
...
Trois éléments caractérisent une ligne: l'\textit{atténuation}, la \textit{distortion} et le \textit{bruit} (voir section 1). Tout ces éléments dépendent de la géométrie, des matériaux et des conditions dans lesquels ces lignes opèrent. Il y a de nombreux types de lignes différents:
\begin{itemize}
...
On représente habituellement les lignes comme une résistance $R'$, une conductance $G'$, une inductance $L'$ et une capacité $C'$. Le morceaux de ligne suivant se répète tout au long de la ligne:
\begin{figure}[H]
\centering
...
Ces paramètres varient suivant le type de ligne, fréquence et matériel utilisé. L'équation des télégraphiste nous donne la relation suivante (avec $z$ la position, $t$ le temps et $v$ la tension sur la ligne):
$$\frac{\partial^2 v}{\partial z^2} = LC \frac{\partial^2 v}{\partial t^2} + (RC + LG)\frac{\partial v}{\partial t}+RGv$$

Dans le cas d'un contexte où les pertes serait négligeables ($R=G=0$) nous obtiendrons la formule suivante.
$$\frac{\partial^2v}{\partial z^2}=LC\frac{\partial^2v}{\partial t^2}$$

Qui fait le liens entre une variation de tension en fonction de la position et une variation de la tension en fonction du temps. Posons $c = \frac{1}{\sqrt{LC}}$ et nous avons la tension:
$$v = f(z+ct) + g(z-ct)$$
...
\begin{description}
  \item[L'effet pélliculaire] (ou l'effet de peau) est un effet electromagnétique qui repousse les lignes de courant vers la surface du conducteur. Les électrons vont s'amasser sur les bords. ~\cite{B31}
\end{description}

Cet effet de peau va en fait diminuer la surface de la ligne qui est effectivement parcouru par du courrant et donc augmenter la résistance de celui-ci comme $\sqrt{f}$. Il est causé par la création d'un champs magnétique.
$$\nearrow frequence \Rightarrow \nearrow effet\ de\ peau \Rightarrow \nearrow resistance
\Rightarrow  \nearrow pertes$$

\subsection{Atténuation}
\begin{description}
  \item[L'atténuation d'un signal] d'une ligne peut-être calculer comme etant $10 log(P_1/P_2)$, avec $P_1$ la puissance d'entrée et $P_2$ la puissance de sortie. Cette atténuation se mesure en \textbf{decibel} ($dB$). Il s'agit d'un rapport de puissance. Si on l'exprime en tension $20 log(V_1/V_2)$.
\end{description}

\subsection{Dispersion}

Soient le nombres de pulsation $\omega = 2\pi f$ ,le vecteur d'onde $\beta = \frac{2\pi}{\lambda}$ et $\lambda$ la longueur d'onde, on a la \textbf{vitesse de phase}:
...
\end{eqnarray*}
On a donc $\alpha$ et $\beta$ qui sont proportionelle à $\sqrt{\omega}$ et l'impédance caractéristique $Z_c$ est complexe et proportionnel à $\frac{1}{\sqrt{\omega}}$. Il s'agit donc d'une mauvaise situation dans laquelle nous avons de la \textbf{distorsion}.
...
On a le droit lors du passage de la ligne 5 à la ligne 6 de mettre la racine au carré car on ne s'interesse qu'au relation entre les variables. (\textit{Je comprends pas pourquoi à la ligne 6 un 2 apparait})
...
\textbf{L'impédance caractéristiques} en annulant $G$ et $R$
\begin{eqnarray*}
Z_c &=& \sqrt{\frac{R + j\omega L}{G + j\omega C}}\\
&\simeq& \sqrt{\frac{L}{C}}
\end{eqnarray*}

Nous avons donc $alpha$ qui est indépendante de $\omega$ et $\beta$ proportionnel à $\omega$. A cela s'ajoute $Z_c$ qui est réel et indépendant de $\omega$.

\subsection{Pupinisation}

Pour le cas de lignes tel que $\omega L << R$, nous allons procéder à une \textbf{pupinisation}. 
Cela consiste à inserer des inductance le long de la ligne afin d'augmenter artificiellement $L$.
Grace à cela nous allons avoir une réduction de l'atténuation le long de la ligne.
...
\begin{itemize}
	...
	\item[-] Rayonnement important (sensibilité aux interférences et problème de confidentialité)
\end{itemize}

\subsection{Câble coaxial}

Un câble coaxial est constitué d'une \textbf{partie centrale} (\textit{fil de cuivre}) enveloppé dans un \textbf{isolant}, puis d'un \textbf{blindage métallique} tressé et enfin d'une \textbf{gaine extérieure}~\cite{Coax}. Il est utilisé comme cable pour la transmission de la télé distribution.
...
On va faire varier l'indice de réfraction plus l'on s'approchera des parois afin que les différents faisceaux lumineux convergent vers le centre de la fibre. La dispersion intermodale est ainsi réduire.
...
Les ondes se propagent et interagissent avec leur environnement. Cet interaction dépend de la \textit{fréquence}, de la \textit{taille des objets} en question et de la \textit{surface} des objets. Il existe différents types de propagation:
\begin{itemize}
	\item Propagation par onde directe
	\item Propagation par onde de sol
	\item Propagation par reflexion/refraction sur l'ionosphere
...
Les ondes directe sont envoyé en \textbf{ligne droite} d'une antenne à une autre. Elles ont une porté relativement limitée et il y a des interférence avec l'onde réfléchie sur la terre. (\textit{En effet entre deux antennes il existe deux chemins : l'un direct et l'autre avec une réflection de l'onde sur la terre}).

\subsection{Onde de sol}

Les ondes de sol utilise des ondes basse fréquence qui ont tendance à se propager le long du sol,\textit{ selon une propriété de celle ci}. En effet, les fronts d'ondes des ondes basses fréquences se déplace perpendiculairement au sol.
\begin{figure}[H]
\centering
\begin{tabular}{ll}
\hline
...
Sous l'effet de l'\textbf{ionisation} de l'air par les UV solaire, la densité en electrons des couches de l'ionosphère les plus éloigné augmente. Ainsi ce "mur" d'electron cause la réfraction, voir même la réflexion des signaux. On reconnait \textbf{4 couches précise} (càd 4 endroits ou la densité augmente de facon particulierement rapide). De la plus proche à la plus éloigné de la terre, la couche $D$, $E$, $F_1$ et $F_2$ (ces deux dernière devenant une seule pendant la nuit, la couche $F$).

Grace à cette propagation, on peut faire "rebondir" des ondes sur ces couches et propager une onde entre les continents (au-delà de l'horizon)\cite{Ion}. On utilise masivement cette technique pour les ondes de hautes fréquence.

\begin{description}
  \item[MUF] est la Maximal Usable Frequency, càd la fréquence maximun tel que l'on est sur
...
Une antenne est un dispositif permettant de rayonner (émetteur) ou, de capter (récepteur), les ondes électromagnétiques\cite{WikiAntenne}. Nous trouvons autour d'une antenne une série de champs magnétique et de champs électrique.

\begin{description}
\item[L' onde TEM] (appellé mode Transverse Electrique-Magnétique) est un mode de propagation tel que les champs électrique et magnétique sont tous deux orthogonaux à la direction de propagation\cite{WikiTEM}.
\item[L'antenne isotrope] est une antenne dont le diagramme de rayonnement est un cercle.
\end{description}

L'antenne envoie un rayonnement dans une certaine direction, mais provoque aussi un rayonnement inverse non désiré. Ce rayonnement a un angle d'ouverture $\theta$. On peut mesurer le gain de l'antenne de la manière suivante:
$$Gain = \frac{puissance\ dans\ direction\ de\ puissance\ maximum}{puissance\ dans\ cette\ direction\ si\ l'antenne\ etait\ isotrope}$$

\begin{description}
  \item[L'angle d'ouverture] d'une antenne est l'ange de direction pour lequel la
    puissance rayonnée est la moitié de la puissance rayonnée dans la direction la plus
    favorable.
\end{description}
\begin{center}
\includegraphics[width=5cm]{img/angle.png}
\end{center}
...
\begin{center}
\includegraphics[width=8cm]{img/dipole.png}
\end{center}
...
\begin{figure}[H]
  ...
  \caption {Principe de démodulation selon fourrier}
\end{figure}

La fréquence de l'oscillateur local du démodulateur doit être la même que celle du modulateur. Il y a donc interret d'envoyer la porteuse ou un signal qui permet de la retrouver.
...
Comme nous avons vu au dessus, il y a des fréquences "négatives" qui sont envoyées, ainsi il n'est pas nécessaire d'envoyer deux fois l'informations, l'envoi de deux bandes latérales suffit (on reconstruit la 2e bande à la démodulation. Il y a donc pour ca deux moyen d'envoi pour savoir quels bandes envoyer:
\begin{itemize}
  \item USB: Upper side band (\textit{Envoi les deux aux extrémités})
  \item LSB: Lower side band (\textit{Envoie les deux aux centres})
\end{itemize}

\subsubsection{Bande latérale unique (QAM)}
On utilise l'envoi en double bande lattéral dans les cas général mais il existe une autre technique, l'envoi en bande latéral unique par modulation en quadrature (QAM). Pour cette modulation en quadrature il faut convertir le signal $m(t)$ en deux signaux $s_1(t)$ et $s_2(t)$. Cette technique à l'avantage de diviser la bande passante nécessaire par deux (c'est utilisé pour la transmission de signaux télévisé).
...
$$m = \frac{f_d}{f_m}$$
Cet indice de modulation va nous donner la valeur maximale de la dévition de fréquence\cite{rennes}. La largeur de la bande passante utilisé par le signal va varier en fonction de cet indice.\\

Exemple: \textit{la fréquence maximal d'un signal hi-fi modulant est de $15 kHz$ et les radios utilisent deux types de bandes, les Wide Band FM qui ont une excursion maximale de $75 kHz$ et donc un indice de modulation de $m=5$, et les Narrow Band FM qui ont une excursion maximale de $15 kHz$ et donc un indice de modulation $m=1$}.
...
En modulation AM nous avons un bruit constant quel que soit la fréquence, tandis qu'en FM le bruit augmente proportionnelement avec la fréquence. Il est toujours possible d'augmenter la qualité en augmentant l'indice de modulation\cite{tafro} (ce qui a pour conséquence évidemment d'augmenter le spectre du signal envoyé et donc d'encombrer la bande passante). 

Mais une technique utilisé en FM pour la réduction du bruit est la \textbf{préaccentutation} avec la \textbf{désaccentutation}.
\begin{description}
	\item[La préaccentuation] consiste à favoriser les fréquences aigues par rapport au fréquence basses afin de minimiser les défaults.\cite{sonelec}. Le bruit rapporté à chaque fréquence est ainsi constant.

	\item[La désaccentutation] est le procédé inverse de la préaccentuation, qui consiste à redonner à un signal audio préaccentué, son contenu fréquentiel d'origin.\cite{sonelec}.
\end{description}

\subsection{Modulation de phase(PM)}

La modulation de phase est l'équivalent de FM avec la dérivé de $s(t)$ à la place de $s(t)$. En effet si l'on reprends le calculs de tantôt, l'on obtiens:
\begin{eqnarray*}
...
La modulation numérique reprends les principes vu au dessus, mais les adaptent aux deux caractéristiques des signaux numériques: ils utilisent des valeurs discretent (0 ou 1) et c'est à chaque coup d'horloge qu'est envoyé une valeur. L'envoi de signal numérique permet grace à des codes correcteurs d'erreur la \textbf{régénération}. 
...
Dans l'emission stereo radio en FM, on envoi dans l'intervalle $0-15 kHz$ la somme des signaux $G+D$. A la fréquence de $19kHz$ un \textbf{signal pilote} indique que l'on utilise autour de la fréquence $38 kHz$ un signal stéréo. Le signal stéréo $G-D$ est alors envoyé en LSB en modulation AM autour de la porteuse $38kHz$.
...
\item \textbf{FDMA}(Frequency division multiple access): une fréquence par utilisateurs et plusieurs fréquence pour toute la bande.
  \item \textbf{CDMA}(Code division multiple access): Chaque utilisateur à une clé (ou code) à l'aide de lquelle son message est codé (\textit{via un XOR}) avant d'être émis. Il est important de noter que chaque utilisateur émet sur toute la largeur de la bande.
\end{itemize}
...
\paragraph{Note}: Une (n*2) erreur sur la même ligne, elles ne seront pas détectée.
...
On veut trouver des codes avec une disance minimale voulue et comportant le plus de mots possible. Avec un \textbf{mot d'information} 
$$i = (i_1, i_2, i_3, ..., i_k)$$
...
Augmenter la taille du bloc donne généralement de meilleures performance pour un même taux, mais complexité et delai augmentent. 

\subsection{Mecanisme ARQ}
...
\begin{description}
  ...
  \item[NACK] : le message n'a pas biené été reçu
\end{description}
...
\begin{itemize}
  ...
  \item \textbf{Effet-dopler}: causé par le déplacement de l'utilisateur par rapport à l'antenne
\end{itemize}
...
\subsubsection*{Décrivez le rôle des différents blocs du shéma de modulation difitale ci dessous.}
\begin{center}
...
\subsubsection*{Qu'est-ce qu'un algotithme de cryptographique à clé publique? 
Dans quel cadre est-il génralement utilisé? Quels sont ses désavantages?}
