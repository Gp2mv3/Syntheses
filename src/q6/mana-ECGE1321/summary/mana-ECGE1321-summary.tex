...
\subsection{Révolution industrielle}

La révolution industrielle marque l'apparition des usines et donc de la
gestion d'un grand nombre de personnes. Le système de pensée est guidé
par des penseurs comme Taylor (parcellisation maximale des tâches),
Ford (payer les employés pour qu'ils puissent consommer nos produits) ou
encore Weber (bureaucratie consistant à un ensemble de règles et une
structure hiérarchique importante). La gestion est guidée par les
pensées de l'époque, à savoir que l'\textbf{homme est paresseux}, il
n'est guidé que par des \textbf{motivations économiques}. On s'appuyera
sur les thèses \textbf{behavioristes} et la \textbf{loi de l'effet}
(Thorndike) qui consiste à définir le comportement humain comme des
réponses à des stimulis. On peut priver/appliquer un stimulus positif ou
appliquer/priver un stimulus négatif selon qu'on veuille punir ou
récompenser. Le \textbf{groupe est clandestin} car il nuit à la
productivité, l'être humain ne devant être présent que pour travailler.
\newline

Les exemples actuels sont peu nombreux mais nous citerons principalement
les \textbf{call-centers} où les emplois sont très bien définis (aucune
initiative et autonomie), on peut
donc faire un \textbf{monitoring serré de la performance} des employés et
appliquer des systèmes de \textbf{salaire à la pièce}, employé du
mois,\ldots C'est ce qu'on nomme la \textbf{conception instrumentale de
la motivation}. \newline

Le GRH est un \textbf{ingénieur} (spécialiste de la surveillance qui
définit les règles, processus, outils et systèmes) ou un
\textbf{juriste} (spécialiste des règles qui classifie
les rôles et établit les règles). Ulrich le définit comme \textbf{Expert
Administratif}.\newline

\subsection{Crise de 1929}

A la suite de la crise de 1929 et des expériences d'Elton Mayo (Western
Electric), les mentalités changent. On comprend la force que peut avoir
un groupe au sein d'une société, ceci marque à la fois l'\textbf{apparition des
groupes} mais également l'\textbf{apparition des cadres} pour les gérer.
\newline

L'engagement affectif dans le groupe peut être utilisé pour améliorer
les performances de l'entreprise. La structure hiérarchique lourde
commence à faire place à une forme de \textbf{leadership informel} où on tentera
de légitimer le pouvoir. Le GRH prend une place de \textbf{psychologue}.
Une \textbf{culture de soutien} prend place dans les entreprises.
\newline

Cette période voit fleurir de nombreuses théories :

\begin{figure}[H]
    \begin{framed}
    \begin{description}
        \item[Théorie X] L'homme n'aime pas travailler (surveillance et
            sanctions)
        \item[Théorie Y] L'homme aime travailler et avoir des
            responsabilités (autonomie et management participatif)
    \end{description}
\end{framed}
    \caption{Théorie de McGregor}
\end{figure}

\begin{figure}[H]
    \begin{framed}
         Il faut faire correspondre les tâches à des objectifs globaux.
    \end{framed}
    \caption{Management by objectives}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Pyramide_des_besoins_selon_Abraham_Maslow.png}
    \caption{Pyramide des besoins de Maslow}
\end{figure}
...
trouvent les réponses (il les laisse chercher eux-même)
...
une approche novatrice. Tous les employés se rassemblent pour
...
eux-mêmes)
    \item Règles de base de Jean-François Zobrist : 
        \begin{enumerate} 
            \item Confiance aux ouvriers et aux commerciaux (bonheur et
            rentabilité vont de pair)
            \item Amour du client 
        \end{enumerate}
...
l’anarchie. Il y a un système de leaders en place.
...
Doit fonctionner seule et ce peu importe l’endroit, basé sur une
...
structure qui mélange au mieux centralisation et décentralisation
    \item[Zone 4] : système ouvert, approche sociale.
...
\begin{itemize}
    \item L’observation naturelle : On observe des individus ou des groupes
longtemps avant de tirer des conclusions. Ce type d’observation peut
être qualifiée de naturelle par experts (des experts rédigent la théorie
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{ulrich_role_framework.png}
\end{figure}
...
La culture « Innovation » : La formalisation est peu présente car ce
...
contrôle. Mintzberg qui était d’abord contre, a finalement défini 10
...
\begin{description}
    \item[S1] : Contrôle est absolu et on travaille avec la contrainte
        et les ordres stricts. Le manager travaille derrières les
        travailleurs et les surveille constamment.
    \item[S2] : Style « convivial » il va favoriser le côté humain tout
        en gardant le contrôle. Dans ce style on trouve plus
        d’autonomie. 
    \item[S3] : Style « laisser faire ». Le manager est là sans être là
        et joue le rôle de transmetteur. 
    \item[S4] : Style idéal qui est considéré comme guide et consultant.
\end{description}
...
Un groupe est l’association d’au moins deux individus afin de
...
des rôles différents divergent. 
    \item[Les normes] : attitude à avoir lorsque l’on est dans un
        groupe. On peut diviser les normes en 4 catégories :
        \begin{enumerate}
           \item performance (quotas, façon de faire, etc\ldots) ;
           \item apparence (tenue vestimentaire, loyauté, etc\ldots) ;
           \item interactions sociales
           \item attribution des ressources (salaires, etc\ldots)
       \end{enumerate}
...
\begin{description}
    \item[De 1900 à 1925] : la motivation économique. Nous sommes dans une période
...
Taylor et l’organisation scientifique du travail\ldots
    \item[De 1925-1950] : la motivation par la satisfaction des besoins. Durant
...
tâches, l’autonomie, etc\ldots De là quelques grandes théories et
expériences émergent.
    \item[De 1950 à 1975] : l’influence de l’environnement sur la motivation. Les
...
alors de la bonne relation au sein d’une équipe, etc\ldots
     \item[De 1975 à 2000] :  variation sur un même thème. On approfondi ce qui a
...
On revient sur la théorie des besoins de Maslow qui dit qu’une fois qu'un
...
lié à un résultat et à une source de récompenses qu’ils valorisent. Il
