...
Il existe d'autres types de conversions électromécaniques :
\begin{itemize}
	\item Conversion d'un signal électrique en action mécaniques :
	actionneurs ;
	\item Conversion d'un signal mécanique en signal électrique :
	capteurs.
\end{itemize}
...
\node[root] {Convertisseur électromécanique}
% The first level, as children of the initial tree
  child {node[level 2] (c1) {Couplage via le champ magnétique}}
  child {node[level 2] (c2) {Couplage via le champ électrique}}
  child {node[level 2] (c3) {Couplage piézoélectrique - magnétostrictif}};

% The second level, relatively positioned nodes
\begin{scope}[every node/.style={level 3}]
\node [below of = c1, xshift=15pt] (c11) {Un conducteur parcouru par un courant
plongé dans un champ magnétique subit une \textbf{force électrodynamique}};
\node [below of = c11] (c12) {Un corps ferromagnétique plongé dans un champ magnétique
subit une \textbf{force reluctance} qui l’attire vers une position maximisant le flux
magnétique produit};
