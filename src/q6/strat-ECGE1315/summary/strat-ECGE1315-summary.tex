...
\textbf{Une alliance} : collaboration entre deux organisations
concurrentes.\newline

\textbf{Un partenariat} : collaboration entre deux organisations qui ne
sont pas concurrentes.\newline

\textbf{Une alliance complémentaire} : échange entre pôles d'excellence
pour assurer un apprentissage commun en s'appuyant sur l'expertise de
chacun.\newline

\textbf{Une alliance supplémentaire} : cumulation de forces de plusieurs
organisations de manière à renforcer la visibilité et la crédibilité sur
les chances de succès d'un projet.\newline

\textbf{Un partenariat d'impartition} : collaboration entre des
organisations qui ont des relations client/fournisseur.\newline

\textbf{Un partenariat symbiotique} : collaboration entre des
organisations qui ne sont pas concurrentes et qui n'ont pas de relation
client/fournisseur.
...
Une \textbf{acquisition} consiste au rachat d'une organisation par une autre
organisation. \newline

Une \textbf{fusion} est une décision mutuellement consentie par des organisations
...
\section{La performance parentale}

Dans les organisations diversifiées, les directions générales améliorent
(ou détruisent) la performance des DAS, c'est ce qu'on appelle
l'\textbf{avantage parental}. \newline

Ceci se passe principalement en faisant des bons contrôles, en mettant
les bonnes ressources à disposition, en insuflant des intentions
stratégiques,\ldots L'inverse peut aussi se produire si on crée de la
bureaucratie p.e.

\subsection{Les logiques parentales}

\begin{description}
    \item[Gestionnaire de portefeuille] : le siège agit pour le compte
        des actionnaires, il rachète des DAS, leur alloue et leur retire
        des ressources et les revend.
    \item[Gestionnaire de synergies] : le siège accroît les performances
        des DAS en s'appuyant sur les synergies entre eux.
    \item[Développeur] : le siège utilise ses propres compétences pour
        améliorer la performance des DAS.
\end{description}

\section{Les matrices de gestion de portefeuille}

\subsection{La matrice BCG}

\begin{center}
    \includegraphics[width=\linewidth]{matrice_bcg.png}
\end{center}

\begin{center}
    \includegraphics[width=\linewidth]{matrice_bcg_desc.png}
\end{center}

La matrice BCG est un vieux modèle qui fonctionne bien avec les
stratégies de volume et l'autofinancement. \newline

\subsection{La matrice McKinsey}

\begin{center}
    \includegraphics[width=\linewidth]{matrice_mckinsey.png}
\end{center}

\begin{center}
    \includegraphics[width=\linewidth]{matrice_mckinsey_desc.png}
\end{center}

La matrice McKinsey est simple d'utilisation mais les termes utilisés
sont mal définis, elle utilise l'hypothèse d'autofinancement et fait
preuve d'une simplification excessive.

\subsection{La matrice Ashridge}

\begin{center}
    \includegraphics[width=\linewidth]{matrice_ashridge.png}
\end{center}

\section{Fusions et acquisitions}

Nous avons vu que les orientations stratégiques peuvent mener à
plusieurs options (diversification, internationalisation, innovation).

Nous pouvons les regrouper en 3 catégories :

\begin{enumerate}
    \item Croissance interne
    \item Croissance externe (fusions et acquisitions)
    \item Collaboration (alliances et partenariats)
\end{enumerate}

La croissance interne (organique) consiste à développer des stratégies à
partir des capacités déjà existantes de l'organisation. Elle a comme
avantages d'accroître l'apprentissage, d'étaler l'investissement dans le
temps,\ldots Cependant, cela ne marche que s'il ne faut pas le nouveau
service tout de suite et si on dispose d'une certaine indépendance.
(Culture d'intrapreunariat). \newline

La stratégie collective consiste à construire un réseau d'alliances ou
de partenariats qui affronte d'autres réseaux équivalents.\newline

L'avantage collaboratif est la capacité d'une organisation à mieux gérer
les alliances et partenariats que ses concurrents.\newline

\subsection{Motivations}

Les motivations stratégiques peuvent être :
...
\begin{itemize}
    \item Efficience financière
    \item Optimisation fiscale (installation à l'étranger)
    \item Vente par appartements (vente en morceaux d'organisation)
\end{itemize}
\bigskip

Les motivations managériales peuvent être :

\begin{itemize}
    \item Ambitions personnelles
    \item Effets de mode
\end{itemize}

\subsection{Processus}

Les choix doivent être guidés par :

\begin{itemize}
    \item Compatibilité stratégique
    \item Compatibilité organisationnelle
\end{itemize}
\bigskip

L'évaluation consiste à établir le coût d'une acquisition (par exemple
la prime de contrôle nécessaire pour acheter les actions).\newline

L'intégration doit être guidée par :

\begin{itemize}
    \item Interdépendance stratégique
    \item Besoin d'autonomie organisationnelle
\end{itemize}

\subsection{Matrice d'intégration d'une acquisition}

\begin{center}
    \includegraphics[width=0.7\linewidth]{matrice_integration_acquisition.png}
\end{center}

\subsection{Justice organisationnelle}

Pour la réussite d'une fusion en particulier, mais également pour la
réussite d'une acquisition, il faut s'assurer qu'il y a une certaine
justice organisationnelle. \newline

Cette justice doit être de type

\begin{itemize}
    \item distributive
    \item procédurale
    \item informationnelle
\end{itemize}

\section{Alliances et partenariats}

Les alliances et partenariats peuvent être de plusieurs types :

\begin{itemize}
    \item Alliance complémentaire (Star Alliance et les compagnies
        aériennes)
    \item Alliance supplémentaire (Airbus)
    \item Partenariat d'impartition (Intel)
    \item Partenariat symbiotique (Nike+iPod)
\end{itemize}

\subsection{Les formes de partenariats}

Les partenariats donnent lieu à la création de structures communes,
prenant la forme de \textbf{joint-venture} ou \textbf{consortium}.
\newline

Certains partenariats sont uniquement des partenariats contractuels qui
permettent par exemple d'utiliser une marque ou de profiter d'un
service, sous la forme de \textbf{franchise}, de \textbf{licence} ou de
\textbf{sous-traitance}.

\subsection{Acheter, collaborer ou faire soi-même?}

\begin{tabular}{p{2.1cm}||p{3.5cm}|p{3.5cm}|p{3.5cm}}
    & \textbf{Acheter} & \textbf{Collaborer} & \textbf{Faire soi-même} \\ \hline \hline
    \textbf{Urgence} & Très rapide & Rapide & Lent \\ \hline
    \textbf{Incertitude} & Possibilité de revente en cas d'échec & Pertes
    partagées ; Possibilité de rachat & Vente difficile en cas d'échec
    \\ \hline
    \textbf{Capacités intangibles} & Problèmes de culture ou d'évaluation &
    Problèmes de culture et de contrôle & Cohérence de la culture \\
    \hline
    \textbf{Capacités autonomes} & Nécessité d'acheter la totalité & Collaborer
    avec l'activité concernée & Créer un isolat \\ \hline
\end{tabular}
