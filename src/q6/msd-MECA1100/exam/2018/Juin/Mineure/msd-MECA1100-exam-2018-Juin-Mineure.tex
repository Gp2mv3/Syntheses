...
Après développement, la solution est une exponentielle décroissante : notre système s'amortit, mais pas de réponse harmonique ; elle ne fait pas intervenir de sinusoïde\dots
...
\begin{enumerate}
 \item Écrire les équations régissant le problème de stabilité.
...
\item Nous allons travailler dans la configuration déformée. En outre, nous supposerons, au début, que $V_B$ nous est donnée. Nous calculons le moment fléchissant au moyen de deux coupes :
...
\begin{align*}
\int _0 ^{\xi} M^2(x) \dif{x} &= [\frac{x^3}{3}V_B^2 - \frac{x^2 L}{2}V_B^2 + xL^2V_B^2 - 2V_B F(\frac{x^3}{3} - \frac{x^2}{2}(L + \xi) + \xi Lx)]_0^{\xi} \\
&= \xi^3 V_B (\frac{V_B}{3} - \frac{2}{3}V_B +V_B) + \xi^2LV_B (- \frac{V_B}{2} + \frac{F}{2} -2F) + \xi L^2 V_B^2
\end{align*}

où le terme en $F^2$ n'a pas été calculé car ce terme sera dérivé, exclusivement, par rapport à $V_B$ par la suite\dots
\begin{align*}
\int _{\xi} ^{L} M^2(x) \dif{x} &= [V_B^2 (\frac{V_B}{3} - \frac{2}{3}V_B +L^2x)]_{\xi}^L = V_B^2 (\frac{L^3}{3} - \frac{L^3}{2} + L^3 + \frac{\xi ^3}{3} -\frac{\xi ^2}{2}L + \xi L^2) \\
...
\end{align*}
Où la dernière égalité a été donnée en vertu du théorème de Castigliano.
...
\item Pour représenter les moments, nous avons utilisé (de manière tout à fait arbitraire) la relation de proportionnalité $V_B = \frac{1}{2}F$, et
...
\item Puisque $M(x) = -EI v''(x)$, on trouve les EDO suivantes :
\begin{enumerate}
\item Sur $[0, \xi]$ : $v''(x) = -\frac{1}{EI}(F(\xi - x) - V_B(L-x))$.
...
\item Sur $[\xi, L]$, $v''(x) = -\frac{1}{EI}(- V_B(L-x))$.
...
\item Par le principe de Saint-Venant, les réactions aux appuis calculés au point 1 sont conservées. L'effort normal reste nul, mais nous avons désormais
...
\item Par l'équilibre des forces et des moments,
...
\item La solution générale est de la forme $v(x) = A\sin(kx) + B\cos(kx) +\frac{V_B}{F}(L-x)$, avec $k = \sqrt{\frac{F}{EI}}$. On impose également $v(0)= v'(0) = 0 = v(L) $. Par les conditions sur v en 0 :
...
\item Donnez l'expression de l'énergie cinétique de la structure.
...
\item On considère dorénavant que la position d'équilibre correspond à $\psi=\theta=0$. Calculer les pulsations propres du système en utilisant les matrices de masse et de raideur suivantes;
\[
...
\[E_{p, G} = mgh = \frac{2m}{2L}g \int_0^{2L} x \sin(\psi) \dif{x} + 0 = 2 mgL \sin(\psi)\]
...
\item Les conditions pour que le système soit à l'équilibre sont $E_p = \underset{\underline{x}}{\min} (E_p)$ et $\underline{\dot{x}} = 0$. Supposons donc une vitesse angulaire nulle dans une configuration telle que :
...
\[E_k = \frac{2m}{2L} \frac{1}{2} \int_0^{2L} (x \dot{\psi})^2 \dif{x} + \frac{m}{L}\frac{2}{2} \int_0^{\frac{L}{2}} (x \dot{\theta})^2 \dif{x},\]
...
\[E_k = \frac{4mL^2}{3}\dot{\psi}^2 + \frac{mL^2}{24}\dot{\theta}^2\]

\item
\begin{align*}
	L_{lag} &= E_k - E_p \\
	& = \frac{4mL^2}{3}\dot{\psi}^2 + \frac{mL^2}{24}\dot{\theta}^2 - \frac{kL^2}{2}\left(23 \sin^2(\psi) - \sin(\psi)\sin(\theta) + \frac{1}{2} \sin^2(\theta)\right) - 2 mgL \sin(\psi)
\end{align*}
...
\begin{align*}
\frac{\partial}{\partial t}\frac{\partial L_{lag}}{\partial \underline{\dot{\psi}}} &= \frac{\partial L_{lag}}{\partial \underline{\psi}} \\
\frac{\partial}{\partial t}\frac{\partial L_{lag}}{\partial \underline{\dot{\theta}}} &= \frac{\partial L_{lag}}{\partial \underline{\theta}}.
\end{align*}
...
\item $\omega_i^2 = \lambda_i$, avec $\lambda_i$ tel que
