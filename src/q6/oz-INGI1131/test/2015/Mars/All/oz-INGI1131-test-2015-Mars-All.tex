...
\[ B_{n} = B_{n-1} + A_{n-1} - 2*A_{n-1}*B_{n-1} \]

avec :

\[ B_{n-1} = B_{n-2} + A_{n-2} - 2*A_{n-2}*B_{n-2} \]

en exécutant cette définition récursive, on en revient au cas de base donc:

\[ B_{n} =  ( ...( ( A_{0} + A_{1} - 2*A_{0}*A_{1} ) + A_{2} - 2*( A_{0} + A_{1} - 2*A_{0}*A_{1} )*A_{2} ) + A_{3} ...) + A_{n-1} - 2*...*A_{n-1}  \] (B_{0} et B_{1} ayant seulement une influence sur B_{2} on commence avec la formule de B_{2})
\end{solution}
