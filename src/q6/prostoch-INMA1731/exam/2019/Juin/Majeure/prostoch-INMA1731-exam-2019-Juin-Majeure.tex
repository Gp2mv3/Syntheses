...
Exam during 3h with 3 questions: 2 questions by PAA et 1 question by LV.

\section{PAA 1}

Let $G(z)$ be an LTI system which is stable and causal. 
One inputs a white signal $X$ of variance $\sigma_x^2$ in the system and outputs $Y$.
...
\begin{enumerate}
    ...
    \item Realize the system by a recurrence equation.
...
\section{PAA 2}
Let Z be an observed random vector in $\mathbb{R}^n$ and $g:\mathbb{R}^n\to \mathbb{R}$ an estimation rule for a stochastic estimation $\Theta$ (Bayesian estimation). Suppose that $\mathbb{E}[g(Z)]=\mathbb{E}[\Theta]$ and that we have a function $h:\mathbb{R}^n \to \mathbb{R}$ such that $\mathbb{E}[h(Z)]=m_{h(Z)}$ is known. Consider the new estimation rule \begin{equation}
...
\end{equation}
\begin{enumerate}
    \item Derive and provide the optimum smoother coefficients $W(\exp^{j\Omega})$ for an estimator optimized according to the MMSE or Wiener criterion. Express your result as a function of the power spectral densities of $x[k]$ and $\nu[k]$, and $H(\exp^{j\Omega})$.
\end{enumerate}

We then investigate a solution where the first step is to whiten the noise (make it white). we know that there exists a filter $L_\nu(\exp^{j\Omega})$ such that $\gamma_\nu(\exp^{j\Omega})=L_\nu(\exp^{j\Omega})$. By passing the signal $y[k]$ in a filter with frequency response $\frac{1}{L_\nu(\exp^{j\Omega})}$, we obtain a new signal $y_2[k]$ from which we investigate the MMSE estimation of $x[k]$ with the help of an optimum smoother with coefficients $w_2[k]$, that is 
\begin{equation}
    \hat{x}'[k]=\sum_{l=-\infty}^{\infty}w_2[l]\: y_2[k-l]
\end{equation}

\begin{enumerate}[resume]
    \item Derive and provide the optimum smoother coefficients $W_2(\exp^{j\Omega})$ for an estimator of $x[k]$ optimized according to the MMSE or Wiener criterion. Express your result as un function of the power spectral densities of $x[k]$ and $\nu[k]$, and $H(\exp^{j\Omega})$.
